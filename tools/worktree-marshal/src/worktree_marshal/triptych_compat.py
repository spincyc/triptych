"""Triptych-compatible Codex worktree lifecycle engine.
...
def integration_source_ref(manifest: dict) -> str:
    ...
    validate_run_id(run_id)
    return f"{SOURCE_ANCHOR_PREFIX}{run_id}/integration-source"
...
def retirement_anchor_ref(manifest: dict) -> str:
    ...
    validate_run_id(run_id)
    return f"{SOURCE_ANCHOR_PREFIX}{run_id}/retirement-discard"
...
def retirement_receipt_ref(manifest: dict) -> str:
    ...
    validate_run_id(run_id)
    return f"{SOURCE_ANCHOR_PREFIX}{run_id}/retirement-receipt"
...
def private_run_refs(repository: Repository, manifest: dict) -> set[str]:
    ...
    validate_run_id(run_id)
    prefix = f"{SOURCE_ANCHOR_PREFIX}{run_id}/"
    result = git(
        repository.root,
        "for-each-ref",
        "--format=%(refname)",
                ...
                try:
                    ...
                    if manifest.get("state") == "integration-cleanup-pending":
                        ...
                        diagnostic(
                            ...
                            f"retry scripts/triptych-codex --triptych-clean "
                            f"{manifest['run_id']}"
                        )
                    else:
                        preserve_run(repository, manifest, exit_code, audit)
...
def launch_resolver(
    ...
    authenticate_retained_worktree(repository, manifest)
    require_run_tmp_directory(repository, manifest)
    argv = codex_argv(real_codex, child_cwd, [RESOLVER_PROMPT])
        ...
        with file_lock(repo_lock_path(repository)):
            ...
            try:
                ...
                raise LauncherError(
                    ...
                    "remains correctable with --triptych-resolve or restorable with "
                    "--triptych-abort"
                ) from exc
            except LauncherError as exc:
                record_integration_recovery_failure(
        ...
        if exit_code == 0:
            if paths:
                diagnostic(
                    ...
                    f"path(s); reopen the resolver with --triptych-resolve"
                )
            else:
                diagnostic(
                    f"run {manifest['run_id']} resolver exited; after all resolutions are "
                    f"staged, continue with scripts/triptych-codex --triptych-continue "
                    f"{manifest['run_id']}"
                )
        return exit_code
    finally:
...
def continue_conflict_run(repository: Repository, run_id: str) -> int:
        ...
        try:
            ...
            if paths:
                ...
                raise LauncherError(
                    ...
                    f"resolve and stage them with scripts/triptych-codex "
                    f"--triptych-resolve {run_id}"
                )
            ...
            if continue_rebase.returncode:
                ...
                if rebase_in_progress(worktree):
                    ...
                    raise LauncherError(
                        ...
                        f"{len(paths)} path(s); open scripts/triptych-codex "
                        f"--triptych-resolve {run_id}"
                    )
                record_integration_recovery_failure(
                    repository,
    ...
    diagnostic(
        ...
        f"authorization, land it with scripts/triptych-codex --triptych-integrate {run_id}"
    )
    return 0
                ...
                try:
                    ...
                    if retain_retryable_restoration_cleanup(
                        ...
                        raise LauncherError(
                            ...
                            "cleanup did not complete; retry --triptych-abort"
                        ) from exc
            ...
            try:
                ...
                if retain_retryable_restoration_cleanup(
                    ...
                    raise LauncherError(
                        ...
                        "did not complete; retry --triptych-abort"
                    ) from exc
                failed = audit_run(repository, manifest)
                record_integration_recovery_failure(
                    repository,
    ...
    with file_lock(repo_lock_path(repository)):
        ...
        if manifest.get("state") in RETIREMENT_PENDING_STATES:
            raise LauncherError(
                ...
                f"scripts/triptych-codex --triptych-clean {run_id}"
            )
        if manifest.get("integrated_head") is not None:
            raise LauncherError(
        ...
        if manifest.get("state") in {
            ...
            raise LauncherError(
                ...
                f"staging-only resolver with scripts/triptych-codex "
                f"--triptych-resolve {run_id}"
            )
        if audit.branch != manifest["branch"]:
        ...
        try:
            ...
            if transaction_state in {
                ...
                raise LauncherError(
                    ...
                    f"path(s); use --triptych-resolve, --triptych-continue, or "
                    f"--triptych-abort with this run ID"
                )
            if transaction_state == "integration-abort-pending":
                raise LauncherError(
                    f"run {run_id} has an interrupted managed abort; retry "
                    f"scripts/triptych-codex --triptych-abort {run_id}"
                )
            if transaction_state in {
                "integration-rebase-pending",
                "integration-rebase-rollback-pending",
                ...
                if transaction_state == "integration-rebase-pending":
                    ...
                    if outcome == "conflict":
                        ...
                        raise LauncherError(
                            ...
                            f"conflict in {len(paths)} path(s); use --triptych-resolve, "
                            "--triptych-continue, or --triptych-abort"
                        )
                    ...
                    if rebase.returncode:
                        ...
                        if rebase_in_progress(Path(manifest["worktree"])):
                            ...
                            raise LauncherError(
                                ...
                                f"scripts/triptych-codex --triptych-resolve {run_id}, then "
                                f"use scripts/triptych-codex --triptych-continue {run_id} "
                                f"or scripts/triptych-codex --triptych-abort {run_id}"
                            )
        ...
        try:
            ...
            if state in RETIREMENT_PENDING_STATES:
                ...
                raise LauncherError(
                    ...
                    "--triptych-clean cannot discard it"
                )
            elif state in {
                "integrated-pending-cleanup",
                "integration-cleanup-pending",
                ...
                if manifest.get("state") in {
                    ...
                    raise LauncherError(
                        ...
                        f"--triptych-continue {run_id} or restore it with "
                        f"--triptych-abort {run_id}"
                    )
