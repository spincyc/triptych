 ...
 * didach.ai identity prototypes. The real public preview is always served below the
 * GitHub-Pages-style /triptych/ prefix. "Before" cases receive those bytes
 * unchanged; "after" cases receive a bounded in-memory identity adapter plus
 * the isolated prototype CSS and JS. Generated preview files remain untouched.
 */
...
const HARNESS = resolve(REPO, 'tools/tests/didach_identity_prototype_browser.mjs');
const PREVIEW = resolve(process.env.TRIPTYCH_REVIEW_ROOT || join(REPO, 'build/public-alpha/preview'));
const PROJECT_PREFIX = '/triptych';
const PROTOTYPE_ROOT = resolve(REPO, 'src/web/browser/prototypes/didach-identity');
...
const SOCIAL_CARD_SPECIMEN = join(PROTOTYPE_ROOT, 'social-card.html');
const ICON_SPECIMEN = join(PROTOTYPE_ROOT, 'icon.html');
const DOCUMENT_CORPUS = join(PREVIEW, 'browse/structure/documents/corpus.json');
const DEFAULT_MATRIX = resolve(REPO, 'tools/tests/fixtures/didach-identity-prototype-matrix-v1.json');
...
function usage() {
  return `usage: didach_identity_prototype_browser.mjs [options]\n\n` +
    `Serve build/public-alpha/preview beneath /triptych/, run the ordered identity\n` +
    `before/after matrix in real Chromium, and optionally write deterministic\n` +
    `viewport captures and canonical JSON. After cases apply a bounded in-memory\n` +
    `HTML identity adapter plus the matrix's isolated CSS and JavaScript.\n\n` +
    `options:\n` +
    `  --matrix FILE       matrix JSON (default: tools/tests/fixtures/didach-identity-prototype-matrix-v1.json)\n` +
    `  --capture-dir DIR   required by this matrix; write captures and print evidence into an absent or empty directory\n` +
...
function validateMatrix(value) {
  ...
  if (css !== PROTOTYPE_CSS || javascript !== PROTOTYPE_JAVASCRIPT) {
    throw new Error('matrix prototype assets must name the isolated didach-identity CSS and JavaScript');
  }
  ...
  const cases = value.cases.map((held, index) => {
    ...
    if (!ALLOWED_VIEWPORTS.has(viewportName)) {
      throw new Error(`${held.id} viewport ${viewportName} is outside the binding identity matrix`);
    }
  ...
  async function walk(directory, prefix = '') {
    ...
    for (const child of children) {
      ...
      if (child.isDirectory()) {
        ...
        entries.push([relativePath, await sha256(path)]);
      }
    }
  }
  await walk(root);
  const digest = createHash('sha256');
  for (const [path, hash] of entries) digest.update(`${hash}  ${path}\n`);
...
function protectedWorktreeAssertion() {
  const base = gitText(['merge-base', 'HEAD', 'origin/main']);
  const changed = [
    ...gitText(['diff', '--name-only', base, '--']).split('\n'),
    ...gitText(['ls-files', '--others', '--exclude-standard']).split('\n')
  ].filter(Boolean);
  const allowed = [
    'AGENTS.md', 'PROJECT-WORK.md', 'promised-deliverables.toml',
    'guidance/didach-identity.md', 'guidance/didach-identity-research.md',
    'guidance/didach-identity-roadmap.md',
    'src/web/browser/prototypes/didach-identity/',
    'tools/tests/didach_identity_prototype_browser.mjs',
    'tools/tests/fixtures/didach-identity-prototype-matrix-v1.json',
    'tools/tests/test_didach_identity_prototype.py'
  ];
  const violations = [...new Set(changed.filter((path) => path !== 'directions' &&
    !allowed.some((entry) => entry.endsWith('/') ? path.startsWith(entry) : path === entry)
  ))].sort();
  return {
    base,
    row: assertion('worktree-task-scope-contained', violations.length === 0,
      JSON.stringify({base, violations}), true)
  };
}
...
function injectPrototype(html, publication, route) {
  ...
  const surface = route === '/' ? 'home' : route === '/texts/' ? 'publications' :
    route === '/catena/' ? 'catena' : route === '/sources/' ? 'sources' :
    route === '/404.html' ? 'error' : 'reader';
  ...
  let output = html.replace(
    /<html\b/i,
    `<html data-didach-prototype-phase="after" data-didach-static-identity="didach.ai" ` +
      `data-didach-identity="${surface}"${recordAttributes}`
  );
  output = output.replace(/<title>([\s\S]*?)<\/title>/i, (_match, held) => {
    const title = held.replace(/(?:\s*[·—-]\s*)Triptych\s*$/u, '').trim();
    const exact = surface === 'home' ? 'didach.ai — A source-first Catholic corpus' :
      surface === 'publications' ? 'Publications · didach.ai' :
      surface === 'catena' ? 'Catena Omnia · didach.ai' :
      surface === 'sources' ? 'The Source Library · didach.ai' :
      surface === 'error' ? 'Page not found · didach.ai' :
      record?.provider?.label ? `${title} — ${record.provider.label} treatment · didach.ai` :
      `${title} · didach.ai`;
    return `<title>${exact}</title>`;
  });
  output = output.replace(
    /<div class="brand">\s*<a([^>]*)>[\s\S]*?<\/a>\s*<span>[\s\S]*?<\/span>\s*<\/div>/i,
    (_match, attributes) => {
      const retained = attributes.replace(/\s+href=(?:"[^"]*"|'[^']*')/i, '');
      return `<div class="brand" data-didach-wordmark><a${retained} href="${PROJECT_PREFIX}/" ` +
        `aria-label="didach.ai, home"${surface === 'home' ? ' aria-current="page"' : ''}>` +
        'didach.ai</a></div>';
    }
  );
  const staticCurrent = surface === 'reader' ? 'publications' : surface;
  const staticBrowse = [
    ['Publications', 'texts/', 'publications'],
    ['Sources', 'sources/', 'sources'],
    ['Commentary', 'catena/', 'catena']
  ].map(([label, path, domain]) =>
    `<a href="${PROJECT_PREFIX}/${path}"${staticCurrent === domain ? ' aria-current="page"' : ''}>` +
      `${label}</a>`
  ).join('');
  output = output.replace(
    /<nav aria-label="Primary navigation">[\s\S]*?<\/nav>/i,
    `<nav aria-label="Primary navigation" data-didach-static-nav>${staticBrowse}</nav>`
  );
  output = output.replace(
    /<footer class="site-footer">/i,
    '<footer class="site-footer"><div class="didach-identity-footer" data-didach-identity-footer>' +
      '<p><strong>didach.ai</strong> — a source-first corpus for Catholic faith, worship, and law.</p>' +
      '<p>Independent study material; not an official publication of the Catholic Church.</p>' +
      '<p>AI assists the work. Claims stand or fall by the identified sources and editions; ' +
      'citations, rights, and stated limits make that basis inspectable.</p>' +
      '<nav class="didach-footer-links" aria-label="About, method, sources, and policy links"><ul>' +
      `<li><a href="${PROJECT_PREFIX}/about.html">About</a></li>` +
      `<li><a href="${PROJECT_PREFIX}/about.html">Method and AI use</a></li>` +
      `<li><a href="${PROJECT_PREFIX}/sources/">Sources</a></li>` +
      `<li><a href="${PROJECT_PREFIX}/contributing.html">Contributing</a></li>` +
      `<li><a href="${PROJECT_PREFIX}/license.html">Licensing</a></li>` +
      `<li><a href="${PROJECT_PREFIX}/third-party.html">Third-party material</a></li>` +
      `<li><a href="${PROJECT_PREFIX}/contributing.html">Feedback</a></li>` +
      `<li><a href="${PROJECT_PREFIX}/about.html#source-and-contact">Source code</a></li>` +
      '</ul></nav><p class="didach-transition-note">didach.ai is the public home of the Triptych corpus.</p></div>'
  );
  if (surface === 'error') {
    output = output.replace(/\b(href|src)="assets\//gi,
      (_match, attribute) => `${attribute}="${PROJECT_PREFIX}/assets/`);
    output = output.replace(
      /<main\b([^>]*)>[\s\S]*?<\/main>/i,
      `<main$1 data-wave-surface="error" data-wave-surface-main="error">` +
      '<h1 id="page-not-found" data-wave-title>Page not found</h1>' +
      '<p>This address is not part of the published corpus.</p>' +
      `<p class="wave-error-actions"><a href="${PROJECT_PREFIX}/">Go to didach.ai</a>` +
      `<a href="${PROJECT_PREFIX}/texts/">Browse publications</a></p></main>`
    );
  }
  if (surface === 'home') {
    output = output.replace(
      /<h1 id="library">Library<\/h1>\s*<p>[\s\S]*?<\/p>/i,
      '<h1 id="library">A source-first corpus for Catholic faith, worship, and law.</h1>' +
      '<p>Read publications and Scripture; trace claims to editions and passages; follow ' +
      'commentary, liturgy, historical change, and canon law.</p>'
    );
  }
  if (record?.pdfHref) {
    output = output.replace(
      /<main\b[^>]*>/i,
      (main) => main + '<noscript><p class="didach-static-pdf"><a data-wave-pdf="recorded-static" ' +
        `href="${htmlAttribute(record.pdfHref)}">Canonical PDF</a></p></noscript>`
    );
  }
  output = output.replace(
    /<\/head\s*>/i,
    `  <link rel="stylesheet" data-didach-prototype-css href="${PROJECT_PREFIX}/__didach/prototype.css">\n</head>`
  );
  return output.replace(
    /<\/body\s*>/i,
    `  <script defer data-didach-prototype-js src="${PROJECT_PREFIX}/__didach/prototype.js"></script>\n</body>`
  );
}

function governedPrototypeRoute(route, publications) {
  if (new Set(['/', '/texts/', '/catena/', '/sources/', '/404.html']).has(route)) return true;
...
function staticServer(state) {
  return createServer(async (request, response) => {
    try {
      ...
      if (url.pathname === `${PROJECT_PREFIX}/__didach/prototype.css`) {
      ...
      if (url.pathname === `${PROJECT_PREFIX}/__didach/prototype.js`) {
      ...
      let held;
      try {
        held = await stat(file);
      } catch (_error) {
        if (request.headers['sec-fetch-dest'] !== 'document') throw _error;
        file = join(PREVIEW, '404.html');
        held = await stat(file);
        status = 404;
      }
      ...
      const adapterRoute = status === 404 ? '/404.html' : route;
      if (state.phase === 'after' && extname(file).toLowerCase() === '.html' &&
          governedPrototypeRoute(adapterRoute, state.publications)) {
        body = Buffer.from(injectPrototype(
        body.toString('utf8'), state.publications.get(adapterRoute) || null, adapterRoute
      ), 'utf8');
      }
...
async function protectedSurfaceAssertions(base, state) {
  ...
  try {
    for (const route of protectedRoutes) {
      ...
      const injected = actual.includes(Buffer.from('data-didach-prototype-'));
...
async function pageFacts(cdp, held) {
  return evaluate(cdp, `(() => {
    ...
    return {
      ...
      identity: (() => {
        const links = [...document.querySelectorAll('[data-didach-wordmark] a')];
        const visibleLinks = links.filter((node) => node.getClientRects().length > 0);
        const link = visibleLinks[0] || links[0];
        let href = null;
        if (link) {
          const url = new URL(link.href, location.href);
          href = url.pathname === ${JSON.stringify(PROJECT_PREFIX)}
            ? '/' : url.pathname.slice(${PROJECT_PREFIX.length}) + url.search + url.hash;
        }
        return {
          count: links.length,
          visibleCount: visibleLinks.length,
          text: link?.textContent?.trim() || null,
          accessibleName: link?.getAttribute('aria-label') || link?.textContent?.trim() || null,
          href,
          current: link?.getAttribute('aria-current') || null,
          title: document.title
        };
      })(),
      visibleCurrentCount: [...document.querySelectorAll('[aria-current="page"]')]
      ...
      phaseMarker: document.documentElement.dataset.didachPrototypePhase || null,
      injectedCss: Boolean(document.querySelector('[data-didach-prototype-css]')),
      injectedJs: Boolean(document.querySelector('[data-didach-prototype-js]')),
      prototypeRan: document.body.hasAttribute('data-wave-ready'),
      identityStyle: (() => {
        const wordmark = document.querySelector('[data-didach-wordmark] a');
        const shell = document.querySelector('[data-wave-shell], .site-header');
        if (!wordmark || !shell) return null;
        const wordmarkStyle = getComputedStyle(wordmark);
        const shellStyle = getComputedStyle(shell);
        return {
          wordmarkColor: wordmarkStyle.color,
          shellBorderWidth: parseFloat(shellStyle.borderBottomWidth) || 0,
          shellBorderStyle: shellStyle.borderBottomStyle
        };
      })(),
      motion: (() => {
        let maximumMs = 0;
        for (const node of document.querySelectorAll('[data-wave-shell], [data-wave-shell] *, dialog, dialog *')) {
          const style = getComputedStyle(node);
          for (const value of [...style.transitionDuration.split(','), ...style.animationDuration.split(',')]) {
            const duration = value.trim().endsWith('ms') ? parseFloat(value) : parseFloat(value) * 1000;
            if (Number.isFinite(duration)) maximumMs = Math.max(maximumMs, duration);
          }
        }
        return {maximumMs};
      })(),
      textSpacing: (() => {
        ...
        return {
          lineHeightRatio: (parseFloat(style.lineHeight) || 0) / fontSize,
          paragraphSpacingRatio: (parseFloat(style.marginBottom) || 0) / fontSize,
          letterSpacingEm: (parseFloat(style.letterSpacing) || 0) / fontSize,
          wordSpacingEm: (parseFloat(style.wordSpacing) || 0) / fontSize
        };
      })(),
      shell: (() => {
        ...
        return {
          ...
          focusableOrder: [...document.querySelectorAll(
            '[data-wave-shell] a[href], [data-wave-shell] button:not([disabled])'
          )].filter(visible).map((node) =>
            (node.getAttribute('aria-label') || node.textContent || '').trim()),
          visibleCurrent: [...document.querySelectorAll(
            '[data-wave-shell] a[aria-current="page"]'
          )].filter(visible).map((node) => node.textContent.trim())
        };
      })(),
      dialog: (() => {
        const dialog = document.querySelector('dialog[data-wave-dialog-owner]');
        let backgroundFocusBlocked = null;
        if (dialog?.open) {
          const held = document.activeElement;
          document.querySelector('[data-didach-wordmark] a')?.focus({preventScroll: true});
          backgroundFocusBlocked = Boolean(dialog.contains(document.activeElement));
        ...
        return {
          ...
          view: dialog?.dataset.waveDialogView || null,
          links: [...(dialog?.querySelectorAll('[data-wave-dialog-body] a[href]') || [])]
            .filter((node) => node.getClientRects().length > 0)
            .map((node) => node.textContent.trim())
        };
      })(),
      homeStructure: (() => ({
        tasks: [...document.querySelectorAll('[data-wave-task-destination]')]
          .map((node) => node.textContent.trim()),
        portals: [...document.querySelectorAll('[data-wave-portals] tbody tr')]
          .map((row) => row.cells[0]?.textContent.trim() || ''),
        secondary: [...document.querySelectorAll('[data-wave-secondary-route]')]
          .map((node) => node.dataset.waveSecondaryRoute)
      }))(),
      publicationRecord,
...
function expectedBrowserTitle(held, publication) {
  if (held.surface === 'home') return 'didach.ai — A source-first Catholic corpus';
  if (held.surface === 'publications') return 'Publications · didach.ai';
  if (held.surface === 'reader') {
    return publication?.title && publication?.payload?.provider?.label
      ? `${publication.title} — ${publication.payload.provider.label} treatment · didach.ai`
      : null;
  }
  if (held.surface === 'catena') {
    if (held.emulation === 'no-js') return 'Catena Omnia · didach.ai';
    const selected = new URLSearchParams(
      new URL(held.route, 'https://invalid.example').hash.slice(1)
    );
    const books = {Gen: 'Genesis', Ex: 'Exodus', Ps: 'Psalms'};
    const book = books[selected.get('book')];
    const chapter = selected.get('chapter');
    return book && chapter ? `${book} ${chapter} · Catena Omnia · didach.ai` :
      'Catena Omnia · didach.ai';
  }
  if (held.surface === 'sources') return 'The Source Library · didach.ai';
  if (held.surface === 'error') return 'Page not found · didach.ai';
  return null;
}
...
function recordAssertions(
  ...
  if (held.phase === 'after') {
    const expectedVisibleWordmarks = held.emulation === 'print' ? 0 : 1;
    rows.push(assertion('public-wordmark-exact', facts.identity?.count === 1 &&
      facts.identity?.visibleCount === expectedVisibleWordmarks && facts.identity?.text === 'didach.ai',
      JSON.stringify(facts.identity), true));
    rows.push(assertion('public-wordmark-accessible-name',
      facts.identity?.accessibleName === 'didach.ai, home', JSON.stringify(facts.identity), true));
    rows.push(assertion('public-wordmark-home-link', facts.identity?.href === '/',
      JSON.stringify(facts.identity), true));
    rows.push(assertion('public-wordmark-current-only-on-home',
      (held.surface === 'home' && facts.identity?.current === 'page') ||
      (held.surface !== 'home' && facts.identity?.current === null),
    JSON.stringify(facts.identity), true));
    rows.push(assertion('one-visible-current-location-or-unowned-error',
      facts.visibleCurrentCount === (
        held.surface === 'error' || held.emulation === 'print' ? 0 : 1
      ),
    `visibleCurrentCount=${facts.visibleCurrentCount}`, true));
    const exactTitle = expectedBrowserTitle(held, publication);
    rows.push(assertion('browser-title-exact', Boolean(exactTitle) && facts.identity?.title === exactTitle,
      `expected=${JSON.stringify(exactTitle)} actual=${JSON.stringify(facts.identity?.title || null)}`, true));
  }
  if (held.surface === 'reader') {
  ...
  const allowedFailurePaths = new Set(held.expect.allowedHttp.map((row) => row.path));
  const unexpectedFailures = channel.failed.filter((row) =>
    !allowedFailurePaths.has(projectRequest(row.url).path)
  );
  rows.push(assertion('request-clean', unexpectedFailures.length === 0,
    unexpectedFailures.slice(0, 8).map((row) => `${row.error}: ${row.url}`).join(' | '), true));
  ...
  if (held.phase === 'after' && !['no-js', 'print'].includes(held.emulation)) {
    ...
    rows.push(assertion('shell-jump-visible', facts.shell.jumpVisible === true,
      JSON.stringify(facts.shell), true));
    if (wideShell) {
      const expectedCurrent = held.surface === 'error' ? 0 : 1;
      rows.push(assertion('wide-shell-one-current-location',
        facts.shell.desktopVisible === true && facts.shell.visibleCurrent.length === expectedCurrent,
      JSON.stringify(facts.shell), true));
    ...
    const shellOrders = {
      home: ['didach.ai, home', 'Publications', 'Browse', 'Jump'],
      publications: ['didach.ai, home', 'Publications', 'Browse', 'Jump'],
      reader: ['didach.ai, home', 'Publications', 'Browse', 'Jump'],
      catena: ['didach.ai, home', 'Publications', 'Commentary', 'Browse', 'Jump'],
      sources: ['didach.ai, home', 'Publications', 'Sources', 'Browse', 'Jump'],
      error: ['didach.ai, home', 'Publications', 'Browse', 'Jump']
    };
    const expectedShellOrder = wideShell ? shellOrders[held.surface] :
      ['didach.ai, home', 'Menu', 'Jump'];
    rows.push(assertion('shell-focusable-order-exact',
      JSON.stringify(facts.shell.focusableOrder) === JSON.stringify(expectedShellOrder),
    JSON.stringify(facts.shell), true));
  }
  if (held.phase === 'after' && held.surface === 'home' && held.emulation !== 'no-js') {
    rows.push(assertion('home-six-task-entrances-exact',
      JSON.stringify(facts.homeStructure.tasks) === JSON.stringify([
        'Read today', 'Find a publication', 'Trace a source', 'Follow commentary',
        'See what changed', 'Look up a canon'
      ]), JSON.stringify(facts.homeStructure), true));
    rows.push(assertion('home-seven-portals-exact',
      JSON.stringify(facts.homeStructure.portals) === JSON.stringify([
        'Faith', 'Scripture', 'Liturgy', 'History', 'Formation', 'Mary', 'Law'
      ]), JSON.stringify(facts.homeStructure), true));
    rows.push(assertion('home-secondary-entrances-exact',
      JSON.stringify(facts.homeStructure.secondary) ===
        JSON.stringify(['propers', 'story-of-salvation']),
    JSON.stringify(facts.homeStructure), true));
  }
  if (held.phase === 'after' && facts.dialog?.open && facts.dialog?.view === 'menu') {
    rows.push(assertion('dialog-native-modal-and-background-inert',
      facts.dialog.modal === true && facts.dialog.backgroundFocusBlocked === true,
    JSON.stringify(facts.dialog), true));
    rows.push(assertion('menu-seven-durable-destinations-exact',
      JSON.stringify(facts.dialog.links) === JSON.stringify([
        'Publications', 'Sources', 'Scripture', 'Liturgy', 'History', 'Law', 'Commentary'
      ]), JSON.stringify(facts.dialog), true));
  }
  if (held.phase === 'after' && facts.dialog?.open && facts.dialog?.view === 'jump' &&
      held.surface === 'home') {
    rows.push(assertion('dialog-native-modal-and-background-inert',
      facts.dialog.modal === true && facts.dialog.backgroundFocusBlocked === true,
    JSON.stringify(facts.dialog), true));
    rows.push(assertion('jump-contextual-home-tasks-exact',
      JSON.stringify(facts.dialog.links) === JSON.stringify([
        'Read today', 'Find a publication', 'Trace a source', 'Follow commentary',
        'See what changed', 'Look up a canon'
      ]), JSON.stringify(facts.dialog), true));
  ...
  const targetSelectorsVisible = new Set(
    facts.targets.filter((target) => target.visible).map((target) => target.selector)
  );
  const missingTargets = held.expect.primaryTargets.filter((one) => !targetSelectorsVisible.has(one));
  ...
  if (held.emulation === 'forced-colors') {
    ...
    rows.push(assertion('forced-colors-identity-visible', Boolean(facts.identityStyle?.wordmarkColor) &&
      facts.identityStyle?.wordmarkColor !== 'rgba(0, 0, 0, 0)' &&
      facts.identityStyle?.shellBorderWidth >= 1 && facts.identityStyle?.shellBorderStyle !== 'none',
    JSON.stringify(facts.identityStyle), gating));
  }
  if (held.emulation === 'reduced-motion') {
    rows.push(assertion('reduced-motion-active', facts.reducedMotion === true,
      `matchMedia=${facts.reducedMotion}`, gating));
    rows.push(assertion('reduced-motion-durations-suppressed', facts.motion?.maximumMs <= 1,
      JSON.stringify(facts.motion), gating));
  }
  ...
  if (held.emulation === 'zoom-400') {
    rows.push(assertion('diagnostic-page-scale-400-percent', facts.visualViewport?.scale >= 3.99,
      JSON.stringify(facts.visualViewport), gating));
  }
  if (held.emulation === 'text-spacing') {
    rows.push(assertion('wcag-text-spacing-applied', facts.textSpacing?.lineHeightRatio >= 1.49 &&
      facts.textSpacing?.paragraphSpacingRatio >= 1.99 &&
      facts.textSpacing?.letterSpacingEm >= 0.119 && facts.textSpacing?.wordSpacingEm >= 0.159,
    JSON.stringify(facts.textSpacing), gating));
  }
  if (held.emulation === 'no-js') {
...
async function runCase(
  ...
  if (held.emulation === 'text-spacing') {
    await evaluate(cdp, `(() => {
      ...
      style.textContent = '* { line-height: 1.5 !important; letter-spacing: .12em !important; ' +
  ...
  const lifecycleOrigin = held.expect.restoreFocusAfterEscape
    ? await evaluate(cdp, `({x: window.scrollX, y: window.scrollY})`) : null;
  ...
  if (held.expect.restoreFocusAfterEscape) {
    ...
    const restoredScroll = await evaluate(cdp, `({x: window.scrollX, y: window.scrollY})`);
    assertions.push(assertion('scroll-restored-after-escape', Boolean(lifecycleOrigin) &&
      Math.abs(restoredScroll.x - lifecycleOrigin.x) <= 1 &&
      Math.abs(restoredScroll.y - lifecycleOrigin.y) <= 1,
    JSON.stringify({origin: lifecycleOrigin, restored: restoredScroll}), held.phase === 'after'));
  }
...
async function main() {
  ...
  try {
    ...
    if (matrix.cases.some((held) => held.emulation === 'print') && !options.captureDir) {
      throw new Error('this matrix includes print evidence and requires --capture-dir');
    }
    ...
    for (const path of [
      ...
      SOCIAL_CARD_SPECIMEN, ICON_SPECIMEN
    ]) {
    ...
    const preview = await treeDigest(PREVIEW);
    repositoryIdentity = {
      head: gitText(['rev-parse', 'HEAD']),
      base: gitText(['merge-base', 'HEAD', 'origin/main'])
    };
    inputHashes = {
      ...
      socialCardSpecimenSha256: await sha256(SOCIAL_CARD_SPECIMEN),
      iconSpecimenSha256: await sha256(ICON_SPECIMEN),
      documentCorpusSha256: await sha256(DOCUMENT_CORPUS),
      previewTreeSha256: preview.sha256,
      previewFileCount: preview.files
    };
  ...
  const profile = await mkdtemp(join(tmpdir(), 'triptych-didach-identity-chrome-'));
  ...
  try {
    ...
    serverAssertions.push(protectedWorktreeAssertion().row);
    ...
    cdp.on('Network.loadingFailed', (event) => {
      if (!channel.requests.has(event.requestId)) return;
