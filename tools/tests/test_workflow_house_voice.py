...
# version bump; the workflow was at 8 before it and 9 after.
HOUSE_VOICE_VERSION = 9
...
class VoiceRepairRoutingTests(RoutingCase):
    ...
    def test_the_tie_breaker_does_not_swallow_a_voice_finding(self):
        """Without this the "prefer research" rule sends every voice finding
        through a full research re-entry, which is the expensive wrong loop."""
        text = flat((FRAGMENTS / "propers" / "content-evaluation.md")
                    .read_text(encoding="utf-8"))
        self.assertIn("prefer `research`", text)
...
class PreservedTopologyTests(unittest.TestCase):
    ...
    def test_the_result_schema_needed_no_new_value(self):
        """Both repair targets already existed and are already routed."""
        schema = json.loads(
