...
- The Mass categories, ranks or classes, occurrence and concurrence problems, first Friday or first Saturday questions, and worked dates or cases to include.
- Desired tables, decision aids, examples, disputed interpretations, exclusions, and all free-form sources, guides, limits, staging, or commit instructions.
...
3. Supply a complete slot-by-slot assembly map that tells the reader when to use the Ordinary, temporal or sanctoral proper, common, votive or ritual formulary, preface, readings, and any permitted substitutions.
4. Treat first Friday, first Saturday, Requiem, Nuptial, ordination, patronal, titular, dedication, and other recurring or exceptional cases only to the extent established by the controlling rubrics and local facts.
5. Build thorny worked examples from fully stated dates and assumptions. Show the decision path and source locus at each branch; do not let examples silently become universal rules.
6. Update the edition manifest, rubric index, worked-case inventory, scope, source audit, generation metadata, README entry, and any affected links.
7. Build and inspect the reference and install only the reviewed PDF when publication is authorized.

## Verification

Re-run every worked case independently; check calendar facts, rank and precedence, occurrence versus concurrence, Mass permission, commemorations, slot assembly, source loci, local assumptions, terminology, research records, metadata, PDF pages, installed artifact, and catalog links.

## Summary

Report the decision problems and cases covered, controlling editions and calendar, records and artifacts changed, verification and authorized commit, and every case that still depends on local facts or specialist rubrical judgment.

## Next Steps

Name the one rubric collation, local calendar fact, or independent worked-case review most important next.
