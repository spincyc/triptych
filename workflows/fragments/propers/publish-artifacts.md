...
3. Record the two installed paths.

## Note

The publication gates at the end of this phase re-verify the installed
artifacts programmatically: since version 13 they compare each installed PDF
byte for byte against the build artifact `final-acceptance` accepted, and
they check the component manifest and the generation metadata against the
installed canonical PDF. That comparison is the gate's, not yours — do not
run it here and report that it passed. Your job is to install and report the
paths. If an install fails, report the failure in your summary rather than
working around it.

## Result

Return a worker result with `disposition: "PASS"`, `artifact_path` set to
`pdf/{provider}/{proper}.pdf`, and a summary naming both installed paths.
