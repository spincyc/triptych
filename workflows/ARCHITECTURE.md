...
`chronology-annotations-current` contract; `workflows/OPERATOR.md`
...
Since `proper` version 24 and `proper-finish` version 2 there are two more
checks, making eleven. `chronology-annotations-current` holds the generated
reader-facing projection byte for byte and requires each Date cell to carry
its matching generated annotation through the canonical pass-through wrapper.
...
Two more make thirteen, and both are prose screens rather than structural ones.
