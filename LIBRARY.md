# Library

Choose a section, then a title to open its PDF. Each publication has one catalog home.

| Section | Contents |
| --- | --- |
| **[Traditional Latin Mass (1962 Roman Rite)](library/traditional-latin-mass.md)** | Ordinary, assembly, calendar, and proper guides. |
| **[Novus Ordo (Postconciliar Roman Rite)](library/novus-ordo-liturgy.md)** | Edition-specific Order, calendar, and proper guides. |
| **[Prayer](library/prayer.md)** | Novenas and daily prayer books. |
| **[Faith](library/faith.md)** | Sacraments, virtues, and theological studies. |
| **[Biographies](library/biographies.md)** | Source-first historical and hagiographic lives. |
| **[Heresies](library/heresies.md)** | Doctrinal errors, ecclesial responses, and reception. |
| **[Historical Accounts](library/historical-accounts.md)** | Histories of texts, institutions, and events. |
| **[Mariology](library/mariology.md)** | Doctrine, prayer, apparitions, and reception. |
| **[Law and Church Discipline](library/law-and-church-discipline.md)** | Canon law and Church discipline. |

## Qualifications

Every PDF is a working study, not an approved Church publication. “Source-audited” means an internal evidence check, not independent specialist or ecclesiastical review; no current publication records such review.

The release manifest accounts for 60 publications: 58 are marked `release`, while *Saint Thomas Aquinas: Friar, Teacher, and the Work of Wisdom* and *Saint Robert Bellarmine: Controversy, Office, Pastoral Rule, and Reception* are on `hold` without approval and are excluded from public and private-preview artifacts pending a work-specific rights review. The later 20 July 2026 “approve all” supplement authorized the exact 58 PDF snapshots and 21 site inputs then recorded, subject to no project-initiated promotion. That approval is byte-specific: the current Ninth Sunday after Pentecost PDF and the current `CONTRIBUTING.md`, `LIBRARY.md`, and `library/biographies.md` bytes no longer match their bound hashes, so release checks fail closed pending renewed authorization. Exact-byte clearance is not a judgment that every work is editorially or profile-final, and it supplies no missing work-specific rights analysis, source collation, independent specialist review, imprimatur, nihil obstat, ecclesiastical approval, general license, or broader reuse permission. The La Salette study retains an unresolved question about the present force of the 1915 Holy Office decree discussed in its source audit.

Project licenses cover only project-created material. Quotations, public-domain works, and separately licensed material keep their own status. See [Licensing](LICENSE) and [Third-Party Material](THIRD_PARTY.md).
