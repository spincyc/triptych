...
| --- | --- | --- |
| E1 Catena implementation — current independent disposition | **Changes required** at exact V15 head `b9202882badbbbc364f1dd3d9057d2710ee47552`; **SEMANTIC CHANGES REQUIRED** and **EVIDENCE CHANGES REQUIRED** on `review/catena-wave-1-e1-corrections-v15-independent`. This current row supersedes the historical V14/V15-producer checkpoint retained below. | V15 fixes the ordinary A-held/B-same-path leak and preserves exact row/projection identity through the normal transport flow, but the path map briefly holds the initiating unresolved Promise before its raw result is frozen, the shared result is not a normalized immutable value, and `bodyAsked` proves row membership rather than completion-to-transport ownership. The exact ZIP seal passes, while executable-command, executed-driver, complete-history, numerical-claim, retained-privacy and final-inventory evidence do not. Continue only with the bounded V16 recorded by the independent review; preserve sound closures and all broader blockers. E1 remains off main; no merge, integration, re-signing, deployment or cutover is authorized. |
...
| E1 Catena implementation | **Changes required** at V14 head `69f2575421ba976271c936b1abd4b39dbe8b98fd`, recorded at review commit `0d11766ec232b2b4e46a7d1b0ada56ef22370004` on `review/catena-wave-1-e1-corrections-v14-independent`, so this lane can name the review it answers where V14 could not; V15 correction candidate `impl/catena-wave-1-e1-corrections-v15` awaiting fresh independent review | The V14 review passed and did not reopen the post-projection `unfetched` closure, the seven-member raw authority inventory, the tally as a consumer of its own, the raw post-projection audit, the structural member matrix, the hostile nested-source refusals, seventeen frozen structures at their exact null-prototype scope, the carried-path and spine-prefix closures, the fulfilled prewarm proof, the rights, provenance and refusal DOM cases, and the whole package protocol. It found one decisive semantic defect: request ownership was recorded at the address decision and discarded one line later, because the resolved path entered a module-scope map that held the unresolved PROMISE, so a second row carrying the same address joined the first row's request and rendered the answer that request was made for; the green oracle over that case required the leak. It also found body application absent from the actual-object roster, a page helper able to mint a second authority over one chapter, the promised nested EDITION accessor case missing, a descriptor count reported as one read when it is three per source key and two per stated field, deep immutability proved without a downstream rerender, and the evidence package materially defective — two browser commands recorded `ELIDED`, the load-bearing parent replay recorded `PROSE`, an executed-tool table contradicting its own assembly transcript, an invocation count presented as a tool count, and a history called complete and append-only while omitting a set-aside cohort and the P10 rows and disclosing a row replacement. V15 corrects only those: a path map holds settled answers alone and takes them from inside their own settle handler, work in flight is held against a frozen per-row owner object carrying the row and its projection, body application is asked as the row at the sink and is on the roster, the unreadable-spine substitute is made once per name, A is held while same-path B settles and renders its own distinguishable body against a pinned thirty-six-field terminal vector that A's late release cannot move, one owner's failure suppresses no other owner's request, a settled value may be shared and is still applied as the row that asked, the edition accessor is asked directly, observations are counted and reported by kind, and the mutated authority is re-consumed by a real rerender. Projection/orphan-source, source-only-fragment, translator, absence, selection, refusal-typing, unreadable-root, broader-terminal/oracle, CLI/web, model-budget, data-seam, release, common-gate, B0/shared-shell, real-device/AT, protected Liturgy and PDF blockers remain open or separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
