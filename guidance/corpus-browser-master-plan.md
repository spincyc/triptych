...
| E1 Catena implementation | **Changes required** at exact V9 head `3c5b78249193df065c4e1c2ee5a98e5989c6e582` (fresh independent review on `review/catena-wave-1-e1-corrections-v9-independent`, 2026-08-15) | V9 closes the request-layer composed fallback escape: absent, present-valid, and present-invalid prefixes remain distinct through projection; only genuine absence consults a valid own-stem carried path; and the exact V8 escape makes no cold, cached, or late request or substitution. The visible sink still ignores projected `text_refused` and renders present-invalid as the false genuine-absence sentence “carries no text file”; prewarmed, late, and present-valid regressions also omit material exact terminal assertions. Final ZIP bytes and manifest verify, but retained-parent/P8 identity, unique non-overwriting logs, immutable assembly, and required handoff/request fields remain incomplete. Correct only that V9 visible-state, proof, and package-protocol boundary from the exact head and return a new immutable package for fresh review. Preserve the request closure and all accepted earlier boundaries. Broader V7 production/oracle/architecture blockers, the historical data-seam contradiction, release bindings, common gate, B0/shared shell, real-device/AT evidence, protected Liturgy, and PDFs remain separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
