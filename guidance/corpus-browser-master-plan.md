...
| E1 Catena implementation | **Changes required** at V14 head `69f2575421ba976271c936b1abd4b39dbe8b98fd` (fresh independent review on `review/catena-wave-1-e1-corrections-v14-independent`, 2026-08-21) | V14 closes the post-projection `unfetched` reread, the seven-member raw-authority inventory, tally separation, structural member inventory, inherited V13 cases and the frozen terminal projection graph. It does not carry row/projection ownership through the pending path cache, completion and rendered body: same-path B remains loading and then consumes A's released body, while the test requires that leak. The actual-object roster omits body application and its wrappers can create the projection independently. The package ZIP/P8/authority/privacy identity passes, but mandatory exact commands, complete ledger history and executed-tool classification are materially contradictory; the missing V13 review identity and missing literal fresh-clone fields are bounded separately. Exact next action is the smallest semantic V15 from this exact head: owner-aware pending work or fulfilled-value-only sharing, a true A-held/B-settled/A-late oracle with application identity, bounded nested-source/deep-mutation proof completion, and rebuilt evidence with literal commands and one coherent complete tool/history ledger. Broader projection/orphan/source-only, translator, absence, ordering, refusal, unreadable-root, terminal/oracle, CLI/web, budget, data-seam, release, common-gate, shell, device/AT, Liturgy, PDF and integration blockers remain open. E1 stays off main; no merge, re-sign, deploy or cutover is authorized. |
