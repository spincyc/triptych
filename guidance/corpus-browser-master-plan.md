...
| --- | --- | --- |
| E1 Catena implementation — current independent disposition | **CHANGES REQUIRED** at exact V16 `cc1f2fb8625f044558c26edd358b99cd7dcc7646` on review branch `review/catena-wave-1-e1-corrections-v16-independent`: **SEMANTIC CHANGES REQUIRED** and **EVIDENCE CHANGES REQUIRED** separately. Evidence commit `bfa913466c559ad410f252f4192fb3af953dd10b` contains ZIP SHA-256 `bb473e7afbfa619182248a02fdc4db28b645792cba054e8be3cf660ff845d4fb`; main remains `2778285849f2973ea89d1cfd5b2751ed4ae58e54`, unintegrated. This current row supersedes the historical V15-review/V16-producer checkpoint retained below. | Preserve V16's accepted atomic finalized cache, exact completion-envelope, cross-owner rebinding/rejection and post-write journal work. One bounded V17 must close inherited/accessor chapter authority and thenable text substitution, add exact V16-parent discrimination, and rebuild evidence after an immutable ledger close with final coherence/privacy gates, truthful phase-labelled history, hardened command/authority scanners and representative fresh-clone replay. Do not merge, integrate, re-sign, deploy or enter another E1 lane. |
...
| E1 Catena implementation | **Changes required** at V15 head `b9202882badbbbc364f1dd3d9057d2710ee47552`, recorded at review commit `67247ecc39a6e5f6224c64ca3ab1af163ee023b1` on `review/catena-wave-1-e1-corrections-v15-independent`, which returned **SEMANTIC CHANGES REQUIRED** and **EVIDENCE CHANGES REQUIRED** separately and **CHANGES REQUIRED** overall; V16 correction candidate `impl/catena-wave-1-e1-corrections-v16` awaiting fresh independent review, with `origin/main` at `2778285849f2973ea89d1cfd5b2751ed4ae58e54` and V16 not integrated with it | The V15 review passed and did not reopen the row-transport owner model, the A-held/B-independent decisive behaviour and its thirty-six-field terminal vector, the wrapper-created-authority closure, one owner's failure suppressing no other owner's request, owner-local retry, the hostile nested `edition` and `edition_published` accessor cases, the thirteen throwing mutations and their downstream rerender, or any inherited V14 closure; V16 re-runs all of that as REGRESSIONS and counts none of it as new closure. It found the decisive semantic defect that publication of the path entry ran inside the fulfilment handler of the promise returned by `.then()`, so a still-pending object was briefly path-visible and publication preceded the freeze; that the shared value was raw shallow-frozen parsed JSON with a mutable prototype, read later by ordinary prototype-sensitive lookup; and that `bodyAsked(row, content)` proved only that the row had been projected, so an actual B row accepted arbitrary A content, with the body journal recorded before the DOM write. It also found the evidence package materially defective — seven of 24 `LITERAL` command rows that cannot expand, a parent replay overloading `$REPO`, a classifier that prefix-matches prose, a handoff checker that trusts its own label, six manufactured non-execution placeholders, build drivers classified unexecuted, two unlabelled tool-count sets for one lane, a stale `COMPLETE` verdict, degenerate compare-gate diagnostics, and a nine-attempt history told as five refusals and a sixth seal. It also found that the durable record claimed 30 example divergences where the shipped transcripts report 28, and **it was right about that**: `logs/attempt-01/make-check-parent.log` and `logs/attempt-02/make-check-head.log` each report 28 `DIFF` rows over 27 distinct commands and `28 diverged … 2 volatile line(s) declared`, and no artifact in the V15 package supports 30. **Its account of the difference does not survive measurement, and neither did V16's first attempt to write it down**, which rewrote the records to 28 + 2 before the experiment was run. That decomposition is arithmetically impossible: `volatile` is a static constant at `scripts/replay_examples.py:734` over the two-entry table at `:182-185`, counting declared LINES for two `tools/pdf-review` captures masked at `:405` that appear as `ok` and `absent` and never as `DIFF` rows, so they were never in the set they are supposed to be subtracted from. **The cause is neither arithmetic nor prose but build state**: measured three times independently at the exact parent in a clean checkout not under `/tmp`, a cold `build/` reports 30 and an immediately repeated warm run 28, the entire delta being two captures of `tools/mass-ordinary check --out build/example-ordinary`, whose comparison directory a later capture in the same target writes; in V15's shipped transcripts both read `ok`, the warm signature, so V15 quoted a cold figure while shipping a warm log and the record never said which tree it measured. 28 is also reachable as distinct DIFF command strings — warm 28 rows over 27, cold 30 over 28 — a row-versus-name conflation of the kind the same review rightly criticised in `compare-gate.py`. **What V16 takes from this is not a number but a rule: a count is meaningless without the state it was taken in, and this package states both.** The battery records `build-state=COLD|WARM` at preflight and the V16 check pins no constant, refusing instead the unsound shape, a figure the package's own transcript does not support, and a summary that disagrees with its own `DIFF` rows. V16 corrects only those: the payload is finalized once at settlement into an owner-independent, null-prototype, frozen, scalar-only record taken by own descriptor; the page publishes the final value and never a promise; a sealed per-caller completion envelope carries the exact row transport owner to the body sink and is required at application; the body is journalled only after a confirmed write; a finished cached value is rebound to a later owner through that owner's own completion; and the package is rebuilt with executable commands, mechanically derived tool execution, one complete attempt history, the example figure derived mechanically and reported in its two distinct senses (divergent ROWS and distinct COMMAND STRINGS) with the volatile count kept apart as the static declaration it is, the build state recorded beside every statement of the figure and no constant pinned by any check, granular diagnostics, a final-state completeness verdict and an explicit shipped-versus-local privacy boundary. Projection/orphan-source, source-only-fragment, translator, absence, selection, refusal-typing, unreadable-root, broader-terminal/oracle, CLI/web, model-budget, data-seam, release, common-gate, B0/shared-shell, real-device/AT, protected Liturgy and PDF blockers remain open or separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
