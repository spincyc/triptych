...
| E1 Catena implementation | **Changes required** at V13 head `6cc85e1a1dea317a48c0bfcfd6f774201ea3a6c3` (independent review recorded 2026-08-20 with **no published branch or commit**: `origin` carries no `review/catena-wave-1-e1-corrections-v13-independent`, and the branch exists only in a local reviewer checkout standing at the reviewed head with no review commit on it); V14 correction candidate `impl/catena-wave-1-e1-corrections-v14` awaiting fresh independent review | One-chapter-one-projection held in a `WeakMap`, `requestSnapshot` for one invocation, the inherited `text_prefix` and inherited `text_refused` closures, the carried `text_path` read once, the fail-closed contamination policy as a design, the six planted V13 scenarios, and the V13 package protocol — immutable sealed package, read-only P8 with equal pre and post digests, one-way post-P8 authority, a complete attempt ledger with unique ordinals, executed-tool byte evidence, privacy sanitization, the substantive handoff inventory and the outer scan — all pass and are not reopened. Six things did not close. Raw `file.unfetched` was still read after the projection existed, so a record answering nothing while readability was decided and a forged string afterwards replaced an accepted chapter with a manufactured unavailable state, removing its rows and its recorded refusal, altering the tally and printing the payload's own string to the reader. "Same instance" was argued from equal ids rather than proved with `===` at the consumer boundary. The tally was folded into the rows rather than recorded as a consumer of its own. Request ownership was reconstructed afterwards from path strings, so a same-path sibling was the same owner and a late completion belonged to whichever row a match found first. Nested `sources["1"]` as an own getter was invoked by the voices and editions walk and declined by every fragment row, producing one projection that stated two incompatible things about one edition. And the projection was frozen while its row, lead and blocked entries were not. V14 corrects only those six: the projection carries `unfetched` and the page reads it there, proved by a walking production case beside a steady control; a bounded observation seam records the actual object each of ten covered consumers receives and compares it with `===` against the reference taken where the projection is made; the tally asks its own question; the page resolves a request address through the projected row and the model records the owning row and projection at that moment, so same-path siblings, same-path multi-projection and genuinely-late completion each keep their own owner and a row no projection made addresses nothing; nested source entries and their edition, rights and voice members are normalized once by descriptor, so no consumer invokes an accessor and four hostile shapes give one coherent result; the member inventory is taken once, with add, remove, reorder, phantom and tally pinned independently beside steady controls; and seventeen trusted structures are asserted frozen with thirteen real mutation attempts, the null-prototype claim stated at its exact scope. Projection/orphan-source, source-only-fragment, translator, absence, selection, refusal-typing, unreadable-root, broader-terminal/oracle, CLI/web, model-budget, data-seam, release, common-gate, B0/shared-shell, real-device/AT, protected Liturgy and PDF blockers remain open or separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
