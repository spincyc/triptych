...
| E1 Catena implementation | **Changes required** at exact V8 head `7e4df42a21bc2be2d28ff14943f63af3e7e3a6f8` (independent review on `review/catena-wave-1-e1-corrections-v8-independent`, 2026-08-15) | V8 correctly rejects isolated wrong-namespace, padded, traversal, absolute, encoded, sibling and boundary-spoof paths and preserves every valid real and fixture path. It does not close the composed fallback: a present-but-invalid prefix collapses to the absent-prefix state, activates a valid same-stem carried path, reaches production `fetch`, and renders a planted body. The regression matrix omits that combination and complete terminal/cache proof, and the sealed package's machine inventory has five stale final-member rows. Make one bounded correction from the exact V8 head, preserve the absent/valid/invalid prefix distinction, prove zero cold/prewarmed/late fallback at the sink, rebuild a truthful immutable package, and obtain fresh independent review. Every other V7 blocker remains open. The authorized Catena data seam transfers no broader ownership; the Day-reader contradiction, release, common gate, B0/shared shell, real-device/AT evidence, protected Liturgy, and PDFs remain separate. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
