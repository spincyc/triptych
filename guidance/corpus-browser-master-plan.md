...
| E1 Catena implementation | **Changes required** at V12 head `d312786dd2b23926aa88e29ea15647dfcc7e7e6e` (fresh independent review on `review/catena-wave-1-e1-corrections-v12-independent`, 2026-08-17) | V12 correctly snapshots request-critical fields once per model projection, closes inherited `text_prefix` and inherited `text_refused`, prevents one projection from validating and using different carried paths, accepts the bounded page-wide contamination policy, and preserves ordinary and late behavior. It does not meet the required stable page snapshot: the same raw spine/fragments are projected three times for readability, tally, and render/request, so later raw answers can alter request/cache/body/ownership state. One of six planted parent cases is vacuous and the package ships no V12 ownership journals. Package authority remains unsafe before P7/P8; discarded/battery history, chronology, negative authority/endpoint gates, handoff inventory/counts, executed-tool identity, and privacy over package plus outer logs are incomplete or contradictory. Exact next action: one bounded V13 from exact V12 head retaining one chapter projection across all request-critical consumers, with walking-state/non-vacuity regressions and a fresh complete, privacy-clean package whose sole publication authority is bound after P8 to the ZIP identity and result. Projection/orphan-source, source-only-fragment, translator, absence, selection, refusal-typing, unreadable-root, broader-terminal/oracle, CLI/web, model-budget, data-seam, release, common-gate, B0/shared-shell, real-device/AT, protected Liturgy, PDF, and cumulative-history-reconciliation blockers remain open or separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
