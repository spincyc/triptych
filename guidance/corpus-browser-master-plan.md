...
| E1 Catena implementation | **Changes required** at V12 head `d312786dd2b23926aa88e29ea15647dfcc7e7e6e` (independent review `728c3e3b3d0d6e899f0da33e06a08a116375896f` on `review/catena-wave-1-e1-corrections-v12-independent`, 2026-08-17); V13 correction candidate `impl/catena-wave-1-e1-corrections-v13` awaiting fresh independent review | `requestSnapshot` is accepted as correct for one invocation, and the inherited `text_prefix` closure, the inherited `text_refused` closure, the carried `text_path` read-once behaviour within a projection, the fail-closed prototype-contamination policy, ordinary request behaviour, the malformed and unestablished wording, the cold, present-valid and prewarmed controls, primitive namespace closure, the 36-field late vector and its `0 -> 1` release, the read-only non-executing P8 with its pre and post rehash, ZIP arithmetic, CRC, manifest and central-directory correctness, candidate immutability and ownership boundaries all pass and are not reopened. The stable snapshot was nevertheless not stable for a page RENDER: the same raw spine and fragments were projected three times — by `spineUnreadable`, by the tally and by `renderChain` — so one descriptor read per projection times three proved no source revisit, and a record answering differently between projections rendered, requested, cached and attributed from an answer nothing had approved. Beside it one of six planted scenarios was vacuous, the packaged ownership journals stopped at the previous lane, the sealed bytes claimed final authority before P8, the authority gate accepted six contradictions, four discarded attempts and four set-aside batteries were absent from every surviving ledger while ordinals were reused, the handoff inventory was lexical and structurally blind to its own output, machine-specific paths leaked in twelve tracked outer-log lines and in two in-archive residues, and contemporaneous executed-byte proof existed for four tools of fifteen. V13 corrects only those findings: each raw chapter is normalized exactly once into one stable page-level projection that readability, tally, rendering, request, cache, body and ownership all read and none reaches past; walking raw-state tests assert one observation per loaded record and prove one projection identity; all six planted scenarios are non-vacuous with their ownership journals shipped; and the package carries one complete append-only ledger with no ordinal reuse, truthful reasons and times, publication authority established only after P8 and bound to the final ZIP, full negative authority and endpoint fixtures, executed and shipped digests for every tool claimed executed, a substantively verified inventory, and privacy scanning of every member and every outer artifact. Projection/orphan-source, source-only-fragment, translator, absence, selection, refusal-typing, unreadable-root, broader-terminal/oracle, CLI/web, model-budget, data-seam, release, common-gate, B0/shared-shell, real-device/AT, protected Liturgy and PDF blockers remain open or separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
