...
| E1 Catena implementation | **Changes required** at V7 head `e876b29e5797edcc6e86422daa807f4b1104ec81` (fresh independent review on `review/catena-wave-1-e1-corrections-v7-independent`, 2026-08-15) | V7 removes the arbitrary top-level fragment spread and preserves the valid language/voice/refusal-umbrella/verse/null boundaries, contradiction neutrality, formal per-file ceilings, and untouched data/release/shared owners. It is not yet closed-world: raw orphan sources create voices and absences, a same-stem non-Catena `text_path` reaches the request sink, a source-only record counts, malformed absence/partial/refusal members make or select claims, unreadable roots and locally suppressible unreadable-bible prose manufacture domain negatives, terminal proof omits required sinks, and the sealed package has internally stale member/count/provenance records. The always-loaded uncapped model also leaves combined route JavaScript enforcement unresolved. Correct only from exact V7 head, then issue a rebuilt immutable package and fresh independent review. The historical Day-reader guard, four release bindings, common gate, B0/shared shell, real-device/AT evidence, protected Liturgy, and PDFs remain separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
