...
| E1 Catena implementation | **Changes required** at V3 head `f2c9bc49` (independent review, 2026-08-12) | V3 remains bounded and preserves accepted V2 behavior, but its held-language union is not a voice-support authority, so `translation:grc` is falsely accepted despite zero Greek translations; its typed-presentation boundary still has Catena-owned coercion, filter-identity, language-metadata, extent, and terminal-cleanup holes; and the package exposes private session identifiers and overstates several comparisons. E1 stays off main. Only one smallest V4 correction from the reviewed head, explicitly authorized to add an exact Catena voice-key projection and to close the route/package findings, may follow; every other generator/data, release, common-gate, B0/shared-shell, pre-release real-device-or-AT, protected Liturgy, and PDF prerequisite retains its owner. |
