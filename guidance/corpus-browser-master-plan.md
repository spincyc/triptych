...
| E1 Catena implementation | **Changes required** at V6 head `4639b139f2179b1fca7f9cb1e4ba3ac19c9bbc46` (fresh independent review, 2026-08-14) | V6 passes language normalization, guessed-English removal, exact voice support, the neutral refusal umbrella, canonical positive verse deduplication, null termination, final control-byte hygiene, formal budgets and the route ownership boundary. It does not yet validate every semantic member before use: raw `text_path` can survive projection to the request sink, hollow members can create rows and claims, contradictory findings can still render one record's rights reason, unreadable roots can manufacture negative corpus claims, and terminal-state oracles omit required sinks. The immutable package is mechanically integral but contains incorrect exact-head, count, diff, screenshot and provenance claims, so it is not closing evidence. The next work is one bounded typed fragment/absence/refusal projection and its actual-request/row/tally regressions from the exact reviewed head. The historical Day-reader guard, release bindings, common gate, B0/shared shell, real-device/AT evidence, protected Liturgy and PDFs remain separately owned. E1 stays off main; no merge, re-sign, deploy or cutover is authorized. |
