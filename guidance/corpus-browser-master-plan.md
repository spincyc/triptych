...
| E1 Catena implementation | **CHANGES REQUIRED** at V5 head `19982ab433dd25704ed60b1ac6ddb678bc3a98f9` (fresh independent review, 2026-08-14) | V5 preserves the exact voice keys, neutral refusal umbrella, unchanged Catena data/CSS/HTML, formal gzip ceilings and accepted real-corpus visual baseline. It does not finish the typed boundary: object-shaped but semantically invalid collection members still create blank rows and counts or mask valid siblings; raw Bible/language/source/path/testament metadata still reaches visible or routed sinks; absence identity is order-dependent; JSON-null bootstrap and pending renders can escape terminal completion; and the route/numeric/verse oracles do not prove the exact sinks and state they claim. The sealed package is byte-integral and private in this environment, but its checkout provenance, parent replay, screenshot labels, focus claim and reusable sealer proof need truthful correction. The one next action is a bounded V6 from `19982ab4` that completes semantic root/member normalization and terminal invalidation, replaces the lossy runtime oracles, and issues one corrected immutable package for fresh review. The Catena data seam is not a broader ownership transfer; release, gate, shell, device/AT, protected Liturgy and PDFs remain separately owned. E1 stays off main; no merge, re-sign, deploy or cutover is authorized. |
