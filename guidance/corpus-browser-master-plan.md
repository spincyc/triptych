...
| E1 Catena implementation | **Changes required** at V11 head `0255b84996e1dc24da3ce75ac318c4f774b7957c` (independent review `22b9bdad5e71920a103e3ec3bcf2f79bba50cebb` on `review/catena-wave-1-e1-corrections-v11-independent`, 2026-08-17); V12 correction candidate `impl/catena-wave-1-e1-corrections-v12` awaiting fresh independent review | The fail-closed prototype-pollution policy is accepted as a design, and ordinary accessor non-invocation, own-property projection, the refused and unestablished wording, the cold, present-valid and prewarmed controls, the 36-field late vector, packaged request ownership, the read-only non-executing P8, the handoff contents, screenshots, privacy, ZIP metadata, the byte scan, ownership boundaries and the existing CSS/JS ceilings pass and are not reopened. The policy was applied incompletely: an inherited spine prefix was read as absence and reopened carried fallback, an inherited `text_refused` did not close an own-valid claim, and a twice-read carried-path descriptor could validate one path and project another. The package separately failed authority coherence, calling a superseded attempt authoritative and the shipped attempt unresolved while reusing log names across attempts. V12 corrects only those four findings: request-critical state is snapshotted once into trusted own data, contamination fails closed, the carried path is read once, planted request/cache/body/ownership regressions prove each at the production sinks, and the package carries one authoritative attempt with unique attempt-root logs and a mechanical coherence check. Projection/orphan-source, source-only-fragment, translator, absence, selection, refusal-typing, unreadable-root, broader-terminal/oracle, CLI/web, model-budget, data-seam, release, common-gate, B0/shared-shell, real-device/AT, protected Liturgy and PDF blockers remain open or separately owned. E1 stays off main; no merge, re-sign, deploy, or cutover is authorized. |
