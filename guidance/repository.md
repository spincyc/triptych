...
The launcher never commits a preserved result and never integrates one merely because Codex exits. After review and separate authorization, `make integrate <run-id>` provides the one-step landing path from the primary checkout; `scripts/triptych-codex --triptych-integrate <run-id>` is its direct equivalent. Using either form requires authority both to integrate the result and to update its recorded local target, including local `main` when `main` was the dispatch branch. It requires that clean named target, an inactive and intact locked worker on its expected branch, a clean worker index and worktree whose `HEAD` exactly matches the last terminal launcher audit, and histories that still descend from the recorded base. If that exact worker commit is not already reachable from the target, the command requires a fast-forward, passes Git’s no-overwrite-ignore guard so that the fast-forward checkout itself does not replace ignored primary files, and advances the target to the audited commit; it never creates a merge commit or enters conflict resolution. It verifies reachability before safely removing the worktree and worker branch. A dirty or mismatched checkout, an uncommitted worker or one whose audited `HEAD` or non-ignored status subsequently changed, an unavailable recorded base, a history that no longer descends from that base, or a diverged target is refused without cleanup. If the target advances but subsequent verification or cleanup cannot finish, the landed commit is not rolled back; the command reports the incomplete lifecycle state for inspection or a safe cleanup retry and does not claim complete success. The launcher itself never switches branches or invokes `git push` or deployment. When a fast-forward is performed, locally configured merge hooks still run; the post-merge audit can prevent cleanup when they alter checked-out state but cannot undo their external side effects.
