...
| The browser structure reading a citation under the **calendar's numbering** while `show` read it under the proper's | Both verbs produced output. `show` followed `numbered_proper_entries` — a cycle's `psalm_numbering`, else the proper's, else the calendar's — while `calendar_structure` handed every citation the calendar's. The postconciliar file declares Hebrew, so the twelve antiphons declaring `psalm_numbering: vulgate` reached the browser as eleven `unresolved` citations blaming the wrong psalm (`hebrew Psalm 118:49-50 is outside the psalm`) and one real, wrong psalm: ot-25's `Psalm 118:4-5` as Hebrew 118. The terminal printed the right verses, so a maintainer checking there saw nothing wrong. | Now caught: the structure pass reads each citation under its owner's numbering by the same rule, and `test_structure_reads_each_citation_under_its_own_numbering` holds it to `numbered_proper_entries` citation for citation, across a mass's own propers, mass- and proper-level `takes_from`, `cycles` and `weekday_cycles`. Corrected 2026-09-25. |
| A psalm range **trimmed at a numbering-table bound** | `_psalms.convert_range` converts only the verses the concordance numbers and drops the rest without a word. The concordance was compiled from a Douay printing that merges Vulgate 28:10-11 and 150:5-6, so it ends those psalms a verse early. Christ the King's `Psalm 28:10-11`, declared Vulgate, came back as Hebrew 29:10 alone, and the Hebrew-declared responsorial `Psalm 150:5-6` (two slots) came back one verse short in the Vulgate: every served verse real, the missing one unreported. The Hebrew-declared `Psalm 56:13-14` came back short the same way for another reason: it is no merge. Every tracked Vulgate witness prints Psalm 55 with thirteen verses and the two systems divide the psalm's body differently, so Hebrew 56:14 has no Vulgate verse of its own, and splitting the Douay's merged verses would not supply one. The psalm-exception ledger names these very loci, but it governs the validator, not the conversion. | Caught in the browser structure: `dropped_verses` in `tools/mass-propers` serves such a citation `unresolved`, naming the bound, and `test_served_structure_converts_every_cited_verse` reads both sides of every served psalm citation in the three named structure files back into Vulgate verses, filling an open end from the psalm's bound, and fails where the two sides are not one set. **Open elsewhere**: `convert_range` is unchanged and still trims for `index-bible`, whose indexed Clementine Vulgate, Douay-Rheims American 1899 and CPDV — three of the four Vulgate-numbered indexes — answer `Psalm 150:1-2, 3-4, 5-6` without verse 6, and for `typeset-bible` and `reading-plan`, whose one plan crosses no bound today. |
| A psalm **cited whole, moved by its chapter number** | `convert_range` converts a range with an open end by its chapter number alone and returns the verses it stands for as a caveat, which `spans_in` discarded. Vulgate Psalm 147 is Hebrew 147:12-20, so the 1962 palm-sunday Procession Antiphon's `Psalm 147`, declared Vulgate, reached the browser as Hebrew 147 whole, and a Hebrew-numbered reader was given the eleven verses of Vulgate 146 before the nine cited. Both sides named a real psalm, and the check for trimmed ranges skipped every locus with an open end. | Now caught: `spans_in` closes an open end at its psalm's concordance bound before converting and serves a converted piece that is a whole psalm open at both ends, as the citation was (so Hebrew 147 whole reaches the Vulgate side as 146 and 147 whole, and Vulgate 150 the Hebrew side as 150 whole, keeping the sixth verse the concordance's Douay merges). The palm-sunday antiphon's Vulgate side stays 147 whole because a citation is never converted into its own numbering, which is what the Clementine, 1899 Douay and CPDV need, since they print Vulgate 147 as verses 1-9; its Hebrew side is 147:12-20. `test_structure_serves_a_whole_psalm_as_the_verses_it_is` holds the rule; the check above now reads open ends. The eight whole psalms that are part of another — Vulgate 114, 115, 146, 147 and Hebrew 9, 10, 114, 115 — convert to their verses, and the four that divide — Vulgate 9 and 113, Hebrew 116 and 147 — to both halves where they were refused; palm-sunday's is the only tracked citation among them. Corrected 2026-09-25. **Open elsewhere**: `index-bible` still files the Vulgate `Psalm 147` as all of Hebrew 147 in the World English, King James and Revised Version indexes, so `mass-propers show` on those bibles prints it from Hebrew 147:1 with only the caveat to warn. **Limit**: a half-open citation that crosses numberings (e.g. Vulgate 28:3-) is closed at the concordance's last verse on the converted side, so in the psalms the concordance merges (28/29, 150) it would lose the verse the editions print; no tracked citation has that shape, and one would need the end reopened rather than closed. |
