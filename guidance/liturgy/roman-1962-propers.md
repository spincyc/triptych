...
- in weekly and ritual Mass guides, `Generation Metadata` is the terminal section and normally uses three compact labeled lines that unambiguously give the final generation date in `YYYY-MM-DD` form, the full verbatim AI model label, every exposed model qualifier, the AI agent and its role, every exposed and accurately labeled agent/client/build version, and the product/interface-level AI environment;
