...
- **Generation:** `G0` not generated; `G1` legacy AI-assisted generation whose exact model, agent, and runtime were not recorded; `G2` generated with fully qualified provenance; `G3` substantially revised with fully qualified final-event provenance. The present `G2` and `G3` cohort records `gpt-5.6-sol`, `effort=ultra`, OpenAI Codex agents, Codex CLI `0.144.1`, and an API workspace; the document-local structured record identifies roles and any specifically unavailable component.
...
The intended rhythm is **survey, historical situation, source-grounded synthesis, source-grounded exposition, exploratory proposals, verification**: survey the Propers map and four senses, place the directly quoted Scriptures in their authors' lives or recoverable compositional horizons, first audiences, geography, and salvation-historical moment, read the two-page synthesis, enter the detailed sourced exposition, consider the separately labeled exploratory proposals, and verify the witnesses in the final references.

## Required Weekly Document Order

Every new or substantially revised guide follows one fixed macro-order. The content and internal teaching forms remain adaptive to the Sunday, but the section sequence does not. Immediately after page 1, use the exact major headings `In Illo Tempore...`, `Source-Grounded Synthesis`, `Source-Grounded Exposition`, `Exploratory Proposals`, `References`, and `Generation Metadata`; insert `Notable Quotables` after the exploratory proposals when verified examples exist, followed by any required sacramental appendix and then `References`.

### Sacramental appendix rule for ritual Masses

When a Mass is celebrated with or specifically for Baptism, Confirmation, Penance, Anointing of the Sick, Holy Orders, or Matrimony, its guide includes the relevant one-page sacrament summary as a clearly labeled `Sacramental Appendix`. Import the canonical fragment from `src/gpt/theology/sacraments/summaries/` after `Exploratory Proposals` and any `Notable Quotables`, but before `References`; `Generation Metadata` remains terminal. If more than one non-Eucharistic sacrament is actually conferred, include each relevant summary. The Eucharist needs no routine appendix because every Mass guide already treats the Eucharistic sacrifice and reception as the liturgy's intrinsic center; a Mass specifically focused upon Eucharistic doctrine may include its summary when that adds real value.
...
### Next two pages: Source-Grounded Synthesis
...
### Thereafter: Source-Grounded Exposition
...
After the source-grounded exposition, place every original AI-generated analogy, cross-proper proposal, symbolic extension, or theological inference not established by the sources in a clearly titled `Exploratory Proposals` section. One compact opening notice establishes that the section contains exploratory AI-generated syntheses, attributes none of them to cited authorities, and makes no claim about historical compositional intent. Once that boundary is set, write in a natural exploratory voice rather than repeating formulaic hedges.
...
The first four classes may appear in the source-grounded sections. The fifth appears only in `Exploratory Proposals`. A canonical or liturgical analogy supported by cited tradition may be source-grounded; an AI-generated analogy without that support is exploratory. An unverified study lead supports no final claim. Never place an AI-generated connection in a Father's mouth, imply consensus by proximity, or hide an exploratory proposal in the four-senses table.
...
Assemble the guide according to `Required Weekly Document Order`. Fit the complete `In Illo Tempore...` table to page 2 and force a boundary after it, then force another after the second synthesis page so that `Source-Grounded Exposition` begins on a fresh page. Edit to make the historical table and both synthesis pages genuinely full, readable, and scan-first. The synthesis requires one compact orienting form; its internal headings, tables, matrices, source cards, short lists, and diagrams may otherwise vary with the Sunday, but the macro-order may not.
...
| Listening or examination card | Converting sourced findings into attention; any new AI application belongs in `Exploratory Proposals` |
...
- **Editorial or AI proposal:** the project's own relationship among texts; it appears only in `Exploratory Proposals`, even when plausible.
...
- the two pages immediately following `In Illo Tempore...` contain exactly two full, readable pages of `Source-Grounded Synthesis` and no exploratory AI proposals;
- `Source-Grounded Synthesis` begins with a one- or two-sentence thesis and a compact orienting form of no more than four primary stages, then uses three to five relationship-titled units keyed to the same argumentative spine;
...
- a signpost-only scan of `Source-Grounded Synthesis` recovers its central claim, direction of movement, decisive witnesses, and interpretive limits, while the full text remains a continuous argument rather than disconnected fragments;
- `Source-Grounded Exposition` begins on the fresh page after the two-page synthesis, gives the evidence and qualifications behind it, and does not duplicate either the historical orientation or the synthesis paragraph for paragraph;
- every original AI analogy, typological extension, compositional inference, or unsourced cross-proper connection appears only in `Exploratory Proposals`;
- `Exploratory Proposals` begins with one global boundary notice, then gives proposals room to develop without repetitive hedging; substantial proposals expose their proper anchors, while material limits or serious alternatives appear where needed rather than as a repeated formula; the section remains free of invented facts, quotations, attributions, or doctrinal contradiction;
...
- `Notable Quotables`, when included, appears after `Exploratory Proposals` and before any required sacramental appendix and `References`, normally contains two to four verified examples from varied works or media, and never adds a weak item merely to reach that range;
...
- uncertain cultural parallels remain in `Exploratory Proposals`, copyrighted excerpts remain brief, and an empty `Notable Quotables` section is not retained when no verified example exists; unsuccessful leads remain in `research/scope.md` rather than padding the PDF;
