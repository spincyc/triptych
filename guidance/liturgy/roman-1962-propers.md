...
The three lines are rendered by importing the guide leaf's `generation-metadata.tex`, not handwritten in `main.tex`. The record uses `\AIGenerationDate` plus one or more `\AIModelContribution` commands; separate contribution commands are required only when materially contributing model, qualifier, or runtime values differ.
...
- in weekly and ritual Mass guides, `Generation Metadata` is the terminal section, imports the leaf's structured record, and normally renders three compact labeled lines that unambiguously give the final generation date in `YYYY-MM-DD` form, the full verbatim AI model label, every exposed model qualifier, the AI agent and its role, every exposed and accurately labeled agent/client/build version, and the product/interface-level AI environment;
...
Do not preserve weak material merely to maintain length, and do not pad the two-page synthesis. Within the fixed macro-order, allow the appointed formulary's actual texts and witnesses to determine the internal architecture. Prefer fewer strong source-grounded axes, then give clearly bounded exploration freedom to test unfamiliar patterns, competing models, and questions. Refresh `Generation Metadata` for the final generation event, build and inspect the PDF, run `make clean`, and commit each proper-guide creation or substantial revision together with its stored `propers/` source record as one coherent change in this repository.
