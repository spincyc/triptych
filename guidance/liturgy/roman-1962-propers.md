...
The full sacramental treatise ([research scope](../../src/gpt/theology/sacraments/research/scope.md); [PDF](../../doc/gpt/theology/sacraments.pdf)) opens with a title page carrying its AI-generation metadata, a table of contents, and its metaphysical vocabulary. Its continuous main treatment follows each sacrament through Scripture, patristic reception, and Thomistic synthesis, distinguishes sacramental from substantial form and states what each sign changes, separates outward operation, intermediate sacramental reality, proper grace, primary and secondary ends, and the common ultimate order to God's glory and beatitude. After the references, terminal appendices collect the seven-sacrament matter–form–minister matrix, the initiation-practice table for all twenty-four Catholic Churches *sui iuris*, and one reusable one-page summary for every sacrament. Its summary fragments live in `src/gpt/theology/sacraments/summaries/`; ritual-Mass guides import those fragments rather than keeping independent condensed accounts.
...
The intended rhythm is **survey, historical situation, themes and movement, detailed commentary, interpretive possibilities, verification**: survey the Propers map and four senses, place the directly quoted Scriptures in their authors' lives or recoverable compositional horizons, first audiences, geography, and salvation-historical moment, read the two-page source-grounded synthesis of themes and movement, enter the detailed sourced commentary, consider the separately bounded interpretive possibilities, and verify the witnesses in the final references.

## Required Weekly Document Order

Every new or substantially revised guide follows one fixed macro-order. The content and internal teaching forms remain adaptive to the Sunday, but the section sequence does not. Immediately after page 1, use the exact major headings `In Illo Tempore...`, `The Propers: Themes and Movement`, `The Propers: Detailed Commentary`, `The Propers: Interpretive Possibilities`, `References`, and `Generation Metadata`; insert `Notable Quotables` after the interpretive possibilities when verified examples exist, followed by any required sacramental appendix and then `References`.
...
`In Illo Tempore...` occupies exactly one page immediately after page 1 and ends with a forced page break. It is a source-grounded historical orientation to the biblical words appointed for the day, not an unsupported reconstruction.
...
### Next two pages: The Propers: Themes and Movement
...
Assemble the guide according to `Required Weekly Document Order`. Fit the complete `In Illo Tempore...` table to page 2 and force a boundary after it, then force another after the second synthesis page so that `The Propers: Detailed Commentary` begins on a fresh page. Edit to make the historical table and both synthesis pages genuinely full, readable, and scan-first. The synthesis requires one compact orienting form; its internal headings, tables, matrices, source cards, short lists, and diagrams may otherwise vary with the Sunday, but the macro-order may not.
...
- in weekly and ritual Mass guides, `References` is the penultimate section, contains every source actually cited, contains no unused study leads, and ends with a `Search Scope and Limitations` subsection whose compact panel is untitled rather than redundantly labeled; `propers/verified.md` and `research/scope.md` retain the full text and research audits;
