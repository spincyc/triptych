...
That contract owns schema 2's presentation and pagination, including the
restored first four pages of its 10–12-page concise study; the source,
identity, language, and rights rules
...
Use a two-tier dossier table with top-aligned ragged-right columns. The first row of each dossier is a concise four-field summary: inherited or conventional orientation belongs in the short `Location` and `Date` cells, not multi-sentence qualification. Follow it with `\cmidrule(lr){1-4}` and a full-width ragged-right explanatory row that gives historical judgment, audience, life stage, uncertainty, and claim-local sources. Use `\midrule` only between complete dossiers. When a passage narrates an event, place a separately labeled italic event row between two `\cmidrule(lr){1-4}` rules before the full-width explanation. This summary-row-plus-spanning-dossier hierarchy is the standard.

Type size, column widths, row spacing and the width of the full-width rows belong to the `dossiertable` and `concisedossiertable` environments, with their `\dossierprose` and `\dossierevent` rows, in `src/common/propers-format.tex`. A leaf under `format_contract = "propers-format-v1"` imports those environments and may not redefine them. A leaf authored before that contract sets its own table in its own sources, usually its `format.tex`, and records there any departure it makes, as Claude 54's `format.tex` does in its departures D1–D3. Change measurements only when the complete inventory cannot remain legible after the hierarchy and evidence have been preserved.
...
**Every biblical date on this page comes from the Scripture chronology corpus.**
[Scripture chronology](../scripture-chronology.md) sections 5, 8 and 14 own
its relations, sources and publication contract. Generate both
`research/chronology.toml` and `research/chronology-annotations.tex` with
`tools/tpt proper-chronology record` and `annotations`, using `--write` and the
publication's provider and document ID. Each appointed Scripture has exactly
one `\chronodate{element-key}{\chronologyannotation{element-key}}` cell. Import
the generated definitions once; never hand-edit or replace their date labels.
The projection retains the exact source label, subject, relation, evidence
profile, disposition and locus reach behind each concise displayed date.

Lead with the passage's traditional frame and identified setting or event,
then its textual history. Keep the relation labels visible, and give the Date
column enough width for them without reducing legibility. A Davidic attribution
may carry David's sourced regnal era as an explicitly named reference point;
it does not place every attributed poem within that reign. An identified
pre-accession episode remains earlier. A composition bound such as “before
c. 165 B.C.” remains beside it under its own label. Do not assign a Davidic
era to anonymous or differently attributed psalms. Narrated and retrospective
events in Genesis, Exodus and other Old Testament passages receive the same
distinction from writing and final formation that Gospel episodes do.
Prophetic referents remain separately labeled. Preserve the corpus's
...
- the interpretive section contains four to six non-recapitulatory, multi-element proposals, each stating its connecting mechanism, its fruit, what the ordinary element-by-element reading misses, and its strongest material limit, and the research scope records each proposal's anchors, mechanism, nearest located precedent or analogue, search boundary, and controlling limit without claiming universal novelty;
...
- the thematic thesis opens directly as prose, and an audit of headings, box titles, run-in labels, and table headings in both editions finds no rhetorical or editorial wrapper such as `Governing thesis`, `Thesis`, `Key takeaway`, `Argument map`, or `Reading order`; one occurrence fails this categorical structural gate independently of whether process prose recurs;
