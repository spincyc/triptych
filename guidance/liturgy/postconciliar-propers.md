...
6. the unbounded `The Propers: Detailed Commentary`;
7. optional `The Propers: Interpretive Possibilities`, with the required editorial or AI disclosure;
...
The title block, complete textual-variable-parts inventory, and four-senses table occupy page 1 for an ordinary Sunday or solemnity. The title block identifies the exact edition-locale, formula or governing identity, and occurrence when resolved. Do not place an edition-verification, identity, use, rights, or review-status callout between the title and the inventory; retain the necessary use, rights, verification, and review disclosures in `Search Scope and Limitations`, the standard rights notice, the catalog, and the tracked records. The inventory follows the title without a visible section heading, and the four-senses table follows the inventory without its own title or a second explanatory essay. Their column headers supply the needed local identification. The four-senses table contains exactly the visible rows `Literal`, `Allegorical`, `Moral`, and `Anagogical`. Each row is grounded in the appointed texts or documented reception and preserves branch limits; it may synthesize composed liturgical texts only by labeling the relation accurately rather than pretending that an oration is inspired Scripture.

`In Illo Tempore...` begins on the next fresh page. For an ordinary Sunday or solemnity it occupies exactly one page and ends at a forced boundary. A textually extensive target---including Palm Sunday, the Easter Vigil, the extended Pentecost Vigil, or a complex Ritual Mass---expands its complete historical inventory rather than omit passages, compress alternatives, or use unreadable type. `The Propers: Themes and Movement` follows immediately and always occupies exactly two substantive, readable pages. Its second page begins directly with its content or a descriptive subsection heading; do not repeat the section heading or add `(continued)`. `The Propers: Detailed Commentary` then begins on a fresh page and has no upper page limit. The detailed commentary, optional later sections, and technical appendix expand according to the celebration's evidence and branch structure rather than a total-document page target.

All material beginning with `In Illo Tempore...` uses the shared `deepstudy` size or an exact readable equivalent; page 1 is not globally reduced merely to force the required survey into place. Visual distinctions remain intelligible in pure black and white, with no color-dependent distinction or gray fill. If an ordinary formula's complete, readable page-one survey or historical table does not fit at the prescribed size, edit repetition and presentation first; do not omit an appointed unit, erase an alternative, or hide a material limitation to satisfy the boundary.
...
The reader-facing order has four deliberate movements: a page-one survey, historical situation, two-page synthesis, and detailed exposition, followed by the technical resolution and verification material. The opening inventory answers which texts the celebration appoints or concretely selects, while the four senses disclose the principal interpretive horizons without replacing exegesis. `In Illo Tempore...` places the appointed scriptural words in their compositional and narrated settings. `Themes and Movement` compresses the guide's source-grounded architecture into two scan-first pages. `Detailed Commentary` supplies the unbounded evidence and qualifications. The appendix identifies the celebration and records the authority and resolution of branches that a reader needs to understand the text inventory or exposition. Complete source, status, and unresolved-choice data remain in the tracked audits.
...
- `In Illo Tempore...` covers every distinct directly appointed scriptural passage and marked adaptation in canonical order, distinguishes composition from narrated event, occupies exactly one page for an ordinary Sunday or solemnity, and expands rather than omits material when the target is complex;
...
- `The Propers: Themes and Movement` occupies exactly the next two substantive, readable pages, joins liturgical movement to theological and spiritual synthesis, makes its decisive witnesses and limits recoverable in a scan without repetition or padding, and does not repeat its section heading or add `(continued)` on the second page;
...
- `In Illo Tempore...`, `The Propers: Themes and Movement`, `The Propers: Detailed Commentary`, and `Appendix: Liturgical Resolution` begin or end at their prescribed page boundaries without compressed type, artificial whitespace, or omitted material;
