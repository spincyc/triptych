...
3. the compact page-one `The Four Senses of Scripture` table;
4. `In Illo Tempore...`;
...
The title, compact note, complete `Textual Variable Parts`, and four-senses table occupy page 1 for an ordinary Sunday or solemnity. Taken together, the title and note identify the exact edition-locale, formula or governing identity, occurrence when resolved, use boundary, rights boundary, and material review limits. The four-senses table follows the textual inventory without a second explanatory essay and contains exactly the visible rows `Literal`, `Allegorical`, `Moral`, and `Anagogical`. Each row is grounded in the appointed texts or documented reception and preserves branch limits; it may synthesize composed liturgical texts only by labeling the relation accurately rather than pretending that an oration is inspired Scripture.

`In Illo Tempore...` begins on the next fresh page. For an ordinary Sunday or solemnity it occupies exactly one page and ends at a forced boundary. A textually extensive target---including Palm Sunday, the Easter Vigil, the extended Pentecost Vigil, or a complex Ritual Mass---expands its complete historical inventory rather than omit passages, compress alternatives, or use unreadable type. `The Propers: Themes and Movement` follows immediately and always occupies exactly two substantive, readable pages. Its second page may carry a quiet continuation label, but it remains one section. `The Propers: Detailed Commentary` then begins on a fresh page and has no upper page limit. The detailed commentary, optional later sections, and technical appendix expand according to the celebration's evidence and branch structure rather than a total-document page target.
...
The table gives citations and historical orientation, not copyrighted biblical text. It begins on a fresh page. For an ordinary Sunday or solemnity it fits one complete page and ends at a forced boundary; the heading, one-sentence interpretive key, and table are the only material on that page. The Easter Vigil and other scripturally extensive targets expand as needed rather than omit readings, compress alternatives, or make the table unreadable. Full historical disputes and source-role details remain in `research/scope.md`.
...
7. **Draft from depth to compression.** Write `The Propers: Detailed Commentary` first; then derive the two-page `Themes and Movement`, the four-senses table, and the compact scope panel. Populate `Textual Variable Parts` only from the verified text audit; it contains no synthesized claim. Consolidate each unit's closed alternatives without erasing their distinctions, and place source, status, and resolution mechanics in the appendix and tracked records. Edit the synthesis to its exact two-page boundary without padding, repetition, compressed type, or evidence absent from the commentary.
...
- for an ordinary Sunday or solemnity, page 1 contains the complete two-column, one-textual-unit-row `Textual Variable Parts` inventory with actual-form phase groups and no interpretive or source-status column, followed by the complete four-row `The Four Senses of Scripture` table; textually extensive forms preserve both modules without compression or omission;
...
- `The Propers: Themes and Movement` occupies exactly the next two substantive, readable pages, joins liturgical movement to theological and spiritual synthesis, and makes its decisive witnesses and limits recoverable in a scan without repetition or padding;
