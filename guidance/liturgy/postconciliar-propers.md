...
1. a compact title block naming the celebration, cycle or form, resolved date when applicable, formula or governing identity, edition-locale, and Lectionary or other principal locator;
2. an unheaded documentary two-column textual-variable-parts inventory in actual liturgical order;
3. an unheaded compact page-one four-senses table;
4. exactly two pages of `The Propers: Themes and Movement`;
5. the unbounded `The Propers: Detailed Commentary`;
6. optional `The Propers: Interpretive Possibilities`, with the required editorial or AI disclosure;
7. optional `Reception and Cultural Echoes`;
8. any required `Sacramental Appendix`;
9. a fresh-page `Appendix: Scriptural and Historical Coordinates`, containing the former `In Illo Tempore...` inventory;
...
11. `Appendix: Scope and Qualifications`, containing the work-wide edition, source, search, date, rights, limitation, and review apparatus;
12. `References`; and
13. terminal `Generation Metadata`, imported from `generation-metadata.tex`.

The title block, complete textual-variable-parts inventory, and four-senses table occupy page 1 for an ordinary Sunday or solemnity. The title block identifies the exact edition-locale, formula or governing identity, and occurrence when resolved because those facts identify the object the reader is using. Do not place an edition-verification, use, rights, source-status, or review-status callout between the title and the inventory; retain those disclosures in the terminal scope appendix, the standard rights notice, the catalog, and the tracked records. The inventory follows the title without a visible section heading, and the four-senses table follows the inventory without its own title or a second explanatory essay. Their column headers supply the needed local identification. The four-senses table contains exactly the visible rows `Literal`, `Allegorical`, `Moral`, and `Anagogical`. Each row is grounded in the appointed texts or documented reception and preserves branch limits; it may synthesize composed liturgical texts only by labeling the relation accurately rather than pretending that an oration is inspired Scripture.

`The Propers: Themes and Movement` begins immediately on page 2 and occupies exactly two substantive, readable pages. Its second page begins directly with its content or a descriptive subsection heading; do not repeat the section heading or add `(continued)`. `The Propers: Detailed Commentary` then begins on a fresh page and has no upper page limit. The detailed commentary and optional later sections expand according to the celebration's evidence and branch structure rather than a total-document page target. In the terminal apparatus, `Appendix: Scriptural and Historical Coordinates` occupies exactly one page for an ordinary Sunday or solemnity and ends at a forced boundary. A textually extensive target---including Palm Sunday, the Easter Vigil, the extended Pentecost Vigil, or a complex Ritual Mass---expands its complete historical inventory rather than omit passages, compress alternatives, or use unreadable type.

All material beginning with `The Propers: Themes and Movement` uses the shared `deepstudy` size or an exact readable equivalent; page 1 is not globally reduced merely to force the required survey into place. Visual distinctions remain intelligible in pure black and white, with no color-dependent distinction or gray fill. If an ordinary formula's complete, readable page-one survey or historical appendix table does not fit at the prescribed size, edit repetition and presentation first; do not omit an appointed unit, erase an alternative, or hide a material limitation to satisfy the boundary.
...
`Appendix: Scope and Qualifications` gives a compact reader-facing account of edition and formulary identity, text verification, principal direct and illuminating sources, geographic and chronological bounds, search method, and branch, rights, use, and review limits that affect the argument. It points to the leaf's `instance/manifest.md`, the leaf's composition audit at `propers/verified.md`, the canonical formulary owner's `propers/verified.md`, and the leaf's `research/scope.md`. A limitation that changes a particular claim remains beside that claim. `References` then contains only sources actually used, `Generation Metadata` follows it, and no expository prose follows the metadata.

### Structural rationale

The reader-facing order has three immediate movements: a page-one survey, two-page synthesis, and detailed exposition, followed by interpretive material and terminal apparatus. The opening inventory answers which texts the celebration appoints or concretely selects, while the four senses disclose the principal interpretive horizons without replacing exegesis. `Themes and Movement` compresses the guide's source-grounded architecture into two scan-first pages. `Detailed Commentary` supplies the unbounded evidence and claim-local qualifications. The historical appendix places the appointed scriptural words in their compositional and narrated settings; the resolution appendix records the authority and resolution of branches; and the scope appendix collects the work-wide bounds and limitations. Complete source, status, and unresolved-choice data remain in the tracked audits.

Within the unheaded textual-variable-parts inventory, one row represents one text-bearing liturgical unit. Closed alternatives belonging to that unit remain together in its `Text / citation` cell, while exact semantic branch IDs, authority, controlled status, and resolution remain in `Branch Resolution` and the tracked audits. Visible phase dividers follow the actual rite and omit empty phases. An ordinary Sunday may use `Introductory Rites`, `Liturgy of the Word`, `Liturgy of the Eucharist`, and `Concluding Rites` when a distinctive textual variable is inventoried there; Palm Sunday, Vigils, Ritual Masses, and other special forms use their own modules. This structure and its page rhythm were re-evaluated across every existing postconciliar proper guide on 16 July 2026.
...
The manifest and verified audit always record the applicable Preface rule and any Eucharistic Prayer choice that activates a proper insert. The textual-variable-parts inventory includes a Preface or Eucharistic Prayer insert only when the text is proper, narrowly appointed, or concretely selected in a resolved instance. The reader appendix states a Preface or Eucharistic Prayer dependency only when it changes an inventoried text or materially limits the exposition.
...
### Unheaded textual-variable-parts inventory
...
### Unheaded four-senses table

Immediately after the complete textual inventory, place a compact two-column table headed `Sense` and `Synthesis`. It has exactly these four rows and no substitutes:

- **Literal:** the appointed biblical passages in their narrative, grammatical, historical, and canonical settings, together with the composed texts' actual liturgical sense where they contribute materially;
- **Allegorical:** Christ, the Church, salvation history, and sacramental fulfillment as established by the canon, the liturgy, or documented reception;
- **Moral:** the conversion, virtue, ecclesial practice, or rightly ordered action disclosed by the appointed texts; and
- **Anagogical:** judgment, resurrection, heavenly worship, final communion, or the consummation actually supported by the textual and doctrinal evidence.

The table synthesizes the whole inventoried formula without pretending that every element shares one literary author or historical design. Preserve the difference between an officially correlated reading, a responsorial or acclamatory relation, an independent semi-continuous strand, a branch-specific text, and a source-grounded editorial synthesis. Do not place an original proposal, an unverified typology, or a claim depending on an unselected alternative in an unqualified page-one row.
...
- `editorial or AI proposal`: a plausible original extension reserved for `The Propers: Interpretive Possibilities`.
...
This section is the source-grounded architectural and theological synthesis. It occupies pages 2 and 3 immediately after the page-one survey. Begin with a one- or two-sentence governing account and one scan-first orienting form of no more than four primary stages. For an ordinary formula, use three to five relationship-titled units keyed to that account; a complex vigil or ritual may use the minimum additional units required by its actual repeated structure. Follow the celebration's real movement---for example, gathering or a special entrance rite, proclamation and response, preparation and thanksgiving, Communion, and sending---and its actual text-bearing structure.

Every inventoried text contributes to a functional grouping or is explicitly identified as an independent, semi-continuous, or optional strand. A thesis may be plural rather than force false unity. Decisive witnesses and guardrails are visibly labeled, no prose paragraph exceeds 120 words, and no unstructured prose run exceeds two paragraphs. Original AI proposals do not appear here. The second page develops the theological and spiritual dimensions already established by the movement rather than becoming a detached additional section. It may use a descriptive subsection heading and a compact dimension matrix, but it does not repeat the section heading or use a continuation label; it must make complete claims in full sentences and preserve branch-specific limits.

Both pages must carry substantive, readable, scan-first content. Do not meet the boundary through repetition, decorative enlargement, compressed type, artificial whitespace, or material withheld from the commentary merely to fill space. Conversely, do not let the synthesis introduce an evidentiary claim absent from the detailed commentary and tracked research. Edit the synthesis to exact length only after the detailed commentary is complete.
...
The table gives citations and historical orientation, not copyrighted biblical text. It begins on a fresh page in the terminal appendix block. For an ordinary Sunday or solemnity it fits one complete page and ends at a forced boundary; the heading, one-sentence interpretive key, and table are the only material on that page. End the table with one horizontal rule, never an adjacent dossier separator and closing rule. The Easter Vigil and other scripturally extensive targets expand as needed rather than omit readings, compress alternatives, or make the table unreadable. Full historical disputes and source-role details remain in `research/scope.md`.

## Appendix: Scope and Qualifications

This compact appendix records the edition-locale and formulary owner, included and excluded branch universe, governing calendar and jurisdiction, chronological and geographic bounds, text-verification and source scope, search method, rights and use limits, currentness, unresolved global questions, and review state. It points to the tracked manifest, leaf and owner audits, and research scope rather than reproducing them. Keep a branch-specific, territorial, historical, or source-status qualification beside the claim it changes, and do not repeat the appendix on page 1 or in `References`.

## Required structural modules

The macro-order above is stable; the textual inventory, visible phase groups, exposition, and liturgical-resolution appendix expand to match the actual form. At minimum, apply these modules to the registered targets already fixed in this profile:
...
7. **Draft from depth to compression.** Write `The Propers: Detailed Commentary` first; then derive the two-page `Themes and Movement` and the unheaded four-senses table. Populate the unheaded textual-variable-parts inventory only from the verified text audit; it contains no synthesized claim. Consolidate each unit's closed alternatives without erasing their distinctions, and place historical coordinates, work-wide source and status qualifications, and resolution mechanics in their respective appendices and tracked records. Edit the synthesis to its exact two-page boundary without padding, repetition, compressed type, or evidence absent from the commentary.
8. **Assemble and verify.** Apply the required macro-order so the synthesis begins on page 2 and the historical, resolution, and scope material remains in the terminal appendix block; refresh structured metadata; build for enough passes to settle references and contents; inspect the log; and visually inspect every page, textual inventory, and branch table before installing a PDF.
...
visible in the guide's terminal scope appendix, catalog, and research records, and do not claim an
...
- for an ordinary Sunday or solemnity, page 1 contains the complete unheaded two-column, one-textual-unit-row textual-variable-parts inventory with actual-form phase groups and no interpretive or source-status column, followed by the complete unheaded four-row four-senses table; the title is followed by no edition-verification or status callout, and textually extensive forms preserve both modules without compression or omission;
- the four-senses table has exactly `Literal`, `Allegorical`, `Moral`, and `Anagogical` rows grounded in the appointed texts or documented reception, preserves branch and relationship limits, and contains no disguised original proposal;
...
- `Appendix: Scriptural and Historical Coordinates` covers every distinct directly appointed scriptural passage and marked adaptation in canonical order, distinguishes composition from narrated event, occupies exactly one page for an ordinary Sunday or solemnity, and expands rather than omits material when the target is complex;
...
- `The Propers: Themes and Movement` begins immediately on page 2 and occupies exactly two substantive, readable pages, joins liturgical movement to theological and spiritual synthesis, makes its decisive witnesses and limits recoverable in a scan without repetition or padding, and does not repeat its section heading or add `(continued)` on the second page;
- `The Propers: Detailed Commentary` begins on the following fresh page, remains unbounded, and supplies the evidence and qualifications behind the synthesis; any `The Propers: Interpretive Possibilities` remains separately disclosed and exploratory;
- the textual record contains every appointed or concretely selected text; the tracked audits account for all alternatives, unresolved selections, omissions, and applicability results; and the reader appendix presents the subset needed to identify the instance or interpret the textual record or exposition;
- `The Propers: Themes and Movement`, `The Propers: Detailed Commentary`, `Appendix: Scriptural and Historical Coordinates`, and `Appendix: Liturgical Resolution` begin or end at their prescribed page boundaries without compressed type, artificial whitespace, or omitted material;
- optional `Reception and Cultural Echoes` and any required `Sacramental Appendix` occur before the terminal historical, liturgical-resolution, and scope appendices, which in turn precede `References`;
...
- `Appendix: Liturgical Resolution` preserves the reader-facing instance and the authority, status, and resolution of branches needed to interpret the text record or exposition; `Appendix: Scope and Qualifications` consolidates the edition, source, search, date, rights, use, limitation, and review apparatus and points to the tracked records; `References` contains no duplicate scope panel; and imported structured `Generation Metadata` is terminal;
