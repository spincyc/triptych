...
The **ferias from January 2 through January 5 in the 1962 Missal take the Mass of January 1**. This is the Missal's own date-keyed provision, not an inference from the preceding Sunday: printed p. 34 (artifact page 115) appoints that Mass with the Gloria and the Nativity preface, without the Creed and without the proper *Communicantes*. It governs every weekday in the span whether the Holy Name occupies its Sunday or whether no Sunday fell within the Christmas octave. It does not govern the Holy Name itself, any other feast that wins occurrence, or any pre-1955 recension; apply it as a 1962 ferial formulary reference and never as cross-recension borrowing.
...
For `PC-S28` through `PC-S59` the Ordinary Time Sunday number and the shared formulary owner are computable and independently checkable:

`Ordinary Time Sunday = parent number − 26`, and the owner directory is `temporal/shared/ordinary-time/weeks/NN` with `NN` that Sunday number written in two digits.

`PC-S26` (Most Holy Trinity) and `PC-S27` (Most Holy Body and Blood of Christ) are Solemnities of the Lord with their own formularies and owners; they consume no `weeks/NN` directory. `PC-S60` (Christ the King) likewise has its own formulary and owner. Weeks 01 and 34 have no numbered Sunday consumer. The parallel Lectionary-number check, target counts, keys, and slugs remain in [the postconciliar proper registry](postconciliar-propers-registry.md).
...
- Because `P − 24` never exceeds four, only the Third through Sixth Sundays after Epiphany are ever resumed. These are the `64` through `67` catalog identities in the 1962 proper-guide profile, each a complete, separately sourced formulary that joins the Epiphany orations, Epistle, and Gospel to the chants appointed for resumed use.
...
The January 2–5 ferial provision above was established on 20 August 2026 from the page image of printed p. 34, artifact page 115, in `artifact.catholic-church.missale-romanum.vatican-typica-1962.cmaa-facsimile-pdf`. The inspected 82,815,941-byte artifact matched its registered SHA-256 `648fdb8fe830ed65a08aa4a95de6f94424c533ddf2398c8fc26b18735fd3518a`; the image, rather than its optical text layer, controlled the reading.
