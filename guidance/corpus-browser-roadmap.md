...
| --- | --- | --- |
| B0/B1 shared non-liturgy implementation and harness | Steps 1-4 and 9 satisfied on `main`; step 5 partial; candidate on `impl/corpus-foundation-b0-b1` | The step-by-step disposition, evidence, and the one remaining blocker are `guidance/corpus-browser-implementation.md` §11.1, which owns them; do not restate the matrix here. Steps 6, 7, and 8 are not open work for this lane: 6 is withdrawn by D2/D18, and 7 and 8 wait on a surface lane that needs them. The blocker is `day-missal.css`'s twelve unscoped `body > .site-header` selectors, which need a protected-Liturgy carve-out. |
