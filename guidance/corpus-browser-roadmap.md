...
| E1 Catena production implementation | Eighth correction candidate (V7) awaiting fresh independent review; the disposition it answers is **CHANGES REQUIRED** at V6 head `4639b139f` (independent review `f183ed1b0`, 2026-08-14) | That review preserved the language normalization, the exact voice support, the neutral refusal umbrella, padded-verse deduplication, null termination, clean encoding, the ownership boundaries and the unraised ceilings, and found the typed boundary incomplete: raw fragment properties were copied forward and guarded afterwards, so an unvalidated `text_path` still reached the request sink, hollow members still made rows and claims, a contradiction still leaked selected rights prose, and an unreadable root still manufactured a domain negative. V7 answers it with an explicit typed projection as the only source for rendering, tallies, rows, findings, refusals, paths, identity, rights text, provenance and request identity, and with package claims machine-derived rather than transcribed. Preserve accepted E0 and every sound V2/V3/V4/V4.1/V5/V6 correction. The exact Catena data seam is authorized but transfers no broader ownership; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
