...
### E1 Catena correction V12 independent review — 2026-08-17

Disposition: **CHANGES REQUIRED** at exact V12 head
`d312786dd2b23926aa88e29ea15647dfcc7e7e6e`, comparison parent
`0255b84996e1dc24da3ce75ac318c4f774b7957c`, literal Git parent
`79a306fe50a98e8315d96bb9386c6fe4cacf6e89`, and V11 review
`22b9bdad5e71920a103e3ec3bcf2f79bba50cebb`. The review inspected evidence
`05306fcfe221c1b0456501463e02323047635607` and the 1,842,342-byte,
81-member ZIP at
`fa43918166b2d708c7911e3604834499260884d8433b9cd665bd7fc0ccf40890`.

The helper is sound for one call: one `Object.getPrototypeOf` call, one
descriptor per name, zero accessor invocations, frozen null-prototype own data,
and no second value inside one projection. That closes the inherited prefix,
inherited refusal,
and carried-path validate-one/project-another defects locally. The page does
not retain that snapshot. It runs `chapterFragments()` over the same raw
spine/fragment three times — readability, tally, render/request — so parent 6
and V12 3 are respectively two and one `text_path` descriptor reads per
projection times three, not a proof that the source cannot be revisited.
Walking prefix/path/prototype answers can pass an earlier gate and produce a
later usable fallback row. The required one-source-snapshot request/cache/body/
ownership invariant therefore remains open. The page-wide critical-prototype
fail-closed policy is accepted; irrelevant pollution and ordinary corpus rows
remain unaffected.

The V12 sink assertions pass, ordinary behavior and the 36-field late proof do
not regress, but the evidence is incomplete. Five of six planted scenarios
are non-vacuous at the parent; the noncycling drifting-path scenario makes no
parent text request or body leak, and only its alternating sibling does.
`journal-dump.py` ships no V12 scenario, so the dedicated head/parent journals
do not retain the new ownership rows. The static pin cannot see repeated
execution of its one lexical call site.

The package's current truncated ledger resolves one effective authority,
`package-20260817T194757Z-05q7ksvh`, and the actual happy path passes P8 and the
coherence command. Authority is still not coherent enough for acceptance.
Four discarded assemblies and four set-aside batteries are omitted from the
sealed ledger despite the complete-history claim, ordinals were reused, and
the retained battery rows are not terminally refused. The frozen timestamps
are internally nonmonotonic. The P5 authoritative row survives inside the
package and ZIP if P7/P8 later fails; the gate consumes neither ZIP, sidecar,
P8 result, complete outer ledger, nor sibling marker. It also accepts several
required negative fixtures, including later discarded/second-authority state
rows and multiline or absent authority prose. Clean wrong-endpoint and dirty
postflight cases are not mechanically bound.

Package privacy and completeness also fail. Outer assembly/P8 logs disclose
the builder home/workspace/tool anchor, package records retain a generic
`/tmp/tmp.*` root and flattened workspace slug, and those bytes contradict the
claimed zero-hit scope. The handoff inventory omits its own sibling log, parent
failure and limitation counts disagree across records, the packaged V12
journal is absent, and executed-to-shipped digest provenance directly binds
only four of fifteen currently matching tools. ZIP identity, CRCs, manifest,
central-directory arithmetic, P8 non-execution/final rehash, final UTF-8/control
cleanliness, evidence topology, V11-package preservation, ownership boundaries,
ordinary controls, late proof, and inherited validation populations pass.

Exact next action: one bounded V13 from exact V12 head. Create one retained
request-critical chapter projection and pass it unchanged to readability,
tally, render, request/cache/body, and ownership; add walking invalid-to-valid
page tests and make every planted parent case non-vacuous. Ship the V12/V13
ownership journals. Reseal with a complete append-only attempt history,
post-P8 authority bound to ZIP identity and P8 success, full negative authority
and endpoint gates, every executed-tool digest, substantive handoff inventory,
and privacy coverage over the package and all outer artifacts. Do not enter a
broader E1 blocker, integrate, merge, re-sign, deploy, or cut over.
...
| E1 Catena production implementation | **CHANGES REQUIRED** at exact V12 head `d312786dd2b23926aa88e29ea15647dfcc7e7e6e` by the fresh independent review on `review/catena-wave-1-e1-corrections-v12-independent` (2026-08-17) | V12 closes the three V11 inputs inside one projection and preserves ordinary/late behavior, but the page reprojects the same raw records three times, violating the required one stable request-critical snapshot. One planted parent case is vacuous and the package omits every V12 ownership journal. Package authority is not failure-safe before P7/P8; the sealed ledger omits discarded history, reuses ordinals, carries untruthful chronology, and the reader gate misses required authority/endpoint contradictions. Outer and package bytes leak workspace/session identities; handoff counts/inventory and executed-tool provenance are incomplete. Exact next action: one bounded V13 from this exact head sharing one retained chapter projection across readability/tally/render/request sinks, then a fresh complete, privacy-clean, post-P8-authoritative package. Preserve accepted local inherited-prefix/refusal/carried-path behavior and all V2–V11 sound work. Broader projection/orphan/source-only/translator/absence/selection/refusal/root/terminal/oracle/CLI-budget-data-seam and separately owned release/common-gate/B0/shell/device/AT/Liturgy/PDF work remains open. E1 stays off main; no merge, re-sign, deploy, or cutover. |
...
| 2026-08-17 | E1 Catena correction V12 independent review | **CHANGES REQUIRED**. Per-call snapshotting, inherited-prefix/refusal closure, carried-path single-projection identity, ordinary controls, the 36-field late proof, exact validation failure sets, P8 mechanics, ZIP arithmetic, evidence topology, and ownership boundaries pass. The same raw chapter is nevertheless projected three times, one planted parent case is vacuous, V12 ownership journals are absent, pre-P7/P8 authority is not failure-safe, complete attempt/refusal history and chronology are false or missing, negative authority/endpoint gates are incomplete, outer/package privacy fails, substantive handoff inventory/counts disagree, and executed/shipped identity is directly bound for only four tools. | Reviewed `d312786dd2b23926aa88e29ea15647dfcc7e7e6e`; comparison parent `0255b84996e1dc24da3ce75ac318c4f774b7957c`; evidence `05306fcfe221c1b0456501463e02323047635607`; ZIP `fa43918166b2d708c7911e3604834499260884d8433b9cd665bd7fc0ccf40890`; review branch `review/catena-wave-1-e1-corrections-v12-independent`. |
