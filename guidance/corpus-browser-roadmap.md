...
| E1 Catena production implementation | Ninth correction candidate (V8) awaiting fresh independent review; the disposition it answers is **CHANGES REQUIRED** at V7 head `e876b29e5` (independent review `d9ad5ec1a`, 2026-08-15) | That review passed the typed projection's valid-corpus behavior, the preserved V6 boundaries and the unraised ceilings, found ten remaining production defects with the text namespace open at the request sink first among them — `structure/paragraphs/` composed a request and a carried same-stem `structure/paragraphs/text/` path fetched a real Sources text — and gave one exact next action: a V8 whose first bounded commit is only the byte-exact `structure/catena/text/` closure for prefix and fallback paths with same-stem wrong-namespace request-sink regressions. V8 is that commit alone; the review's other enumerated blockers remain open for the authorized continuation. Preserve accepted E0 and every sound V2–V7 correction. The exact Catena data seam is authorized but transfers no broader ownership; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
