...
### E1 Catena correction V5 independent review — 2026-08-14

**Disposition: CHANGES REQUIRED.** Reviewed candidate
`19982ab433dd25704ed60b1ac6ddb678bc3a98f9` at parent
`f93757854b54c19e50bdcb97ca0fed9b48d22bb7`, against prior review
`7f69575b982926e827974f2ed236b1c8bfd8aaad`. Evidence branch
`evidence/catena-e1-corrections-v5-handoff` is exact commit
`fe71d03e51bc3a89f01b9262cd3a4d9077bb0cef`; package
`20260814T123524Z-catena-e1-corrections-v5.zip` independently hashes to
`18500400ce617365ef8322e41f011f44dc5a0a88dc39fbbcb5deb1abd78b75ea`.
Current main remains `9b9ff74a77d1bcd7d454d2a7fc448b8a6c8f1fd4` and is an ancestor of the
candidate.

| Review question | Independent judgment |
| --- | --- |
| Exact scope and commit structure | Exact production diff is `catena-model.js` plus `catena.js`; one test and four records complete the seven-file range. Only two commits touch production, one later commit changes a test comment, and the final commit is records-only. No data, CSS, HTML, release, common-gate, shell, Liturgy or PDF path changed. |
| Language and identity metadata | **Changes required.** Malformed fragment/Bible language is guessed as `lang="en"`; raw Bible language still renders `[object Object]` in an option. Arbitrary textual IDs/paths remain routable/fetchable, malformed source keys can coerce into valid source records, and malformed testament becomes `New Testament`. Normalize every visible/routed identity and omit unsupported claims. |
| Collection members and valid siblings | **Changes required.** `records()` filters container shape only. Malformed record objects render blank lead/blocked rows, enter tallies, manufacture an empty refusal, and can mask a later valid held/refusal/absence sibling. Add record-family semantics before counting, state, selection or rendering. |
| Typed absence findings | **Changes required.** A malformed same-language finding before a valid one erases valid identity. A stray `partial` on an unknown or unsurveyed finding manufactures `Partly public domain`. Prefer recognized typed records and license partial prose only from `partial-public-domain`. |
| Numeric and verse metadata | Core positive-safe-integer normalization is sound, but **proof changes are required**. The word oracle deduplicates class names, the verse oracle never reads Scripture, extent edges are incomplete, and `"01"` can duplicate verse 1. Inspect exact sink text and canonical keys with full edge cases. |
| Path, bootstrap and partial arrival | **Changes required.** JSON `null` can throw after bootstrap's request catch; malformed roots can collapse into corpus emptiness; and `startFailed()` does not invalidate pending render work. Funnel all post-fetch assembly through one terminal transaction and prove an actually late completion cannot repaint it. |
| Route completion | The shim runs tracked production code, but does not prove the claimed whole application state. No V5 test asserts focus; replace/status/tally/announcement checks are incomplete; the malformed payload silently becomes blank; the later valid action has weak positive assertions; and the “nothing stale” case has no late stale work. Replace the lossy oracles with exact hash/push/replace/control/status/tally/busy/focus/content assertions. |
| Extra fixes, voices and refusal behavior | The reflected-`lang` harness improvement, language-shape gate and silent-return funnel are legitimate bounded findings. Exact supported voices remain `original`, `translation:en`, `translation:la`; unsupported Greek/unknown voices fail closed. Neutral refusal umbrella copy is unchanged. These passes do not cure malformed object refusals or raw Bible labels. |
| Adversarial fixtures | The replay and Chromium probe run tracked production code with substituted fabricated data, and the package discloses that globally. Raw JSON/PNG artifacts are not self-labeling, however, and the replay uses a local DOM/history/fetch shim. Several oracles expressly bless blank counted records or guessed language and therefore demonstrate remaining defects rather than correctness. Retain fabricated fixtures, label each artifact, and inspect the real presentation/state sinks. |
| Budgets and unchanged accepted areas | Formal ceilings pass unraised: CSS 7,629/8,000 whole and 2,676/2,700 stripped; page JS 12,990/13,000 whole and 8,363/8,800 stripped. The model itself grows 5,359 whole gzip bytes and combined page-plus-model grows 5,379; relocation explains formal compliance but is not unchanged load. The accepted V4.1 real-corpus visual baseline is not reopened by the code-only delta. |
| Evidence integrity and privacy | The 60-member manifest verifies, the ZIP matches it, and no private host token was found. **Evidence correction is required:** validation checkout provenance conflicts with the 30-entry ledger, an alleged unfiltered parent run/filter is absent, a focused log is not reproducible from its stated command, five before/after image pairs are byte-identical while labeled visibly different, after-images carry before-state descriptions, the probe has no focus field, and the reusable sealer misses timezone aliases and can leave a stale manifest on failure. |
| Data seam, release and other owners | `src/web/data/` is byte-identical and the day-reader contradiction is preserved for its owner. Four stale release bindings remain fail-closed; none was re-signed. Common-gate, shared-shell, real-device/AT, protected Liturgy and PDF obligations remain separate and open. |
| Ancestry and integration | Current main is the exact merge base and ancestor, so no textual code conflict is forecast. The prior review is correctly a sibling at the reviewed parent. Any future accepted landing is cumulative and must semantically reconcile the four durable records. No integration authority is granted here. |

Independent reruns at the exact candidate verify the focused Catena suite
(`306`, exit 0), promise ledger (`30` tracked / `19` complete, exit 0), Catena
model (`1,351` fragments / 1 Bible / 73 canon books, exit 0) and exact package
digest. Unrestricted full discovery ran `1,657` tests in 494.977 seconds and
exited 1 with the sealed population of 14 failures, 13 errors and 11 skips;
the sealed parent ran 1,618 with the same 27 fully qualified FAIL/ERROR header
identities. Fresh unrestricted browser gates at both parent and candidate
exited 1 with 2,290 assertions: 1,836 pass, 226 fail and 228 skip, with every
status identity unchanged. They were not whole-report equal: unrelated
`/liturgy/day.html` text-200 tab detail was `10 of 10` at the parent and `9 of
10` at the candidate while remaining a pass. Fresh `make -k check` exited 2
at `check-release-bindings`, `check-tool-registry` and `check-examples`.
Preliminary sandboxed full/gate attempts failed localhost setup with `EPERM`
and are excluded in favor of these unrestricted results. No failing command
is rounded into green.

The exact next action is one bounded V6 from `19982ab4`: complete the semantic
root/member boundary and render-transaction invalidation, replace the lossy
runtime oracles with exact whole-state assertions, and publish one corrected,
self-labeling immutable package for fresh independent review. No candidate
amendment, main merge, release signing, deployment or separately owned repair
...
| E1 Catena production implementation | **CHANGES REQUIRED** at V5 head `19982ab433dd25704ed60b1ac6ddb678bc3a98f9` (fresh independent review, 2026-08-14) | V5 preserves the exact supported voices, neutral refusal copy, unchanged data/CSS/HTML and formal ceilings, but semantic object members, Bible/language/source/path/testament joins, finding ordering, null bootstrap, pending-render invalidation and exact route/numeric/verse proof remain incomplete. Its package is integral and private in this environment but contains material provenance, reproducibility and labeling errors. The only authorized next action is the bounded V6 described above. Preserve accepted E0 and sound earlier corrections. Data, release, common-gate, shell, device/AT, protected Liturgy and PDFs remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
...
| 2026-08-14 | E1 Catena correction V5 independent review | **CHANGES REQUIRED** at `19982ab433dd25704ed60b1ac6ddb678bc3a98f9`. The record boundary remains shape-only for semantic objects, leaves raw Bible and identity sinks, makes finding selection order-dependent, and does not close null-bootstrap/pending-render transactions. Runtime oracles omit the exact focus, Scripture, tally, history and stale-work proof claimed. Package bytes and privacy pass, but validation provenance, parent-run completeness, screenshot descriptions, focus evidence and sealer regression proof require a corrected successor. Voice keys, neutral refusal copy, unchanged data/CSS/HTML, formal budgets and earlier real-corpus visual baseline remain preserved. | Evidence `fe71d03e51bc3a89f01b9262cd3a4d9077bb0cef`; package SHA-256 `18500400ce617365ef8322e41f011f44dc5a0a88dc39fbbcb5deb1abd78b75ea`; bounded V6 and fresh review required. No merge, signing, deployment or outside-owner repair authorized. |
