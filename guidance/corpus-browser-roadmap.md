...
### E1 Catena correction V9 independent review — 2026-08-15

**CHANGES REQUIRED** at exact candidate
`3c5b78249193df065c4e1c2ee5a98e5989c6e582`. V9 correctly closes the composed
request escape: the model preserves absent, present-valid, and present-invalid
prefixes through projection; only genuine absence consults a valid own-stem
carried path; the exact V8 planted fallback makes no request or substitution
cold, prewarmed, or genuinely late; and the primitive byte-exact namespace
matrix, valid controls, focused suite, inherited regression identity, formal
budgets, and ownership boundary pass.

The projected third state stops at the model. `catena.js` never consumes
`text_refused`; it sends the empty path through the same `ABSENT` sentinel as
genuine absence and tells the reader the fragment “carries no text file” even
when the fixture carries a valid text file whose governing prefix was refused.
The prewarmed proof does not exact-pin the whole final request journal,
announcement writes, replacement, row identity/count, or focus; the late proof
compares all 36 guarded fields before/after release but does not exact-pin B's
complete baseline; and the present-valid control pins only its request and
body. Correct the visible refused state and exact terminal proofs while
preserving the request closure.

The exact 272,005-byte ZIP verifies as 59 unique members and 3,202,934
uncompressed bytes; its 58-row manifest covers every other final member with
matching hashes, the frozen/derived partition sums exactly, privacy and byte
scans pass, and the seven-check P8 replay leaves the ZIP unchanged. Package
protocol still needs one new immutable artifact: retained-parent provenance
is not contemporaneous in the execution logs; a duplicate order slug
overwrites one request-journal log and disagrees with the shipped battery; the
P8 transcript does not bind its success to the ZIP name/hash/size; required
handoff and review-request fields are absent; and the assembler overwrites a
reused timestamp. The next bounded V10 begins from exact V9 head, renders a
distinct neutral present-invalid state, fails contradictory prefix claims
closed, exact-pins cold/prewarmed/late/present-valid terminal vectors, and
rebuilds that protocol in a new timestamp. E1 remains off main; every broader
V7 and separately owned blocker remains open.
...
| E1 Catena production implementation | **Changes required** at exact V9 head `3c5b78249193df065c4e1c2ee5a98e5989c6e582` (fresh independent review on `review/catena-wave-1-e1-corrections-v9-independent`, 2026-08-15) | V9 preserves the three prefix states through the request decision, permits carried fallback only on genuine absence, blocks the exact V8 escape cold/prewarmed/late, and preserves the byte-exact namespace boundary. Its visible sink ignores `text_refused` and collapses present-invalid back into the false “carries no text file” absence message; the prewarmed, late, and present-valid tests also omit material exact terminal assertions. The final ZIP arithmetic and manifest are exact, while clean retained-parent provenance, transcript identity, unique non-overwriting logs, immutable assembly, and required handoff/request structure remain incomplete. Make one bounded V10 from exact V9 head that preserves the request closure, renders the refused state truthfully, completes the sink proofs, and issues a new immutable package. Preserve accepted E0 and every sound V2–V9 boundary. Broader V7 production/oracle/architecture blockers and the historical data-seam contradiction remain open; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy, and PDFs remain separate. E1 stays off main. No merge, re-sign, deploy, or cutover. |
...
| 2026-08-15 | E1 Catena correction V9 independent review | **CHANGES REQUIRED** at exact candidate `3c5b78249193df065c4e1c2ee5a98e5989c6e582`. The request layer preserves absent/present-valid/present-invalid and blocks the exact V8 carried escape cold, prewarmed, and late; primitive namespace cases, valid controls, inherited regression identity, formal budgets, ownership boundaries, and exact final ZIP/manifest bytes pass. The visible renderer never consumes `text_refused` and falsely reports a carried-but-refused text as no file, while prewarmed, late, and valid-control regressions omit material exact terminal assertions. Retained-parent execution identity, unique logs, immutable assembly, P8 transcript identity, and required handoff/request fields also remain incomplete. One bounded V10 must preserve the request closure, render a distinct refused state, complete the terminal proofs, and issue a fresh immutable package. | Review branch `review/catena-wave-1-e1-corrections-v9-independent`; evidence `eb1ee9987425339c2b5522987bee1fb862cd7d33`; package `20260816T044924Z-catena-e1-corrections-v9.zip`, SHA-256 `4872b04ed47873d84e5a8090aff42d3e43b7a2c11653484d659205db5488d945`; current main `cfe588f81c1c239341b48f82e1b27ab55f582fd3`. No candidate modification, merge, re-signing, deployment, or outside-owner work occurred. |
