...
### E1 Catena correction V8 independent review — 2026-08-15

**CHANGES REQUIRED** at exact candidate
`7e4df42a21bc2be2d28ff14943f63af3e7e3a6f8`. The namespace primitives reject
the requested wrong, padded, traversal, absolute, encoded, sibling, parent and
`textual/` paths, valid real and fixture paths remain byte-identical, and the
isolated wrong-prefix and carried-path parent defects are closed. The composed
rule still loses whether `text_prefix` was absent or present-but-invalid:
`chapterFragments()` turns either into `''`, after which `fragmentRow()` uses a
valid same-stem carried path. A production replay with wrong prefix
`structure/paragraphs/` fetched
`structure/catena/text/fallback-owned.json` and rendered its planted body. The
committed scenarios never combine an invalid prefix with a carried fallback,
never assert final `statusText`, and do not prove prewarmed or late
substitution cannot occur.

The exact ZIP digest, safe layout, CRCs, manifest, sanitization, and Git/test/
budget identities verify, but `claims.json` does not inventory the final sealed
bytes: it understates the package by 1,822 bytes and has stale rows for
`DERIVED-CLAIMS.md`, `MANIFEST.sha256`, itself, and the two derivation/audit
logs. The retained parent evidence reproduces the exact V7 baseline but does
not explicitly ledger the discarded `/tmp` run or its own exact-SHA clean cwd.
V8 therefore remains off main. The next correction is limited to preserving
the prefix tri-state, proving zero carried/cache fallback at the production
sink with full terminal assertions, and issuing a truthful new immutable
package. Every unrelated V7 and separately owned blocker remains open.
...
| E1 Catena production implementation | **Changes required** at exact V8 head `7e4df42a21bc2be2d28ff14943f63af3e7e3a6f8` (fresh independent review on `review/catena-wave-1-e1-corrections-v8-independent`, 2026-08-15) | V8's namespace validators, valid-path preservation, isolated wrong-namespace rejections, micro-boundary and formal budgets pass. A present-but-invalid prefix still collapses to the absent-prefix state and activates a valid same-stem carried fallback, which reaches production `fetch` and renders a planted body. The tests omit that cross-field case, final `statusText`, and prewarmed/late substitution; the package's final-byte inventory is stale. Correct only that V8 closure/evidence boundary from the exact head, rebuild one truthful immutable package, and obtain another fresh independent review. All other V7 production, terminal, oracle, architecture and payload blockers remain open. Release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy, and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy, or cutover. |
...
| 2026-08-15 | E1 Catena correction V8 independent review | **CHANGES REQUIRED** at exact candidate `7e4df42a21bc2be2d28ff14943f63af3e7e3a6f8`. The review passed the primitive byte-exact namespace boundary, all 1,356 real path occurrences, 47 preserved fixture paths, isolated wrong/padded-prefix and carried matrices, inherited regression identities, ownership boundaries, and unraised formal budgets. It reproduced the remaining production escape: a rejected wrong or padded prefix becomes the same empty value as an absent prefix and activates a valid same-stem carried path, which reaches `fetch` and renders a planted body. The test matrix omits that combination and exact full terminal/cache proof. The ZIP transport and manifest verify, but its machine inventory understates the final package by 1,822 bytes and contains five stale member rows. | Review branch `review/catena-wave-1-e1-corrections-v8-independent`; evidence `60122b472c3f9a09aff5f8663eb3b062c585a557`; package `20260816T011521Z-catena-e1-corrections-v8.zip`, SHA-256 `002a75d587a3535d7df9d2b3239d71d811cc7948e9d219e5d9118edcb21e4ff7`; current main `9b9ff74a77d1bcd7d454d2a7fc448b8a6c8f1fd4`. No candidate modification, merge, re-signing, deployment, or outside-owner work occurred. |
