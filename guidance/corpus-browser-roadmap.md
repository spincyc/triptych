...
| E1 Catena production implementation | Fourteenth correction candidate (V13) awaiting fresh independent review; the disposition it answers is **CHANGES REQUIRED** at exact V12 head `d312786dd` (independent review `728c3e3b3` on `review/catena-wave-1-e1-corrections-v12-independent`, 2026-08-17) | That review passed `requestSnapshot` for one invocation, the inherited prefix and inherited refusal closing locally, the carried path read once per projection, the fail-closed prototype policy as a design, ordinary request behaviour, the malformed and unestablished wording, the cold, present-valid and prewarmed controls, primitive namespace closure, the 36-field late vector and its `0 -> 1` release, the read-only non-executing P8 with its rehash, ZIP arithmetic and CRCs, candidate immutability and ownership boundaries — and found the stable snapshot was not stable for a page RENDER. The same raw spine and fragments were projected three times, by `spineUnreadable`, by the tally and by `renderChain`, so one descriptor read per projection times three proved no source revisit and a record answering differently between projections rendered and requested from an answer nothing approved. Beside it one of six planted scenarios was vacuous, the packaged journals stopped at V11 and carried no scenario of the lane they shipped for, the sealed bytes claimed authority before P8, the authority gate accepted six contradictions, four discarded attempts and four set-aside batteries were absent from every ledger while ordinals were reused, the 10/10 inventory was lexical and blind to its own output log, twelve tracked outer-log lines and two in-archive residues leaked machine-specific paths, and the executed-byte proof covered four tools of fifteen. Its exact next action: normalize each loaded chapter exactly once into one stable page-level projection and pass it unchanged through readability, tally, request, cache, body and ownership; add walking raw-state tests asserting one observation per loaded record; make all six planted scenarios non-vacuous and ship their ownership journals; and issue a fresh package with one complete append-only ledger, no ordinal reuse, truthful reasons and times, post-P8 authority bound to the ZIP, full negative fixtures, executed and shipped digests for every claimed tool, a substantive inventory, and privacy scanning of every member and outer artifact. V13 is that correction alone; every other enumerated blocker remains open for the authorized continuation. Preserve accepted E0 and every sound V2–V12 correction. The exact Catena data seam is authorized but transfers no broader ownership; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
