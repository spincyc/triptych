...
| 2026-08-31 | Independent cold rereview of the selector oracle | **B0/B1 — ACCEPT_WITH_CORRECTIONS.** An uncontaminated clone detached at the candidate, driving `Chrome/151.0.7922.173`, confirmed nine claims: (1) Chromium and not Python decides selector truth — the whole matching machinery is deleted, and sabotaging the harness's `reach()` to answer safe turns the suite red with 55 failures; (2) all six cold-review counterexamples reach chrome, reproduced with the reviewer's own CDP script rather than a harness helper; (3) the legitimately route-dependent forms are still permitted, verified by independent `querySelectorAll` in the neutral shell; (4) fail-closed is real — dropping the refusal fails four assertions, and eight of the reviewer's own probes are refused and unsafe, including CSS nesting failing closed in extraction; (5) the four protected inventories are exactly 12/3/2/3, independently re-derived, and fail under substitution, removal, and a comma-inside-functional-pseudo substitution; (6) zero new hazards across 15 sheets, 1,721 arm occurrences and **1,193 unique arms with zero refusals**, Scripture included; (7) one browser session, 1 batch, 72 navigations, 52 arms, 665 ms startup, 34 tests in 9.154 s; (8) Blocker A stays closed at `test_browser_model_gate` 22/OK; (9) no protected Liturgy, Catena or release-binding change, the candidate diff being exactly six paths. It probed **52 adversarial arms** beyond the candidate's list and found each classified as claimed. **One latent fail-open:** the walk forces one user state at a time, so an arm needing two simultaneous user states on two different chrome elements was reported safe — real Chromium, after a real Tab and a real pointer move, matches `a:focus ~ .site-footer:hover` and `.skip-link:focus ~ .site-footer:hover a` against layout-owned elements where the harness reported `reach={}`. No production selector has that shape today, so it was latent rather than live. **Four corrections required:** disclose the one-state-at-a-time bound in §11.4 with that witness; refuse that shape instead of reporting it safe; assert the harness's `interactive` measurement or withdraw the claim that a caller can fail on it; and reword "the changed paths are …" to "the only changed non-record paths are …" in §11.4 and `PROJECT-WORK.md`. The rereview did NOT re-run full discovery, `make -k check`, the artifact gate, `check-examples`, `check-browser-static`, `check-browser-harnesses`, the Catena suites, the site builds, or the Scripture route proof; it relied on the previously measured baseline for those. `shared-shell-blocking-collisions-resolved` stays `blocked`; no merge, deploy, signing, binding refresh, protected-Liturgy edit, Catena edit, or next lane is authorized. | Reviewed candidate `73363fcbce22fc551528047c7e69f33275d54b58`; base/instruction commit `2440e3e84929c81bc42631bcd3622c592f71da39`; branch `impl/corpus-foundation-b0-b1`; the nine claims, the fail-open witness, the 52 adversarial arms, and what was not re-run are owned by `guidance/corpus-browser-implementation.md` §11.5 and are not restated here; this row is the record commit, and the corrections land in the commit after it. |
