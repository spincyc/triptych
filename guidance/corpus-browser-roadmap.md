...
### E1 Catena correction V14 independent review — 2026-08-21

**SEMANTIC CHANGES REQUIRED; EVIDENCE CHANGES REQUIRED; overall CHANGES
REQUIRED** at exact V14 head
`69f2575421ba976271c936b1abd4b39dbe8b98fd`, against comparison base
`6cc85e1a1dea317a48c0bfcfd6f774201ea3a6c3`. Evidence commit
`f74f8f4d4de44e21afdbef1fc4e9589a9898e986` carries the 1,366,960-byte,
69-member package with SHA-256
...
The read-once chapter projection, `unfetched` walking proof, separate tally,
structural inventory, getter non-invocation, frozen projection graph and V13
strong cases pass. Ask-time row ownership does not survive the path-keyed
pending Promise. In the purported late case, B remains `Loading…` until A's
sole held request is released, then B renders A's planted body; the green test
requires that result. Body application is absent from the actual-object roster
and the package journal. Nested descriptor multiplicity, the missing edition
accessor case and downstream mutation rerender are bounded proof gaps to close
with the lifecycle correction.

ZIP structure, manifest, read-only P8, post-P8 authority identity and privacy
pass. Evidence closure fails because two relevant commands are explicitly
`ELIDED`, the parent replay is `PROSE`, the executed-tool table contradicts the
assembly transcript, and claimed complete/append-only attempt history omits
material rows. The absent V13 review SHA is a truthfully recorded historical
limitation, and the missing literal fresh-clone fields are a bounded condition;
neither is the evidence blocker.

Exact next action: one smallest semantic V15 from the exact V14 head. Preserve
row/projection authority through pending transport, completion and body
application, or share only fulfilled immutable values; prove A held, B settled
independently, then A late without any B change. Complete the bounded nested
source and mutation proofs and rebuild evidence with exact portable commands,
complete history and one final coherent executed/trusted/shipped table. Do not
merge, integrate, re-sign, deploy or enter unrelated E1 work.
...
| E1 Catena production implementation | **Changes required** at V14 head `69f2575421ba976271c936b1abd4b39dbe8b98fd`; fresh independent review recorded on `review/catena-wave-1-e1-corrections-v14-independent` | Read-once chapter authority, `unfetched`, tally, structural inventory, the frozen projection graph and inherited V13 cases pass. Pending same-path work still collapses to one path-keyed Promise: B does not settle while A is held and renders A's body after release, so completion/body ownership is not row/projection-authoritative and the green oracle is inverted. Package identity/P8/privacy pass, while exact-command, complete-ledger and executed-tool evidence are materially contradictory. The absent V13 review identity and missing literal fresh-clone fields are bounded conditions. Next: the smallest semantic V15 from exact V14, with owner-aware pending completion, a true A/B late oracle and application witness, bounded nested/deep-mutation proof completion, and rebuilt coherent evidence. Preserve accepted E0 and sound V2–V13 work; all broader E1 blockers remain open. No merge, re-sign, deploy or cutover. |
...
| 2026-08-21 | E1 Catena correction V14 independent review | **CHANGES REQUIRED** with `SEMANTIC CHANGES REQUIRED` and `EVIDENCE CHANGES REQUIRED`. Read-once raw chapter authority, walking `unfetched`, tally separation, structural members, projection freezing and V13 regressions pass. Request ownership ends at address selection: the global path-to-Promise cache lets active B consume held A's late same-path body, and the test requires the leak; body application is absent from the object roster/journal. Package ZIP/P8/authority/privacy pass, but mandatory exact commands are elided/prose, executed-tool classification contradicts execution, and complete/append-only ledger claims omit material history. Missing V13 review identity and exact fresh-clone lines are bounded separately. | Reviewed `69f2575421ba976271c936b1abd4b39dbe8b98fd`; comparison base `6cc85e1a1dea317a48c0bfcfd6f774201ea3a6c3`; evidence `f74f8f4d4de44e21afdbef1fc4e9589a9898e986`; ZIP `414f303954d79b966f4d7f0ad6814376c0014fb73f8e2b78a0d4dc2495124bb1`; review branch `review/catena-wave-1-e1-corrections-v14-independent`; next action is the smallest semantic V15 from exact V14. |
