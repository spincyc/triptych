...
| E1 Catena production implementation | Fifteenth correction candidate (V14) awaiting fresh independent review; the disposition it answers is **CHANGES REQUIRED** at exact V13 head `6cc85e1a1`, recorded by an independent review that has **no published branch or commit** — `origin` carries no `review/catena-wave-1-e1-corrections-v13-independent`, the branch exists only in a local reviewer checkout standing at the reviewed head with no review commit on it, and this lane records that gap rather than inventing a SHA | That review passed one-chapter-one-projection held in a `WeakMap`, `requestSnapshot` for one invocation, the inherited prefix and inherited refusal closures, the carried path read once, the fail-closed contamination policy as a design, the six planted scenarios as useful, and the V13 package protocol — the immutable sealed package, the read-only P8 with equal pre and post digests, one-way post-P8 authority, the complete attempt ledger with unique ordinals, executed-tool byte evidence, privacy sanitization, the handoff inventory and the outer scan. It found six things: raw `file.unfetched` was still reread after projection and could replace an accepted chapter with a manufactured unavailable state, losing rows, refusal and tally and printing the payload's own string to the reader; "same instance" was proved by comparing ids rather than with `===` at the consumer boundary; the tally was folded into the rows instead of recorded as a consumer of its own; request ownership was reconstructed from path strings, so a same-path sibling was the same owner; nested `sources["1"]` as an own getter was invoked for voices and editions and declined for fragment provenance, out of one projection; and the projection was frozen while its row, lead and blocked entries were not. Its exact next action: remove the post-projection raw reread and prove it with a walking-`unfetched` production case beside a steady control; prove actual projection identity with `===` observed at the production boundary; record the tally separately and publish a covered-consumer roster; bind requests to the actual projected row object across same-path, multi-projection and late completion; make the member-list case structural over add, remove, reorder, phantom and tally with controls; normalize nested source entries and their fields exactly once so no consumer invokes an accessor; freeze the graph as deep as it is trusted and assert it directly; audit the sources for post-projection raw access; and correct the evidence wording so pins and oracles are not presented as semantic closures. V14 is that correction alone; every other enumerated blocker remains open for the authorized continuation. Preserve accepted E0 and every sound V2–V13 correction. The exact Catena data seam is authorized but transfers no broader ownership; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
