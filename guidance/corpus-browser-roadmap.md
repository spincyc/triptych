...
with `14` errors across **all twenty-four** V6 classes.
...
| E1 Catena production implementation | Seventh correction candidate (V6) awaiting fresh independent review; the disposition it answers is **CHANGES REQUIRED** at V5 head `19982ab43` (independent review `fa5b2f601`, 2026-08-14) | That review preserved the exact voice-key projection, the neutral refusal umbrella, the unchanged data/CSS/HTML and the formal ceilings, and found one distinction stated fourteen ways: a shape was validated where a membership was required, and text was accepted where an identity was required. V6 answers it by validating every semantic root, collection member, textual identity and bootstrap payload into an accepted typed representation before use, reading findings as a set rather than a sequence, omitting unsupported claims instead of guessing them, and replacing the runtime oracles the review proved lossy with assertions on the production sinks. Preserve accepted E0 and every sound V2/V3/V4/V4.1/V5 correction. The exact Catena data seam is authorized but transfers no broader ownership; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
