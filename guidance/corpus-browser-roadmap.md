...
| E1 Catena production implementation | Tenth correction candidate (V9) awaiting fresh independent review; the disposition it answers is **CHANGES REQUIRED** at V8 head `7e4df42a2` (independent review `611b5eed8`, 2026-08-15) | That review passed every isolated namespace case and the preserved boundaries, and found the composed rule open — a prefix the file stated and the page refused collapsed into the same `''` as genuine absence, so a valid same-stem carried `structure/catena/text/` file was fetched and its planted body rendered — beside a package whose machine inventory understated the final sealed bytes. Its exact next action: preserve the absent / valid / present-invalid distinction, permit a carried path only on genuine absence, prove it cold, prewarmed and genuinely late at the production sinks with complete terminal assertions, and rebuild a truthful immutable package. V9 is that correction alone; the review's other enumerated blockers remain open for the authorized continuation. Preserve accepted E0 and every sound V2–V8 correction. The exact Catena data seam is authorized but transfers no broader ownership; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
