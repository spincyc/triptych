...
| E1 Catena production implementation | Thirteenth correction candidate (V12) awaiting fresh independent review; the disposition it answers is **CHANGES REQUIRED** at exact V11 head `0255b8499` (independent review `22b9bdad5` on `review/catena-wave-1-e1-corrections-v11-independent`, 2026-08-17) | That review passed the fail-closed prototype policy as a design, ordinary accessor non-invocation, own-property projection, the refused and unestablished wording, the cold, present-valid and prewarmed controls, the 36-field late vector and its timing, packaged request ownership, the read-only non-executing P8 with its identity binding and rehash, the ten handoff contents, screenshots, privacy, ZIP metadata, the byte scan, ownership boundaries and the existing ceilings — and found the policy applied to some of the ways in and not to others. An inherited valid spine `text_prefix` was read as genuine absence and reopened the carried fallback; `Object.prototype.text_refused = true` did not contaminate an own-valid claim, because `ownContract` checked three names; and the carried `text_path` descriptor was read twice, so a drifting descriptor validated one address and projected another. Beside them the package failed authority coherence: its ledger called the superseded attempt authoritative and the shipped attempt unresolved, two browser logs were empty, and repeated assemblies reused log names. Its exact next action: make an inherited spine prefix and an inherited refusal marker fail closed, snapshot the carried path once, add planted request/cache/body/ownership regressions, and return the corrected head with a fresh package carrying one terminal authoritative attempt, unique attempt-root logs, and mechanical rejection of cross-record authority disagreement. V12 is that correction alone; every other enumerated blocker remains open for the authorized continuation. Preserve accepted E0 and every sound V2–V11 correction. The exact Catena data seam is authorized but transfers no broader ownership; release, common-gate, B0/shared-shell, Day-reader guard, real-device/AT, protected Liturgy and PDF prerequisites remain separate. E1 stays off main. No merge, re-sign, deploy or cutover. |
