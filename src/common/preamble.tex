...
% contribution renders the timestamp in a compact provenance block.  Complete
% contribution and runtime history remains in the tracked metadata file rather
% than being reproduced as a reader-facing process ledger.  A mechanically
...
\newcommand{\AIDisplayDocumentRevisionTimestamp}{%
  ...
  \fi}
\makeatletter
\let\AI@lastmodel\relax
\let\AI@lastqualifiers\relax
\newif\ifAI@rendermodel
\newcommand{\AIModelContribution}[3]{%
  \begingroup
  \edef\AI@currentmodel{\detokenize{#1}}%
  \edef\AI@currentqualifiers{\detokenize{#2}}%
  \AI@rendermodeltrue
  \ifx\AI@currentmodel\AI@lastmodel
    \ifx\AI@currentqualifiers\AI@lastqualifiers
      \AI@rendermodelfalse
    \fi
  \fi
  \global\let\AI@lastmodel\AI@currentmodel
  \global\let\AI@lastqualifiers\AI@currentqualifiers
  \par\noindent
  \begin{minipage}{\linewidth}
  \exhyphenpenalty=10000
  \AIDisplayDocumentRevisionTimestamp
  \ifAI@rendermodel
    \textbf{Model:} \texttt{#1}; \texttt{#2}\par
  \fi
  \end{minipage}\par
  \endgroup}
\makeatother
