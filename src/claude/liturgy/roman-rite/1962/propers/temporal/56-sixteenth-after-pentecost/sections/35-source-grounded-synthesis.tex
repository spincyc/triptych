...
Taken whole, the formulary shows what no element of it says alone: its three
strands reached this Sunday on different schedules, only the Introit's
centonisation is its own alone, the book's general rules and not this day settle
what is printed around it, and the published English reaches its doctrine at one
element. Dom Lucien Fromage's chapter for this Sunday, in his
continuation of Guéranger's \work{The Liturgical Year}, is devotional
...
\subsection*{C2. Only the Introit's centonisation is this Mass's alone, and the Alleluia has the weakest claim of the five to its place}
...
The calendar registry named in the References carries all of those
appointments but the votive Mass; Schuster asserts three of them independently
and the registry adds the others, and every one of the negatives above is its
count.
...
\textbf{Nothing printed around the ten appointed texts is proper to this
Sunday}, and the one element of the frame that did change here is the one whose
removal a decree outside the Missal records.

% C4 is the last subsection before the section's \clearpage, so whatever it
% cannot fit on the page it starts from stands alone on the section's final
% sheet. Set at its own height, the guard keeps the heading and its three
% paragraphs together on that sheet instead of leaving the closing paragraph
% on it by itself; the section still occupies the same four pages.
\Needspace{27\baselineskip}
\subsection*{C4. The published English reaches this Mass's doctrine at one element, and stops short at the two readings its strongest arguments turn on}
...
the two Latin readings on which the strongest cross-element arguments of this
formulary turn --- so what those arguments join stands only in the Latin, at the
element, and no English of the appointed words shows it to a reader at all.
\textbf{At the Gradual the two comings reach English even though the word does
...
rendering the English \work{Liturgical Year} supplies}: Shepherd's translation
prints an English of the Collect beside Fromage's exposition of it and reads the
Secret, the Communion and the
