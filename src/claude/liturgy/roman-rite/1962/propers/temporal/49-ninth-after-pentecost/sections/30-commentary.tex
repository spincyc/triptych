...
is blended, and the typical edition governs. This is a presentation change
only: every published form was collated exactly as before, and every
divergence found is published here.
...
Ten of them were verified directly and are textual observations, not
inferences. They are gathered here once, and the commentary below returns to
each where it bears on an argument. This is the point at which the Latin has
to be on the page: an argument about wording cannot be conducted in
translation.
...
it. The divergences are verified textual observations; the derivation is not
established, because no \work{Psalterium Romanum} witness was collated for
it.
...
Introit's psalm verse without that caveat, their exegesis becomes a comment on
...
petitioners the recipients. The 1861 English is quoted because it is the
registered public-domain witness for this formulary, and at this point it will
not bear the weight of the argument. The Latin carries the argument, and is
...
Two features of the Latin are worth holding, and neither survives into any
English. \latin{Páteant} is not ``open thine ears'' but ``let thine ears lie
...
\textit{No commentary on the wording.} No patristic or medieval commentary
on this oration's wording was located; composed Roman collects of this type
carry no ancient commentary tradition comparable to that on the Scriptures.
The analysis above therefore rests on the printed Latin, its punctuation, and
its juxtaposition alone, and no commentator stands behind it.
...
stands in Paul's text, and both explanations remain the commentary's.
...
Gradual quotes only v.~2, and v.~2 is all of the psalm that the liturgy sings.
...
and must not be cited as Luke's text.
...
wept.'' That elision is the one grammatical fact of the Gospel that no English
can show, because the Douay's ``If thou also hadst known \dots\ the things
...
very passage, and without it the reception history of this pericope is
falsified.
...
\subsubsection*{Where Augustine and the \work{Summa} are silent}
...
and it cannot honestly be passed over in silence. The most
...
and ``the apostate nation'' (pp.~227, 230). That book is no longer the source
of any English printed in this guide --- the Douay--Rheims and the 1861
Cummiskey missal are --- but it is cited here because it shows what the
devotional reception of this formulary actually said in living memory.
...
the temple. The missal's decision to end the reading here, rather than four
words later where the plot begins, means the assembly's last impression of the
...
The Offertory is the one appointed element for which no continuous English
exists, because no continuous Latin exists either: the antiphon is built out
of four separated parts of four verses. Stitched into a seamless paragraph,
the Douay would present the seam as a translation; the four verses are given
whole, with the parts the antiphon takes marked.
...
made thing, not simply ``Psalm 18:9--12.'' One Latin detail does not survive into English at all: the missal
...
is a documented reuse of the clause and not a verbatim repetition. This is the clearest case in the formulary where the Latin must be on
the page, because what is being reported is that four particular Latin words
