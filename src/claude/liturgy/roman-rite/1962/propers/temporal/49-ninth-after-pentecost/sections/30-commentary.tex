...
1962 Latin'' is not a single artefact, and that a guide which does not name
its witness has not told the reader what it checked.
...
Two negative results belong with that table. The Gradual (Ps. 8:2) and the
...
it. This guide records the divergences as verified textual observations and
does not assert the derivation, because no \work{Psalterium Romanum} witness
was collated for it.

\subsubsection*{Four attributions this guide corrects}
...
\latin{iúdica me}, not the Introit's \latin{líbera me}. A guide that quotes
their exegesis of the Introit's psalm verse without saying so has quietly
attributed to them a comment on words they did not have.
...
Spirit. \textit{Source limit:} Theodoret was consulted in Robert Hill's modern
English translation, which is in copyright; he is therefore summarised and not
quoted, and the PG column numbers above are those printed inline in that
...
\textbf{A caution about that English before anything is built on it.} The
1861 rendering ``that they may succeed in their desires'' has a different
...
``there fell in one day three and twenty thousand'' (v.~8) & Numbers 25: Baal-Peor & Numbers 25:9 reads \latin{viginti quattuor milia} --- four and twenty thousand --- and so do the Hebrew and the Greek. No witness to Numbers gives twenty-three thousand: the difference is Paul's, and it is left standing\\
...
suffer on the fourth.'' \textit{Edition note:} read in the Migne text of the
1845 printing, where the embedded column marks put the passage at
...
answers it directly and no gap has to be declared. Augustine, however, was
...
and a guide that reproduced the strain silently would be dishonest. The most
...
of four separated parts of four verses. Rather than stitch the Douay into a
seamless paragraph and present the seam as a translation, the four verses are
given whole, with the parts the antiphon takes marked.
...
than asserted.} The Clementine chapter has seventy-two verses, and so does the
...
6, 57} for a clause most current editions number 6:56. The guide keeps the
missal's number, states the other, and quotes the Douay at its own v.~57,
where the same words stand.
