...
revised edition extends only to the material that edition contributed. All ten
elements are printed, in the same order and at the same references, in three
Missals tracked in this repository as public-domain text --- the Vatican
\latin{typica} of 1604, the Pustet Ratisbon 1862 and the Venice 1570 --- read in
uncorrected optical layers classified degraded in their own source records, of
which no page image was opened here. It is not a page-image collation and
...
identified historical English witness on that footing. Three places where the
registered English does not answer the Latin are declared at their elements and
left open: the Alleluia's \latin{super omnem terram}, the Offertory's three
departed phrases, and the Communion's \latin{dédero} and \latin{sǽculi}. Of these
the Communion is a genuine negative --- no English witness examined renders either
reading distinctly.
...
ledger is not cited as evidence either way, and reconciling it belongs to the lane
that owns it.
...
the Cummiskey of 1861, the Lasance of 1945, Guéranger's 1900 volume --- renders
either distinctly.
...
four psalm elements the corpus answers with a single whole-Psalter boundary on a
modern critical profile, which states in its own words that no individual psalm
can be dated securely; the traditional attribution for the psalms is therefore
authorship and not a year, and it stands in the explanatory rows. The Epistle
