...
(() => {
  ...
  const staticIdentity = html.hasAttribute('data-didach-static-identity');
  const staticIdentitySurface = staticIdentity ? html.getAttribute('data-didach-identity') : null;
  if ((html.hasAttribute('data-didach-identity') && !staticIdentity) ||
      html.hasAttribute('data-didach-identity-pending'))
    return;
  if (staticIdentity) html.removeAttribute('data-didach-identity');
  html.setAttribute('data-didach-identity-pending', '');
  const DOMAINS = {
    home: 'Home',
  ...
  const READING_LABELS = {
    ...
    sources: 'The Source Library results and passage'
  };
  ...
  function detectSurface() {
    if (staticIdentitySurface) return staticIdentitySurface;
    ...
    if (scope?.matches('.sources-page') || document.body.matches('.sources-page') ||
        /\/sources\/$/.test(path))
      return 'sources';
    if (/\/404\.html$/.test(path)) return 'error';
  ...
  function copyLink(anchor) {
    ...
    if (anchor.hasAttribute('data-wave-contents-target')) {
      copy.setAttribute('data-wave-contents-target', anchor.dataset.waveContentsTarget);
    }
  ...
  function destinationLinks(markCurrent = false) {
    const current = html.getAttribute('data-didach-identity') === 'reader' ?
        'publications' :
        html.getAttribute('data-didach-identity');
  ...
  function policyLinks() {
    return [
      ['About', 'about.html'],
      ['Method and AI use', 'about.html'],
      ['Sources', 'sources/'],
      ['Contributing', 'contributing.html'],
      ['Licensing', 'license.html'],
      ['Third-party material', 'third-party.html'],
      ['Feedback', 'contributing.html']
    ].map(([label, route]) => {
      const anchor = element('a', '', label);
      anchor.href = localRoute(route);
      return anchor;
  ...
  function wrapFilter(surface) {
    ...
    summary.id = `didach-${surface}-filters`;
  ...
  function createDialog() {
    ...
    dialog.id = 'didach-identity-dialog';
    dialog.setAttribute('data-wave-dialog-owner', '');
    dialog.setAttribute('data-wave-dialog', '');
    dialog.setAttribute('aria-labelledby', 'didach-identity-dialog-title');
    ...
    title.id = 'didach-identity-dialog-title';
    ...
    function jumpLinks() {
      const surface = html.getAttribute('data-didach-identity');
      if (surface === 'home') {
        return [...document.querySelectorAll('[data-wave-task-destination]')];
      }
      if (surface === 'reader') {
        return readerHeadings().map((heading) => {
          const anchor = element('a', '', heading.textContent.trim());
      ...
      if (surface === 'error') {
        return [...document.querySelectorAll('.wave-error-actions a[href]')];
      }
      const definitions = {
        publications: [['Filters', 'didach-publications-filters'], ['Publication results', 'reading']],
        catena: [['Chapter and voice', 'didach-catena-filters'], ['Scripture and commentary', 'reading']],
        sources: [['Source filters', 'didach-sources-filters'], ['Source records and passage', 'reading']]
      }[surface] || [];
      return definitions.flatMap(([label, id]) => {
        if (!document.getElementById(id)) return [];
        const anchor = element('a', '', label);
        anchor.href = '#' + id;
        anchor.setAttribute('data-wave-contents-target', id);
        return [anchor];
      });
    }
    function jumpPanel() {
      ...
      node.appendChild(element(
          'p', 'wave-dialog-note',
          'Context-specific destinations for this page—not corpus search.'));
      node.appendChild(navigationList(
          jumpLinks(), 'Page destinations', 'wave-dialog-list wave-jump-list'));
  ...
  function actionButton(label, kind) {
    ...
    button.setAttribute('aria-controls', 'didach-identity-dialog');
  ...
  function enhanceShell(surface, dialog) {
    ...
    const brandLink = brand?.querySelector('a[href]');
    if (brandLink) {
      brandLink.textContent = 'didach.ai';
      brandLink.href = new URL('.', brandLink.href).href;
      brandLink.setAttribute('aria-label', 'didach.ai, home');
      if (surface === 'home')
        brandLink.setAttribute('aria-current', 'page');
      else
        brandLink.removeAttribute('aria-current');
      brand.setAttribute('data-didach-wordmark', '');
      brand.querySelector('span')?.remove();
    }
    if (nav.hasAttribute('data-didach-static-nav')) {
      const publications = [...nav.querySelectorAll('a[href]')].find(isPublicationsRoute);
      nav.replaceChildren(...(publications ? [publications] : []));
      nav.removeAttribute('data-didach-static-nav');
    }
    ...
    [...nav.querySelectorAll('a[href]')].find(
        (anchor) => /\/$/.test(routeOf(anchor)))?.remove();
    ...
    const currentDefinition = currentDomain === 'home' ? null :
    ...
    if (!domain) {
      ...
      if (!['home', 'error'].includes(surface)) domain.setAttribute('aria-current', 'page');
      const brandNode = header.querySelector('.brand');
      if (brandNode?.nextSibling)
        header.insertBefore(domain, brandNode.nextSibling);
      else
  ...
  function enhanceHome(main) {
    ...
    let homeTitle = main.querySelector('h1');
    if (!homeTitle) {
      homeTitle = element('h1', '', 'A source-first corpus for Catholic faith, worship, and law.');
      main.prepend(homeTitle);
    } else {
      setOwnText(homeTitle, 'A source-first corpus for Catholic faith, worship, and law.');
    }
    homeTitle.setAttribute('data-wave-title', '');
    let homeLede = homeTitle.nextElementSibling;
    if (!homeLede?.matches('p')) {
      homeLede = element('p', 'lede');
      homeTitle.insertAdjacentElement('afterend', homeLede);
    }
    ...
    const trust = element(
        ...
        'the identified sources and editions; citations, rights, and stated limits make that basis inspectable.');
    trust.setAttribute('data-wave-home-trust', '');
    tasks.insertAdjacentElement('afterend', trust);
  ...
  function enhanceIdentityFooter() {
    const footer = document.querySelector('.site-footer');
    if (!footer || footer.querySelector('[data-didach-identity-footer]')) return;
    const identity = element('div', 'didach-identity-footer');
    identity.setAttribute('data-didach-identity-footer', '');
    const line = element('p');
    const name = element('strong', '', 'didach.ai');
    line.append(name, document.createTextNode(' — a source-first corpus for Catholic faith, worship, and law.'));
    const policies = navigationList(policyLinks(), 'About, method, sources, and policy links', 'didach-footer-links');
    const sourceCode = element('a', '', 'Source code');
    sourceCode.href = localRoute('about.html#source-and-contact');
    policies.querySelector('ul')?.appendChild((() => {
      const item = element('li');
    ...
    identity.append(
        line,
        element('p', '', 'Independent study material; not an official publication of the Catholic Church.'),
        element('p', '', 'AI assists the work. Claims stand or fall by the identified sources and editions; ' +
            'citations, rights, and stated limits make that basis inspectable.'),
        policies,
        element('p', 'didach-transition-note', 'didach.ai is the public home of the Triptych corpus.'));
    footer.prepend(identity);
  }
  function setBrowserTitle(surface) {
    if (surface === 'home') {
      document.title = 'didach.ai — A source-first Catholic corpus';
      return;
    }
    if (surface === 'reader') {
      const title = document.querySelector('[data-wave-reader-document] h1, main.page-reader h1')
          ?.textContent.trim();
      const provider = html.dataset.waveRecordProviderLabel;
      if (title && provider) document.title = `${title} — ${provider} treatment · didach.ai`;
      return;
    }
    if (surface === 'catena') {
      const object = document.querySelector('#reading .chapter-name')?.textContent.trim();
      document.title = object ? `${object} · Catena Omnia · didach.ai` : 'Catena Omnia · didach.ai';
      return;
    }
    const titles = {
      publications: 'Publications · didach.ai',
      sources: 'The Source Library · didach.ai',
      error: 'Page not found · didach.ai'
    };
    if (titles[surface]) document.title = titles[surface];
  }
  function enhanceError(main) {
    markPrimary(main, 'error');
    const title = main.querySelector('h1');
    setOwnText(title, 'Page not found');
    title?.setAttribute('data-wave-title', '');
    let copy = title?.nextElementSibling;
    if (!copy?.matches('p')) {
      copy = element('p');
      title?.insertAdjacentElement('afterend', copy);
    }
    copy.textContent = 'This address is not part of the published corpus.';
    if (!main.querySelector('.wave-error-actions')) {
      const actions = element('p', 'wave-error-actions');
      const home = element('a', '', 'Go to didach.ai');
      home.href = localRoute('');
      const publications = element('a', '', 'Browse publications');
      publications.href = localRoute('texts/');
      actions.append(home, publications);
  ...
  function start() {
    html.removeAttribute('data-didach-identity-pending');
    const surface = detectSurface();
    if (!surface) return;
    html.setAttribute('data-didach-identity', surface);
