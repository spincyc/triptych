# Saint Jerome — research scope

## Document identity

- **Publication ID:** biographies/saint-jerome
- **Title:** *Saint Jerome: Scripture, Asceticism, and the Labor of Words*
- **Genre:** full historical and hagiographic biography under
  guidance/biographies.md
- **Subject:** Jerome (Hieronymus), son of Eusebius, presbyter, ascetic,
  biblical reviser and translator, exegete, controversialist, hagiographer,
  Father, saint, and Doctor of the Church (born 331 or c. 347; died 419 or
  420)
- **Places:** unidentified Stridon near the late Roman Dalmatian-Pannonian
  border; Rome; Trier; Aquileia; Antioch; Chalcis; Constantinople; Palestine;
  Egypt; and principally Bethlehem
- **Languages and ecclesial frame:** principally Latin Christianity, with
  Greek, Hebrew, Aramaic/Syriac, Jewish learning, the Old Latin biblical
  tradition, Roman and eastern ecclesial networks, and later Catholic
  reception where biographically material
- **Mutable-claim date:** official Catholic reception retained in the
  publication was checked through 2026-07-16. No current relic-custody,
  archaeological, shrine, patronage, or institutional claim is asserted
  without a directly checked current source.

## Reader and question

The intended reader is a serious non-specialist willing to distinguish a
subject's self-presentation, hostile counter-memory, textual history, official
reception, later legend, and project synthesis. The central question is how
Jerome's pursuit of Christ through Scripture became a material and
collaborative life of languages, manuscripts, travel, patronage, ascetic
community, teaching, and controversy—and how a Catholic hagiography can honor
that labor without making his invective or historical uncertainties exemplary.

## Governing thesis

Jerome's holiness was pursued through the dangerous abundance of words. His
classical learning, biblical philology, ascetic discipline, and ecclesial
service made him the decisive individual contributor to the later Vulgate,
but not the solitary translator of its whole Bible. The same rhetorical force
that served textual correction also wounded opponents. A truthful biography
therefore receives his love of Christ in Scripture while restoring Paula,
Eustochium, Jewish teachers, scribes, patrons, correspondents, and critics to
the history of the work.

## Included

- identity, disputed birth and death chronologies, unresolved Stridon, Roman
  education, baptism, travel, and ascetic turn;
- Antioch, Chalcis, the Ciceronian dream, Hebrew/Syriac study, ordination, and
  the Antiochene schism;
- Constantinople, Gregory of Nazianzus, and formative Greek Christian learning;
- service to Damasus, the Roman women's scriptural network, controversy, and
  departure for the East;
- Paula's and Eustochium's differentiated intellectual, financial, ascetic,
  and institutional agency;
- Bethlehem as a collaborative household, ascetic complex, hospice, school,
  library, and translation workshop;
- the Gospel revision, Psalter complexities, Hebrew-based Old Testament work,
  canonical prologues, and the composite transmission history of “the
  Vulgate”;
- relationships and disputes involving Rufinus, John of Jerusalem, Augustine,
  Jovinian, Helvidius, Vigilantius, and Pelagian controversy;
- the consequences of ascetic hierarchy and abusive polemic at inspected loci;
- commentaries, letters, chronicle, and *De viris illustribus*;
- final losses, the sack of Rome as received at Bethlehem, the 416 attack,
  death, early textual memory, and bounded Catholic reception;
- dedicated review of controlled cardinal-office and frustrated-papacy claims,
  the sins dialogue, modern nationality, expanded name, and detached sayings;
  uncollated visual, patronage, burial/relic, lion, and related transmission
  material is identified only as future research.

## Excluded or bounded

- no complete textual history or stemma of every Vulgate book or Psalter;
- no new critical edition, translation, manuscript attribution, or linguistic
  proficiency test;
- no reconstruction of Jewish teachers' names or voices beyond the surviving
  evidence;
- no medical judgment about Blaesilla's death or causal assignment to ascetic
  practice;
- no legal verdict concerning Jerome's departure from Rome or command
  responsibility for the 416 violence;
- no harmonization of Rufinus, Jerome, Palladius, or other partisan archives;
- no use of individual episodes in the lives of Paul, Hilarion, or Malchus as
  external biography without corroboration; their inspected prologues support
  only claims about Jerome's literary program;
- no archaeological or scientific authentication of cave, tomb, or remains;
- no current-status or authentication claim from an uncollated visual,
  patronage, burial/relic, or related tradition; the early Bethlehem tomb and
  medieval lion transmission are retained only with source-local limits;
- no modern nationality inferred from an unresolved late Roman border town;
- no assumption that ecclesial titles or iconographic attributes are
  contemporary offices or objects;
- no independent judgment of sanctity or ecclesiastical approval.

## Evidence classes

- **A:** Jerome's letters, prefaces, catalogues, commentaries, polemics, and
  translations, read as purposeful literary acts rather than transparent
  transcripts.
- **N:** Augustine's reciprocal correspondence and near-contemporary external
  testimony from Sulpicius Severus and Palladius; Rufinus becomes usable at an
  exact locus only after direct inspection.
- **E:** early received chronology, textual transmission, cult, and patristic
  memory, used only where a witness is directly collated.
- **L:** later tradition at an exact inspected witness. Uncollated legend,
  iconography, relic, expanded-name, quotation, and patronage material remains
  a lead and carries no publication claim.
- **M:** Catholic liturgical, papal, conciliar, or institutional reception,
  limited to what the checked act establishes.
- **K:** identified philological or historical scholarship actually inspected
  at a reproducible locus; publisher and bibliographic records alone do not
  carry arguments.
- **S:** project synthesis explicitly supported by the preceding classes.

## Principal corpus used at the cited loci

- Jerome, Letters 3, 5, 7, 14–16, 19–22, 35–39, 45, 49, 51–52, 57, 84,
  108, 125, 127, 130, and 138–139; *De viris illustribus* 135; the Gospel and
  Old Testament prefaces; *Against Rufinus* 3.22; Rufinus, *Apology against
  Jerome* 2.5–6, 32–44; *Against John of Jerusalem*;
  *Against Jovinian*; *Against Helvidius*; *Against Vigilantius*; and the
  *Dialogue against the Pelagians*; *Life of Paul* 1, *Life of Hilarion* 1,
  and *Life of Malchus* 1–2. These were consulted principally through the public-domain
  NPNF translation hosted by New Advent. The publication names exact loci and
  does not treat the English research text as a critical Latin edition.
- Augustine and Jerome, the dual-numbered exchange comprising Augustine
  Letters 28/40/71/73/82 and Jerome Letters 56/67/75/76/116, with Letter
  82/116 directly inspected and the remaining exchange checked at the
  indicated online records.
- Sulpicius Severus, *Dialogue* 1.8–9, and Palladius, *Lausiac History*
  36.6–7 and 41.2, as interested external witnesses.
- Augustine, *On the Proceedings of Pelagius* 66, together with Innocent's
  Letters 136–137 and Jerome's Letters 138–139, for the reported 416 attack, with the
  attribution and legal limits recorded in the audit.
- Benedict XVI, General Audiences of 7 and 14 November 2007, and Francis,
  *Scripturae Sacrae affectus* (30 September 2020), as official Catholic
  reception rather than neutral late-antique biography.
- H. A. G. Houghton, *The Latin New Testament* (2016), pp. 31–35, for the
  bounded Gospel revision and the non-Jerome remainder of the later Latin New
  Testament.
- Alison G. Salvesen, “Deuterocanonical and Apocryphal Books” (2021),
  pp. 384–402, for Jerome's prologues within the larger Greek and Latin canon
  history.
- Aline Canellis's 2016 teaching chronology only for the explicitly qualified
  c. 347, 419/420, and late-418/early-419 chronological alternatives.

The exact CSEL and CCL references supplied in the Francis footnotes were
recorded where relevant, but the Latin at those loci was not comprehensively
collated. NPNF section headings and editorial dates are research aids, not
ancient testimony.

## Located leads not used as evidentiary support

- Prosper's exact chronicle entry used in older birth chronologies;
- Rufinus's remaining *Apology against Jerome* dossier outside the inspected
  book-2 loci and the complete reciprocal Origenist sequence;
- the primary dossier and legal history of the 416 Bethlehem violence;
- external controls for individual episodes in the now-inspected lives of
  Paul, Hilarion, and Malchus;
- manuscripts and critical editions needed to refine individual biblical,
  Psalter, commentary, and letter attributions;
- the precise transfer mechanism between the controlled Gerasimus,
  *Plerosque nimirum*, and *Golden Legend* stages;
- primary burial, relic-translation, shrine, cave, iconographic, patronage,
  and expanded-name witnesses;
- books by J. N. D. Kelly, Stefan Rebenich, Megan Hale Williams, Andrew Cain,
  and the contributors to Cain and Josef Lössl's *Jerome of Stridon*, for
  which only publisher or bibliographic records were controlled.

These may guide later specialist review. Their unseen arguments do not support
contested claims in the publication and are not represented as read.

## Material uncertainties

- whether Jerome was born in the early 330s or around 347;
- the exact location of Stridon and the limits of modern territorial claims;
- whether “Sophronius” belongs to Jerome's name and the transmission history of
  the expansion;
- dates of baptism, western travel, Chalcis withdrawal, ordination, and some
  writings;
- the degree and development of Hebrew and Syriac competence;
- independent voices and precise authorship roles of women collaborators,
  Jewish teachers, scribes, and assistants;
- exact procedure behind departure from Rome;
- the competing parties' motives and complete positions in Origenist,
  ascetic, and Pelagian controversies;
- exact responsibility, reported casualty details, proceedings, and aftermath
  of the 416 attack;
- Eustochium's death date and Jerome's death year;
- burial, relic, lion, cave, visual-attribute, and patronage transmission
  histories.

## Rights boundary

Project-created prose and organization are intended for CC BY 4.0. Ancient
Latin and the cited nineteenth- and early-twentieth-century NPNF and Palladius
translations are public-domain research materials. The only direct translated
ancient phrase retained in the narrative—“a follower of Cicero and not of
Christ”—is identified as the NPNF rendering of Letter 22.30. Other ancient and
Scriptural content is paraphrased or named by locus; no extended Scripture
quotation or translated prayer is incorporated. Modern scholarship, critical
apparatus, publisher material, New Advent's site presentation, and Vatican
English retain their own rights and are linked, cited, or paraphrased within
those limits. No copyrighted corpus, scan, or substantial modern quotation is
vendored.

## Review completed

- universal and biography-profile guidance applied;
- provider, collection, genre, source families, language setting, official
  reception, and mutable-claim boundary identified;
- major consequential claims mapped to exact loci or explicitly demoted;
- the Gospel-only New Testament revision, composite Vulgate, collaborators,
  partisan counter-witnesses, polemical harms, and hard chronology disputes
  made explicit;
- historical life, official reception, and hagiography about Jerome kept
  distinct; the three attributed ascetic lives were checked as literary works
  and are not treated as transparent event records;
- internal source, chronology, and tradition records drafted;
- translation and rights bases recorded for every direct ancient quotation;
  and
- earlier source-gap production verification completed on 17 July 2026 after the
  source-gap revision: two settled `pdflatex` passes produced a 22-page PDF;
  the final log has no fatal,
  undefined-reference, overflow, underfull, or other layout warning; PDF title,
  subject, extracted text, structure, and fonts were checked; all fonts are
  embedded and subset; every page was visually inspected at ordinary print
  size; and the installed PDF was verified byte-identical to the reviewed
  build, SHA-256 `c192e632e8eb575fb176d3989cacfa2c603447423608ef84d9604ff88d4848e4`.

## Outstanding review

- independent patristic, late-antique historical, theological, and textual
  review;
- specialist Hebrew, Greek, Aramaic/Syriac, Jewish-Christian, gender-history,
  canon-history, and manuscript review;
- complete critical-edition collation of principal letters, prefaces,
  polemics, commentaries, chronicle entries, and external witnesses;
- complete reciprocal collation of the Rufinus and 416 legal dossiers, and the
  publisher-only modern bibliography before any carries a stronger claim;
- external corroboration for individual episodes in the three ascetic lives;
- archaeological, art-historical, cult, shrine, relic, and patronage review;
- independent rights review; and
- ecclesiastical approval.

The PDF snapshot installed before the 2026-07-19 final-typography revision had
distribution clearance in the release manifest and its dated rights record.
That historical snapshot-specific operational decision did not complete any
independent rights or specialist review and was not ecclesiastical approval.
At that review point, fresh exact-snapshot distribution authorization for
those installed bytes remained pending.

The completed production checks are internal and do not imply independent
historical, theological, philological, rights, or ecclesiastical review.

Reader-order revision and production review (2026-07-17): scope, chronology conventions, evidence key, orientation, and detailed chronology now form one terminal `Scope, Chronology, and Evidence` appendix before references. Two settled `pdflatex` passes produced a 22-page PDF; the final log is warning-free, and metadata, PDF structure, embedded and subset fonts, and extracted text passed inspection. Every rendered page was visually reviewed, and the installed PDF was byte-identical to the reviewed build, SHA-256 `afd69caf9e6a7f4cb5c0a2b459e848e0b800950448d49e1d9d26675ca2ccb41e`. That snapshot is superseded.

Final-typography production review completed on 2026-07-19: a settled two-pass build produced the current 22-page PDF, SHA-256 `a2fecf24149c1d9bf2676d32a54473253af3fd9a365782458a0e25d220572b3d`. The warning-free final log, `qpdf`, PDF title and page metadata, embedded/subsetted/Unicode-mapped fonts, and nonempty text extraction passed. Every rendered page was visually reviewed, and the installed PDF is byte-identical to the reviewed build. These internal production checks do not imply independent historical, theological, philological, rights, or ecclesiastical review.

## Compact-rights-colophon production review — 2026-07-19

The settled 21-page PDF, SHA-256 `70dadb81abfaf4127e0f8be6e218ac76ff7308481db3a26caebf45a99b63a8ad`, is the exact repository-reviewed build snapshot. The final log was warning-free; generation metadata, `qpdf` structure, PDF title/page metadata, embedded and subsetted fonts with Unicode maps, and nonempty text extraction passed their gates. Repository production review inspected every physical page; the compact final-page reuse-and-rights colophon is readable, unclipped, non-overlapping, remains with terminal content, and creates no rights-only spill page. The installed PDF was byte-identical to this reviewed build. This reviewed build superseded every earlier production snapshot and received release-specific distribution clearance under the 20 July 2026 dated rights supplement. That operational decision supplied no independent review or ecclesiastical approval, and its exact-byte clearance does not attach to the revision below.

## Corpus-cleanup production review — 2026-07-21

## Staleness review — 2026-07-26

Both candidates found paired research confirms the composite-Vulgate boundary.
**No material change.** See
`src/sources/inventories/augustine-jerome-staleness-review-2026-07-26.md`.

The source and audit records were revised in the corpus-wide omission audit. A settled two-pass build produced this 21-page PDF with SHA-256 `8500ff6fa2ac9bf12d4b724a24e4a028e87d3b72f87bc5354418d766f1377a5e`. The final log contained no fatal error, undefined reference, overfull or underfull box, or unresolved rerun or layout warning. Generation metadata, `qpdf` structure, Letter page size, embedded and subsetted fonts with Unicode maps, and nonempty text extraction passed. Review rasters were generated through `scripts/pdf-review`; every rendered page was visually inspected for clipping, overlap, accidental blanks, bad breaks, footer and metadata placement, and legibility. The reviewed build was installed at its mirrored `doc/` path and verified byte-identical.

This is internal production review only; it grants no independent editorial, specialist, rights, theological, or ecclesiastical approval. The exact current installed PDF is release-cleared by the 21 July 2026 exact-current-snapshot exception; the exception supplies none of the missing review or approval just named and authorizes no changed bytes.
