...
\markboth{How to use this volume}{How to use this volume}

Volume II supplies the syntax that a reader must control before the long sentences of the
\Missal{} become manageable.  It assumes that the learner can already decline regular nouns
and adjectives, conjugate the indicative system, and use the ordinary functions of the six
cases.  Every translation below is made for this course.  It deliberately stays close enough
to the Latin to expose the grammar; it is not a quotation from a published English missal.

The examples are drawn from the 1962 Ordinary and Propers or from the Clementine Vulgate,
whose diction continually enters the Missal.  A label such as ``Canon'' or ``John 3:16''
identifies the source.  Punctuation and capitalization may differ among editions without
changing the analysis.

The Latin follows the 1962 Missal's consonantal \Latin{I} and ligature \Latin{æ}:
\Latin{Iesus, eius, iustitia; cælum, quæ, præclarus}.  Modern \Latin{J/ae} spellings are the
same words and forms, but the learner should become comfortable reading the source typography.

\section*{The working method}

\StudyLoop

For every finite verb, write person, number, tense, voice, and mood.  For every non-finite
verb, name its relative time and voice and identify the word on which it depends.  For every
subordinate clause, box its conjunction or relative word, bracket the clause, and write its
function in the margin.  A translation counts only after those decisions are visible.

A \Term{finite verb} carries person and number and can anchor a clause. A
\Term{non-finite verb}---an infinitive, participle, gerund, gerundive, or
supine---lacks person and normally depends on another word. A \Term{subordinate
clause} has its own subject--verb structure but functions within a larger
clause. Its \Term{governing word} is the word on which that function depends.
These definitions will be restated when each construction is introduced.

The gates are intentionally measurable.  Use a fresh worksheet variant, without notes or a
facing translation.  A form error, an unexplained mood, or a translation that reverses agency
is a substantive error.  Do not average away a weak construction: correct it, state its rule,
and retest it.
...
\MasteryGate{from memory you reproduce at least 28 of the 30 relative-pronoun cells correctly,
parse every pronoun in an unseen six-sentence set with at least 90\% accuracy, and compose four
relative clauses with correct agreement and internal case.}
...
\MasteryGate{you form twelve prompted comparative/superlative pairs with no more than one
error, translate both standards of comparison correctly in eight of eight sentences, and
write the cardinal and ordinal forms one through ten at 90\% accuracy.}
...
\MasteryGate{you form eighteen prompted infinitives at 90\% accuracy, identify subject and
relative time in ten unseen infinitive constructions with no more than one error, and convert
three direct statements of different times into accurate indirect statement.}
...
\MasteryGate{you form and decline fifteen prompted participles at 90\% accuracy, identify the
controller and relative time of every participle in an unseen eight-line passage, and convert
four finite clauses to or from ablative absolutes without changing agency or chronology.}
...
\MasteryGate{you produce 44 of 48 prompted subjunctive forms correctly in twelve minutes,
classify primary/historic and same/prior time correctly in ten of ten clauses, and explain the
use as well as the tense of every subjunctive in an unseen six-line passage.}
...
\MasteryGate{you classify twelve mixed clauses with at least 11 correct and a stated reason,
choose the correct negative marker in eight of eight items, and compose one positive and one
negative example of each clause family with correct sequence.}
...
Copy a collect from the Missal.  Box the address, underline the relative ground once, the
request twice, and bracket its substantive clause.  Rewrite the sentence in prose order
...
\MasteryGate{in ten mixed examples you identify the noun slot and clause family with at least
90\% accuracy, analyze one unseen collect from address through petition, and write all five
controlled models with correct marker, mood, and sequence.}
...
\MasteryGate{you choose and justify mood in twelve conjunction clauses with at least 11
correct, translate four values of \Latin{cum} without flattening them all to ``when,'' and
compose one accurate example of each major relation.}
...
\MasteryGate{you classify ten unseen conditions and parse every verb with no more than one
error, transform one lexical pair through all five patterns without consulting the table, and
translate two contrary-to-fact conditions with the correct English time.}
...
\MasteryGate{you choose and defend mood in ten contrasting relative clauses with at least nine
correct, parse the relative word's case in every item, and compose four clause types whose
English translations preserve the intended difference.}
...
\MasteryGate{you form fifteen direct commands/prohibitions with no more than one error,
identify the time and reality of six wishes, and translate four fear clauses without reversing
the feared event.}
...
\MasteryGate{you identify gerund, gerundive, future participle, and supine in twelve mixed
forms with at least 11 correct, convert four gerund phrases with objects to exact gerundive
agreement, and compose personal and impersonal passive periphrastics with the correct dative.}
...
\MasteryGate{you label and translate twenty mixed adverbial phrases with at least 18 correct,
produce all six exceptional place forms from memory, and compose the ten contrasts without
confusing agent, means, location, or direction.}
...
\textbf{Analysis.} The triple nominative adjectives form a verbless nominal
acclamation. One may supply \Latin{[est]} in an analytic translation, but the
Latin does not require that as its sole parse. They are predicative or
acclamatory adjectives focused on \Latin{Dominus Deus}, not predicate
nominatives. In the second, \Latin{pleni} agrees with the
compound subject \Latin{cæli et terra}; ablative \Latin{gloria tua} expresses fullness in
ecclesiastical usage.

\textbf{Translation.}  ``Holy, holy, holy is the Lord God of hosts.  Heaven and earth are full
of your glory.''
\end{workedexample}
...
\MasteryGate{in twenty minutes you mark every finite verb and correctly attach at least 90\%
of the words in one unseen eight-line Missal period, produce a complete prose-order Latin
version without additions or omissions, and defend each supplied ellipsis or claimed
attraction by a specific ending or construction.}

\Needspace{18\baselineskip}
\section*{Volume II cumulative gate}
\phantomsection
\addcontentsline{toc}{section}{Volume II cumulative gate}
\markboth{Volume II cumulative gate}{Volume II cumulative gate}

Choose an unseen collect, an unseen lesson paragraph of six to ten lines, and one Canon period
not used in a worked example.  Without a facing translation:

\begin{enumerate}
  \item copy each text accurately by hand;
  \item parse every verb and verbal noun, every pronoun, and every comparative or numeral;
  \item bracket and name every subordinate clause;
  \item mark the function of each ablative and all expressions of place and time;
  \item write prose-order Latin, a close translation, and then idiomatic English;
  \item compose a four-sentence prayer using one relative, one participle or absolute, one
        substantive request, and one other subjunctive construction from this volume.
\end{enumerate}

Pass only with at least 90\% correct morphology, 85\% correct syntax labels, no reversal of
subject/object, agency, negation, time, or conditional reality, and a composition free of
case-government and sequence errors.  If one construction family falls below 80\%, complete a
new pair of its parallel worksheet variants and retake that portion with a fresh passage.
\end{courseunit}
