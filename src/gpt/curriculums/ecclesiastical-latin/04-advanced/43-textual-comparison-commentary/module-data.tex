...
\newcommand{\ModuleSessionOne}{Prepare the diplomatic transcription and minimal apparatus required by Worksheet IV.18.}
\newcommand{\ModuleSessionTwo}{Select difficult forms from that same text, complete Worksheet IV.19, and audit each six-line note against the witnesses.}
\newcommand{\ModulePrintedPractice}{Worksheets IV.18 (minimal apparatus, Variant B) and IV.19 (six-line commentary, Variant C), with terminal keys}
...
\newcommand{\ModuleReferenceFocus}{\begin{itemize}
  \item diplomatic transcription, normalization, base witness, and siglum;
  \item orthographic, punctuation, morphological, lexical, omission, and substantive difference;
  \item lemma, function, clause relation, contextual sense, parallel, and alternative; and
  \item rubric as instructional register rather than a special morphology.
\end{itemize}}
\newcommand{\ModuleContrast}{A normalized line may aid reading but may never replace or silently alter the diplomatic base. Comparison itself does not create a hybrid; silently importing preferred readings into the base does.}
...
\newcommand{\ModuleScopeNote}{The selected core is collation plus commentary. Variant classification is supplied in enrichment, while the full rubrical worksheet remains an explicit extension.}
