# Twelfth Sunday in Ordinary Time, Year A — Research Scope

**Formula:** `PC-S36-A`
**Occurrence:** 2026-06-21
**Last updated:** 2026-07-19
**Registry controls:** `guidance/liturgy/postconciliar-propers-registry.md`; [edition formula dispositions](../../../registry/formula-dispositions.md); [2026 occurrence record](../../../registry/occurrences-2026.md)

Formula identity is controlled by [the instance manifest](../instance/manifest.md), element boundaries by [the leaf audit](../propers/verified.md), and reusable Missal evidence by [the Week XII owner](../../shared/ordinary-time/weeks/12/propers/verified.md).

## Texts and contexts studied

- Jeremiah 20:7–18, retaining both the appointed deliverance confession and the surrounding prophet’s anguish.
- Psalms 28, 69, and 145 in full for the Entrance, Responsorial, and first Communion assignments.
- Matthew 10:16–39, locating 10:26–33 within the missionary discourse’s persecution, witness, and allegiance sayings.
- John 10:1–18 and 15:18–27 for the second Communion branch and acclamation.
- Romans 5:12–21, preserving the exact 5:12–15 boundary and the unequal abundance of trespass and gift.
- The Week XII orations in the complete Latin identities and functions recorded by the owner; protected U.S. English wording was not reconstructed.

## Historical orientation

- **Psalms:** Psalm 28’s inherited title names David; Psalm 69’s title assigns the lament to Davidic tradition; Psalm 145 is an acrostic praise psalm bearing a Davidic title. These headings shape reception but do not securely fix autograph place or date.
- **Jeremiah:** the book locates Jeremiah’s ministry in Judah and Jerusalem from the late seventh into the early sixth century BC. Chapter 20 follows his beating and confinement by Pashhur. The book’s final literary formation reflects later transmission and editing; exact stages remain debated.
- **Matthew:** ancient tradition names the Apostle Matthew and a Palestinian Jewish-Christian audience; the modern majority dates the canonical Greek Gospel about AD 80–90, often proposing Antioch without certainty. Jesus’ discourse belongs to the Galilean ministry about AD 28–29, with no exact speaking site preserved.
- **John:** tradition associates the Gospel with the Apostle John and Ephesus late in the first century; modern composition judgments vary. John 15 locates the acclamation’s source in Jesus’ final-supper discourse in Jerusalem about AD 29–30; John 10 belongs to the Jerusalem ministry.
- **Romans:** Paul writes from Corinth about AD 56–58 to Roman house churches. Romans 5 develops Adam–Christ typology after the proclamation of justification and hope.

## Reception used

### Direct reception

- St. John Chrysostom, *Homily 34 on Matthew*, directly treats Matthew 10:26–33. He reads the sparrows and numbered hairs as assurance that nothing is hidden from divine knowledge or outside providential care, while expressly refusing to infer that God positively causes every fall: <https://www.newadvent.org/fathers/200134.htm>.
- St. John Chrysostom, *Homily 10 on Romans*, directly treats Romans 5:12–15. Adam and Christ are not equal measures: the abundance of grace and gift exceeds the trespass and its consequences: <https://www.newadvent.org/fathers/210210.htm>.
- St. Augustine, *Exposition on Psalm 69*, directly receives the afflicted psalm in relation to Christ and his body: <https://www.newadvent.org/fathers/1801069.htm>. This is reception of the psalm, not proof that Augustine knew Lectionary 94.
- St. Jerome, *Commentary on Jeremiah*, book IV, directly treats Jeremiah 20:9–13 in Latin, including the burning word, betrayal by intimates, the Lord as strong warrior, the prophet’s impatience, and praise assigned to grace: <https://la.wikisource.org/wiki/Commentaria_in_Ieremiam_%28Hieronymus%29>.
- St. Augustine, *Tractate 92 on John*, directly treats John 15:26–27: <https://www.newadvent.org/fathers/1701092.htm>; *Tractates 46–47 on John* directly span John 10:11–15: <https://www.newadvent.org/fathers/1701046.htm> and <https://www.newadvent.org/fathers/1701047.htm>.
- St. Thomas Aquinas, *Commentary on Romans* V, lect. 3, directly treats the opening Adam–Christ comparison: <https://aquinas.cc/en/la/~Rom.C5.L3>.

### Doctrinal and liturgical illumination

- *Catechism of the Catholic Church* 302–314 illuminates creation’s journey under providence, divine action through secondary causes, and the mystery of evil. It prevents a simplistic inference that every injury is positively willed by God.
- *Catechism* 402–412 and the Council of Trent, Session V, illuminate Romans 5’s Adam–Christ framework while keeping the apostolic text primary.
- GIRM 46–90 and 352–367 and the *General Introduction to the Lectionary* 65–69, 78–91, and 93–110 govern liturgical function and relationship labels. *Tempus per annum* rubric 5 supplies a Sunday Preface unless the selected authorized Eucharistic Prayer carries its own; GIRM 365(b) preserves that flexibility for Eucharistic Prayer II, while 365(d) makes Eucharistic Prayer IV and its invariable Preface one coupled selection.

No direct patristic commentary on the complete Week XII formulary is claimed. No direct early witness to the exact Jeremiah–Matthew pairing was retained; that relationship rests on official Lectionary structure.

## Passage-by-passage reception matrix

| Appointed scriptural unit | Direct checked reception | Illuminating reuse | Result and control |
| --- | --- | --- | --- |
| Entrance, cf. Psalm 28:8–9 | Augustine, *Exposition on Psalm 28* | — | Direct psalm reception; the modern antiphon adapts only the ending and does not appoint Augustine’s allegorical particulars. |
| Jeremiah 20:10–13 | Jerome, *Commentary on Jeremiah*, book IV, on 20:9–13 | — | Direct Latin continuous treatment. No comparable early Greek continuous exposition of the exact unit was retained. |
| Responsorial Psalm 69 | Augustine, *Exposition on Psalm 69* | — | Direct psalm reception; the psalm’s Israelite lament remains the literal horizon, and Augustine does not establish the Lectionary pairing. |
| Romans 5:12–15 | Chrysostom, *Homily 10 on Romans*; Aquinas, *Commentary on Romans* V, lect. 3; Augustine, *Against Two Letters of the Pelagians* IV.8 for 5:10–12 | — | Direct Greek, medieval Latin, and Latin-patristic reception; each work’s polemical setting remains visible and the appointed boundary at v. 15 is preserved. |
| Acclamation, John 15:26b, 27a | Augustine, *Tractate 92 on John* | — | Direct treatment of both source verses: the Spirit witnesses and enables apostolic witness. The acclamation remains a liturgical excerpt rather than a second reading. |
| Matthew 10:26–33 | Chrysostom, *Homily 34 on Matthew* | Cyprian, *Testimonies* III.4, martyrial reuse of fear and confession sayings | Direct exposition plus a bounded early Latin use; neither converts prudence into cowardice or danger-seeking into fidelity. |
| Communion A, Psalm 145:15 | Augustine, *Exposition on Psalm 145* | — | Direct psalm reception; the food verse remains an unselected antiphon branch. |
| Communion B, John 10:11, 15 | Augustine, *Tractates 46–47 on John*, spanning 10:11–15 | Cyril of Jerusalem, *Catechetical Lecture 10* §5; Gregory Nazianzen, *Oration 18* | Augustine directly treats both appointed verses, the Good Shepherd/hireling contrast, knowledge, and self-gift; Cyril and Gregory furnish Christological and pastoral reuse. The branch remains unselected. |

### Search record and negative-result boundary

The 2026-07-18 pass searched public ANF/NPNF English collections at New Advent and CCEL; Latin texts and indexes for Jerome and Augustine; Aquinas’s Latin/English commentary; and targeted public-corpus leads for Origen, Hilary, Ambrose, Gregory, Cyril, Bede/Rabanus, and later Doctors. Greek works were principally checked in identified English translations and Latin works in Latin and English where public texts permitted. Jerome supplied a strong direct Latin witness for Jeremiah 20:9–13. No continuous early Greek treatment of exactly Jeremiah 20:10–13 was retained, and no early witness was found for the later Jeremiah–Matthew pairing as such. These are bounded results from accessible indexed corpora, not assertions about all manuscripts, untranslated catenae, or subscription databases.

## Claim matrix and controls

| Claim | Evidence class | Control |
| --- | --- | --- |
| Jeremiah’s threatened witness and Matthew’s fearless witness belong together | Officially correlated | The Lectionary establishes the relation; it does not make Jeremiah’s biography a prediction of every Christian ordeal. |
| Psalm 69 voices afflicted trust | Responsorial and direct reception | Lament permits anguish; faith is not emotional invulnerability. |
| God’s knowledge of sparrows grounds courage | Gospel exegesis and direct reception | Providence does not mean God positively causes abuse, persecution, or every creaturely fall. |
| Christ’s gift exceeds Adam’s trespass | Semi-continuous exegesis and direct reception | Romans remains an independent strand; grace is not measured as a mere reversal of equal quantities. |
| Shepherd imagery can frame God’s care | Textual observation / synthesis | John 10 is an unselected Communion branch, not an enacted text in every Mass. |

## Four-sense and two-page synthesis architecture

The page-one four-sense table and the two-page `The Propers: Themes and Movement` section compress only the audited texts, relationship classes, reception, and detailed commentary. The four senses retain the passages’ historical claims, receive Christological and ecclesial fulfillment through documented canonical and doctrinal controls, state the demanded moral fidelity, and name disclosure and redemption as the final horizon; they introduce no new witness or historical-design claim.

The first synthesis page preserves the established liturgical movement, while the second preserves the theological and spiritual synthesis and adds a four-dimension matrix keyed to appointed anchors. Jeremiah and Matthew remain officially correlated, Romans remains semi-continuous, and John 10 remains an unselected Communion branch; the shared Week XII orations do not establish one Year A compositional plan. On 2026-07-17 the guide was rebuilt for two passes after the reader-facing simplification; its log, prescribed page boundaries, single historical-table closing rule, and every rendered page were checked.

## Branch, pastoral, and weekday limits

- No enacted selection is inferred for `sunday-sprinkling`, `entrance-chant-selection`, `offertory-chant-selection`, or `communion-chant-selection`; neither `communion-antiphon-psalm-145` nor `communion-antiphon-john-10` is marked selected. A resolved instance must also record either `compatible-preface-and-eucharistic-prayer`—a Sunday-in-Ordinary-Time Preface unless the selected authorized non-IV Prayer carries its own—or `eucharistic-prayer-iv-with-preface`, preserving EP IV with its inseparable Preface under GIRM 365(d).
- “Do not be afraid” does not counsel remaining in preventable danger, concealing abuse, refusing legal protection, or neglecting prudent care.
- Martyrial witness is public fidelity under grace, not self-endangerment or contempt for bodily life.
- There is no Missal Offertory antiphon.
- Week XII ferias use independent weekday Cycle II in 2026. Use of Week XII orations is a permitted selection requiring a separate weekday instance.

## Interpretive-proposal audit

On 2026-07-18 a distinct conjunction search tested the proposals across public New Advent ANF/NPNF texts, Jerome's public Latin commentary, Augustine's public corpus, Aquinas's public commentary, and indexed official Holy See material. Searches paired Jeremiah's whispering/watchers with Matthew's rooftop proclamation and numbered hairs, Psalm 69's heard poor with Matthew's sparrow and John 10's known sheep, Romans 5's abundant gift with Matthew's fear sayings, and the Psalm 145/John 10 Communion alternatives. The classifications are bounded to this corpus and access window and make no universal novelty claim.

| Proposal | Anchors and connection mechanism | Novelty classification | Bounded search / nearest analogue | Concrete fruit | Controlling limit |
| --- | --- | --- | --- | --- | --- |
| The whispering circuit is broken by rooftop speech | Jeremiah 20's hostile whispers/denunciation move through John 15's Spirit-enabled testimony to Matthew 10's public disclosure and acknowledgment. | `not located in the checked corpus` | Searches for `Jeremiah 20 whisper denounce Matthew 10 rooftops Spirit testimony` found no exact conjunction. Nearest separate witnesses are [Jerome on Jeremiah 20](https://la.wikisource.org/wiki/Commentaria_in_Ieremiam_%28Hieronymus%29), [Augustine on John 15](https://www.newadvent.org/fathers/1701092.htm), and [Chrysostom on Matthew 10](https://www.newadvent.org/fathers/200134.htm). | Gives public witness a standard of received truth, verifiability, and nonretaliatory accountability. | Spirit-aid does not certify impulses or abolish preparation, confidentiality, source protection, safeguarding, or prudent timing. |
| Two kinds of watching assign opposite meanings to visibility | Jeremiah's friends watch to exploit a fall; Matthew's Father knows the sparrow's fall and numbered hairs, opposing predatory surveillance to worth-conferring attention. | `not located in the checked corpus` | Exact search `Jeremiah 20 watch stumble Matthew 10 hairs numbered sparrows` located the passages only in separate commentary: [Jerome](https://la.wikisource.org/wiki/Commentaria_in_Ieremiam_%28Hieronymus%29) and [Chrysostom](https://www.newadvent.org/fathers/200134.htm). No checked patristic or official result developed the surveillance/providence contrast. | Audits whether institutional data and personal attention serve vulnerable persons or organizational leverage. | Divine knowledge is not human data collection and does not positively cause every fall; privacy, consent, and protection remain. |
| The one heard as poor is valued as sparrow and known as sheep | Psalm 69's divine hearing, Matthew's sparrow-worth, and optional John 10's knowing/self-gift intensify attention into answerable relation. | `near analogue located` | Search `Psalm 69 poor heard Matthew 10 sparrow John 10 sheep known` found no three-way conjunction. [Apostolic Constitutions II](https://www.newadvent.org/fathers/07152.htm) closely joins Good-Shepherd self-gift, feeding, and accountable pastoral care; [Augustine on Psalm 69](https://www.newadvent.org/fathers/1801069.htm) and [Chrysostom on Matthew 10](https://www.newadvent.org/fathers/200134.htm) remain separate anchors. | Integrates reporting, material care, recognition, and institutional accountability around the vulnerable person. | Communion B is unresolved; no immunity is promised, persons are not infantilized, and systems cannot be replaced by sentiment. |
| A surplus of grace can interrupt inherited fear | Romans 5's unequal Adam–Christ abundance supplies the ground on which Matthew's threatened witness can refuse inherited regimes of fear. | `near analogue located` | Targeted search `Romans 5 Adam death gift Matthew 10 fear killers acknowledge` found [Chrysostom on Romans 5](https://www.newadvent.org/fathers/210210.htm) and early [martyrial reception of Matthew 10](https://www.newadvent.org/fathers/07155.htm), but no checked source joined them into an inherited-fear mechanism. | Grounds courage in received grace rather than willpower and denies fear the final word. | Romans is semi-continuous and not a clinical theory; preserve prudent flight, legal protection, danger assessment, and trauma-informed care. |
| The Communion branches distinguish provision from recognition | Communion A's food in due season and Communion B's knowing/self-giving Shepherd offer alternative material and relational grammars of care. | `near analogue located` | Searches for `Psalm 145 food due season John 10 good shepherd knows sheep` located [Augustine on Psalm 145](https://www.newadvent.org/fathers/1801145.htm), who relates receiving to giving/teaching, and [Apostolic Constitutions II](https://www.newadvent.org/fathers/07152.htm), which relates shepherding to feeding and accountability. No exact branch comparison was located. | Critiques systems without relationship and relationships without dependable provision. | Both branches are unresolved alternatives; neither is a full social program, and Eucharistic gift exceeds the analogy. |

## Rights and review

The reader document gives citations, boundaries, Latin incipits, and original paraphrase rather than full protected U.S. Lectionary or ICEL texts. No scan or bulk transcription is tracked.

The formula, official occurrence, complete online Lectionary path, Latin oration identities, and antiphon assignments are source-audited. Complete U.S. publisher-specific altar-book English-oration collation and independent theological review remain outstanding. The study claims neither ecclesiastical approval nor an exhaustive patristic, manuscript, critical, musical, or local-calendar survey.

## Structural conformance review

Civil date, governing editions, territory, and Lectionary identity remain in the terminal Liturgical Resolution appendix and tracked manifest. The compact-title revision received a settled two-pass build with a warning-free final log; generation metadata, PDF structure, embedded fonts, Unicode text, and extraction passed; all 9 pages were visually inspected; and the installed PDF is byte-identical to the reviewed build, SHA-256 `8941f04075a8cae26fba8f430907d024d35b5c4ab862f677cbeb52df99feb501`.

On 2026-07-18, the page-two coordinate revision superseded that production snapshot. It received a settled two-pass build with warning-free logs; metadata, PDF structure, embedded fonts, Unicode, and text extraction passed; all 9 pages were visually inspected; and the installed PDF is byte-identical to the reviewed build, SHA-256 `cad8d1f97d002a278c76aabff378d2e4da57412c59241738fa458f313caeb8eb`. At that review point, this snapshot remained in private review pending renewed exact-snapshot authorization.

On 2026-07-18, the final passage-by-passage production snapshot received a settled two-pass build with warning-free logs; metadata, PDF structure, embedded fonts, Unicode, and extraction passed; all 11 pages were visually inspected; and the installed PDF is byte-identical to the reviewed build, SHA-256 `070942815781348e23b5c33126de5b8f1e19fdf228cb9c8b544cd45f52981385`. At that review point, it remained in private review pending renewed exact-snapshot authorization.

## 2026-07-18 Interpretive-Discovery Production Record

The interpretive-discovery revision received a settled build with a warning-free final log. Metadata and Unicode checks, qpdf structure, embedded fonts, and text extraction passed; all 11 full-size pages were visually inspected. The explicitly installed mirror is byte-identical to the reviewed build, SHA-256 `20e664915cc3e11664d6d0fd6a67a74d2891d2bab2024c1aa023ed1c4c4fa95f`. At that review point, this exact snapshot remained in private review pending renewed exact-snapshot authorization.

## Notable-and-quotable audit

The gallery was rebuilt on 2026-07-19 around later cultural, humorous, ironic, idiomatic, and media-art uses of the appointed Gospel's wording. Straight exposition was removed. Short excerpts identify the verbal afterlife; no wrapper or modern edition is treated as wholly reusable.

| Proper phrase and locus | Later use and source locus | Relationship / wording check | Context and payoff | Rights / limits |
|---|---|---|---|---|
| “not one of them falls to the ground” (Matt 10:29, Gospel) | Shakespeare, *Hamlet* 5.2.230–238 | Hamlet says there is “special providence in the fall of a sparrow” immediately before accepting the duel. The close verbal echo is recorded as an echo; no claim is made about Shakespeare's precise biblical edition from the Folger page alone. | The scriptural bird enters tragic dramatic irony: Hamlet's confidence precedes, rather than prevents, his death. Providence is not reduced to fatalism or bodily immunity. | Eight-word lectionary identifier; ten-word Shakespeare excerpt from a public-domain play, checked against the modern Folger edition. [Folger](https://www.folger.edu/explore/shakespeares-works/Hamlet/read/), checked 2026-07-19. |
| “all the hairs of your head are counted” (Matt 10:30, Gospel) | “The Back Numbers,” *New Hampshire College Monthly* 8.3 (December 1900), p. 72, credited there to *Grey Jacket* | The eight-line joke quotes the older English form “The hairs of your head are all numbered,” then has a bald worshipper request “the back numbers.” It is an explicit scriptural joke, though the reprint does not name an author or original issue. | “Numbered” changes from divine enumeration to periodical back issues, turning reassurance into a baldness pun. | Ten words total quoted from a U.S.-public-domain periodical scan; attribution limit retained. [Scan](https://upload.wikimedia.org/wikipedia/commons/e/ed/The_New_Hampshire_College_Monthly_%28IA_newhampshirec19001901newh%29.pdf), checked 2026-07-19. |
| “the hairs of your head are counted” (Matt 10:30, Gospel) | Doublelucky Productions / Kim Albrecht, *The Hairs of Your Head Are Numbered* (2017–2018) | The unusual eight-word title closely reproduces the Gospel saying, while the project page describes sensor gloves, live-visualized audience data, measured emotions and bodies, advertising, and manipulation. This is recorded as a strong intertextual echo; no checked source establishes the naming decision or exclusive dependence on Matthew rather than Luke 12:7. | Divine intimate knowledge receives an ambivalent cultural echo in quantified bodies and corporate or artistic measurement. The guide does not equate God with surveillance. | Eight-word title needed to identify the work; project description paraphrased. [Project](https://www.kimalbrecht.com/item/hairs-of-your-head/); [artist statement](https://www.kimalbrecht.com/about/), checked 2026-07-19. |

Searches also tested “terror on every side,” “fear no one,” “proclaim on the housetops,” and Romans 5's gift/transgression language. The “shout from the rooftops” dictionary lead documents only a similar phrase, not biblical derivation, so it was rejected with the mostly sermonic, ministerial, or unrelated results rather than promoted as an afterlife.

## 2026-07-18 Notable-and-Quotable Production Record

The final 12-page build is settled and deterministic across two forced root rebuilds. Its warning-free log, generation metadata, qpdf structure, embedded Unicode fonts, text extraction, physical-page-2 boundary, and every-page full-size visual review passed. The installed PDF is byte-identical to the reviewed build, SHA-256 `1222a97fc6e5441b8c59c824522163a65ab9848da3615e43d2f46bd8b79ec041`. At that review point, this exact snapshot remained in private review pending renewed exact-current-snapshot authorization.

## Compact-rights-colophon production review — 2026-07-19

The settled 11-page PDF, SHA-256 `219f7d245b36e55c305b24caa6d282c64c80c1c1418bf8f2505ac26fb77f366c`, is the exact repository-reviewed build snapshot. Its Generation Metadata displays each model/configuration once per contiguous group while retaining every distinct Agent/runtime record. The final log was warning-free; generation metadata, `qpdf` structure, PDF title/page metadata, embedded and subsetted fonts with Unicode maps, and nonempty text extraction passed their gates. Repository production review inspected every physical page; the compact final-page reuse-and-rights colophon is readable, unclipped, non-overlapping, remains with terminal content, and creates no rights-only spill page. The installed PDF is byte-identical to this reviewed build. This reviewed build supersedes every earlier production snapshot. At that review point, because its bytes had changed, exact-snapshot distribution approval was pending.

## Corrected scriptural-afterlives production review — 2026-07-19

The final 12-page PDF, SHA-256 `db9fb8b7f163ddbf42dbedeca7024fbefcd1f966b40a924f108c6e168e449e24`, is the exact clean-build snapshot for this revision. Its rebuilt Notable and Quotable gallery contains only source- and locus-audited, non-obvious cultural, humorous, ironic, idiomatic, literary, political, institutional, visual, scientific, commercial, or comparable afterlives of wording from appointed Scripture; the audit above records verbal strength, context, semantic turn, rights, limits, and rejected leads.

Generation metadata is singular and terminal. The warning-free clean build passed metadata, `qpdf`, title and subject, embedded and subsetted Unicode fonts, extraction, the page-2-only dossier boundary, and terminal-colophon gates; every rendered page was inspected at full size with no clipping, overlap, bad split, artificial padding, sparse spill, or rights-only page. The installed PDF was byte-identical to this reviewed build. This record superseded earlier production snapshots and received release-specific distribution clearance under the 20 July 2026 dated rights supplement. That operational decision supplied no independent review or ecclesiastical approval, and its exact-byte clearance does not attach to the revision below.

## Corpus-cleanup production review — 2026-07-21

The source and audit records were revised in the corpus-wide omission audit. A settled two-pass build produced this 12-page PDF with SHA-256 `cab27a59ba770fb0c71d6d35c8dc1c2846ccdece313ecc0a76bb5fb1141f5f89`. The final log contained no fatal error, undefined reference, overfull or underfull box, or unresolved rerun or layout warning. Generation metadata, `qpdf` structure, Letter page size, embedded and subsetted fonts with Unicode maps, and nonempty text extraction passed. Review rasters were generated through `scripts/pdf-review`; every rendered page was visually inspected for clipping, overlap, accidental blanks, bad breaks, footer and metadata placement, and legibility. The reviewed build was installed at its mirrored `doc/` path and verified byte-identical.

This is internal production review only; it grants no independent editorial, specialist, rights, theological, or ecclesiastical approval. The exact current installed PDF is release-cleared by the 21 July 2026 exact-current-snapshot exception; the exception supplies none of the missing review or approval just named and authorizes no changed bytes.

## Research-staleness verdict — 2026-07-26

The current source and both candidates were compared claim by claim against
the changed GIRM and Missal inputs. No material correction was found; this
exact study edition is ready for rebaselining.
