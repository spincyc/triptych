# Third Sunday after Pentecost — Research Scope

## Textual baseline

- Formulary: *Missale Romanum*, editio typica (Vatican City: Typis Polyglottis Vaticanis, 1962), `DOMINICA TERTIA post Pentecosten`, `II classis`, printed pp. 380–382 (PDF pp. 461–463).
- Public baseline: <https://media.churchmusicassociation.org/pdf/missale62.pdf>.
- Machine-readable locating aid: Internet Archive item `MissaleRomanum1962RomanMissalColorLatin`, focused OCR lines 57,654–57,814 preserved without silent correction in `../propers/retrieved.txt`.
- Secondary image witness: the Internet Archive color item, printed pp. 374–376 (DjVu pp. 451–453).
- All ten elements, their references, rubric, and formulary boundaries were visually collated on 2026-07-10. The baseline's readings control `../propers/verified.md`; witness differences and checksums are recorded there.
- The Secret's historical English now follows Dom Gaspar Lefebvre, O.S.B., *Daily Missal with Vespers for Sundays and Feasts* (1925), printed p. 1062, visually checked on 2026-07-14 against the [edition scan](https://archive.ccwatershed.org/media/pdfs/21/02/16/05-35-23_0.pdf). The exact facing English and its abbreviated `Through our Lord.` conclusion are retained without modernization or expansion. The 1962 Latin baseline separately ends at its printed `in unitáte.` boundary; the witnesses are not harmonized.
- The 1925 English is an identified historical human translation and is public domain in the United States by publication date. It remains received third-party wording outside the project's CC BY grant; no worldwide rights status or current liturgical approval is claimed.
- No seasonal substitution or addition appears. The six distinct directly appointed biblical dossiers below exclude the composed Collect, Secret, and Postcommunion and consolidate Luke 15:10, which occurs within both the Gospel and Communion.

## Canonical inventory and historical coordinates

### Psalm 7:12 — Alleluia

- The Greek and Latin superscription presents the psalm as David's lament concerning Chus/Cush the Benjaminite. The *Catholic Encyclopedia* groups Psalm 7 among psalms traditionally associated with David's persecution under Saul, but neither Scripture nor the title gives a writing site.
- The table therefore uses `Judah / Israel; exact site unrecorded` and `c. 1010 BC` as approximate traditional metadata. David is heard before secure kingship, appealing from pursuit and accusation to the God who examines hearts and judges peoples; Israel's worshipping community is the inherited audience.
- The identity of Cush and the precise incident are unrecoverable. The NABRE treats Psalm 7 as an individual lament and the Psalms introduction cautions that no psalm can be securely dated from its title.

### Psalm 9:11–13 — Offertory

- The superscription calls Psalm 9 Davidic but gives no life episode or composition site. The table therefore uses `Israel; exact site unrecorded`, `c. 1000 BC`, and explicitly leaves David's biographical stage unspecified.
- Its thanksgiving for judgment and worldwide rule turns toward Zion, the oppressed, and the cry of the afflicted. It first trains Israel's worshippers while opening Israel's praise toward the nations.
- In the Greek and Latin tradition Psalms 9–10 are a single alphabetic poem; the Hebrew tradition transmits them as two. Modern historical author, date, and writing site remain unknown.

### Psalm 24:1–2, 16, 18 (modern Psalm 25) — Introit

- The superscription attributes the acrostic lament to David but supplies no life event or location. The table therefore gives `Israel; exact site unrecorded`, `c. 1000 BC`, and states that no responsible point within David's life can be recovered from the title.
- The prayer joins trust, guidance, remembered mercy, forgiveness, loneliness, and affliction before widening in its final verse to Israel's redemption. The inherited audience is thus Israel at prayer rather than a reconstructed private circle.
- Modern scholarship identifies its acrostic lament form but cannot securely establish author, date, or place. Vulgate/Septuagint numbering is one below the common modern number.

### Psalm 54:17, 19, 23 (modern Psalm 55) — Gradual

- The superscription calls the maskil Davidic but names no incident or site. The table uses `Israel; exact site unrecorded`, `c. 1000 BC`, and does not identify the betraying companion with a particular figure from David's biography.
- The speaker suffers public violence and a trusted companion's betrayal, prays throughout the day, and entrusts his burden to God. The song carries personal crisis into Israel's common worship and supplies language later echoed directly in 1 Peter 5:7.
- Modern author, date, city, and historical betrayer remain unknown; the city imagery does not by itself prove Jerusalem as the place of composition.

### Luke 15:1–10 — Gospel and Communion

- Ancient tradition identifies the Antiochene physician Luke, Paul's companion, as author. The proper row deliberately joins two distinct inherited strands rather than treating them as one itinerary claim: an ancient *Prefatio* says that Luke wrote in Achaia (present-day Greece), while an early chronology places the Gospel before Acts, around AD 60.
- Luke addresses Theophilus and similarly catechized Christians after participation in apostolic mission. The Gospel–Acts project locates Israel's promises, Jesus' saving work, and the Gentile mission within one salvation history.
- The modern majority dates Luke about AD 80–90, treats the precise city as unknown, and describes the author as a non-Palestinian addressing a largely Gentile, non-Palestinian audience. These judgments remain in the explanatory row rather than replacing the requested traditional metadata.
- Luke 9:51–19:27 frames the material as journeying toward Jerusalem; Luke 13:22 supplies the nearer travel notice, but Luke 15 names no town. The narrated-event row therefore says `Journey toward Jerusalem; exact site unrecorded` and does not adopt an unsupported harmonized region. Its c. AD 28–29 range identifies the traditional late-ministry horizon without pretending to recover a town.
- The immediate hearers are tax collectors and sinners drawing near, and Pharisees and scribes murmuring. Jesus defends his welcome by joining the seeker's initiative, repentance, and heaven's joy; the Communion repeats verse 10 from within the same dossier.

### 1 Peter 5:6–11 — Epistle

- Christian tradition identifies Peter as author, writing from Rome—the `Babylon` of 1 Peter 5:13—about the end of AD 63 or beginning of 64. He writes as apostle and fellow elder near the end of his ministry, before the Neronian persecution and his martyrdom, with Silvanus involved in transmission and Mark present.
- The recipients are Christian communities across Pontus, Galatia, Cappadocia, Asia, and Bithynia in Asia Minor (principally present-day Turkey), probably largely Gentile. The appointed close joins humility, God's care, resistance to the devil, worldwide Christian solidarity in suffering, and God's call to eternal glory.
- The NABRE records substantial modern disagreement: some scholars retain Petrine authorship; others propose a later author, and a mediating judgment places a Petrine disciple or circle at Rome about AD 70–90. Cultivated Greek, Septuagint use, Pauline contacts, and the social horizon drive the dispute. The traditional Roman metadata remains in the proper row, while these alternatives are disclosed in its prose.

## Passage-by-passage reception matrix

| Appointed passage | Direct exegesis checked | Illuminating reuse checked | Material result / boundary |
| --- | --- | --- | --- |
| Psalm 7:12 (Alleluia) | Augustine, *Exposition on Psalm 7* §§8–11, on judgment, received righteousness, and preserving help. | The Epistle’s adversary and vigilant resistance. | Augustine’s broader Christological reading is reception, not the psalm’s sole historical sense. |
| Psalm 9:11–13 (Offertory) | Augustine, *Exposition on Psalm 9* §§9–12, on seeking God, Sion, proclamation, blood, and the poor. | Irenaeus on just offering and ecclesial thanksgiving. | Offering does not buy mercy; the verse first commands praise and remembrance of the afflicted. |
| Psalm 24:1–2, 16, 18 (Introit) | Augustine, *Exposition on Psalm 24/25* §§1–2, 16, 18. | The solitary sinner gathered into Luke’s shared joy. | Spiritual ascent does not erase literal affliction, loneliness, or petition. |
| Psalm 54:17, 19, 23 (Gradual) | Augustine, *Exposition on Psalm 54/55* §23 and the surrounding prayer sequence. | 1 Peter 5:7’s command to cast care on God. | Same-psalm resonance is documented without pretending Augustine comments on the Roman Gradual. |
| Luke 15:1–10 (Gospel, Communion) | Cyril of Alexandria, Homily 106; Gregory the Great, *Gospel Homily* 34; Ambrose, *Exposition of Luke* VII.207–212. | Tertullian, *On Patience* 12; Gregory of Nyssa, *On Virginity* 12; Augustine, *Confessions* VIII.3–4; Anthony of Padua, *Third Sunday after Pentecost* §§2–15. | Divine pursuit, repentance, image, ecclesial search, inward search, and common joy are retained as distinct layers. |
| 1 Peter 5:6–11 (Epistle) | Anthony, *Third Sunday after Pentecost* §§2, 15, directly coordinates the passage with Luke 15; the full apostolic sequence was checked canonically. | Augustine’s Psalm 54 exposition and Gregory’s humility/mercy preaching. | No complete Greek- or Latin-Father line-by-line commentary on all six appointed verses was located in the checked public corpus; that gap is stated rather than supplied from a loose allusion. |

The widened survey searched New Advent’s Greek/Latin Fathers in public translation, direct Latin at Wikisource/Augustinus repositories, the official Basilica of St. Anthony sermon in Italian/Latin, and exact passage/work-title queries. English, Latin, Italian, and translated Greek were checked. No literal exhaustiveness is claimed for untranslated Greek/Syriac, manuscripts, the full PL/PG, or subscription databases.

## Geography and chronology controls

- Composition locations, recipient locations, and narrated-event locations are kept distinct. `Achaia` is the Roman province in present-day Greece; `Rome` is in present-day Italy; and the five recipient provinces of 1 Peter lie in Asia Minor, principally present-day Turkey. Luke's travel narrative supplies direction toward Jerusalem but not a recoverable town or region for Luke 15.
- Traditional Davidic dates are deliberately approximate. The psalm rows use `c.` dates and retain `exact site unrecorded`; none presents poetic setting or later liturgical use as a recovered writing address.
- The Gospel event date follows the traditional chronology used consistently in this project: public ministry beginning c. AD 27 and this Jerusalem-journey discourse late in that ministry, c. AD 28–29. The chronology itself is disputed, so the event row names a ministry phase as well as an approximate range.

## Source ledger and evidentiary roles

### Liturgical and biblical controls

- *Missale Romanum*, editio typica (Vatican City: Typis Polyglottis Vaticanis, 1962), printed pp. 380–382, in the [CMAA facsimile](https://media.churchmusicassociation.org/pdf/missale62.pdf), is the governing Latin witness for the heading, rank, all ten appointed elements, references, rubrics, and formulary boundaries. It was visually collated on 2026-07-10 and re-collated on 2026-07-14. It is not used as an English-translation source.
- Internet Archive item [`MissaleRomanum1962RomanMissalColorLatin`](https://archive.org/details/MissaleRomanum1962RomanMissalColorLatin), printed pp. 374–376, is a secondary image witness; its [OCR](https://archive.org/download/MissaleRomanum1962RomanMissalColorLatin/Missale-Romanum-1962-Roman-Missal-color-latin_djvu.txt) is only a locating aid. The raw OCR excerpt remains in `../propers/retrieved.txt`; it never governs a disputed reading. Retrieved and checked 2026-07-10; witness roles re-audited 2026-07-14.
- Dom Gaspar Lefebvre, O.S.B., *Daily Missal with Vespers for Sundays and Feasts* (1925), printed p. 1062, in the [edition scan](https://archive.ccwatershed.org/media/pdfs/21/02/16/05-35-23_0.pdf), governs only the received historical English of the Secret. It was visually checked on 2026-07-14. Its English is neither silently corrected from the Latin nor presented as a current approved translation.
- The *New American Bible, Revised Edition* on the USCCB site supplies modern introductions, chapter context, and critical qualifications for Psalms 7, 9, 25, and 55, Luke 15, and 1 Peter 5. It is an official Catholic Bible edition used for historical orientation and prose paraphrase, not as the source of the Missal's appointed Latin. Accessed 2026-07-10.
- New Advent's parallel Bible pages supplied Vulgate/Septuagint superscriptions and complete contextual reading for the four psalms, Luke 15, and 1 Peter 5. These public transcriptions are contextual aids rather than critical editions. The *Catholic Encyclopedia* entries on the Psalms, David, Luke, First Peter, and the chronology of Christ's life witness older traditional metadata; they are not substituted for the NABRE's modern qualifications. Consulted 2026-07-10; roles re-audited 2026-07-14.

### Reception witnesses

- Direct Gospel exegesis: St. Cyril of Alexandria, *Commentary on Luke*, Homily 106, in the R. Payne Smith English translation; St. Gregory the Great, *Forty Gospel Homilies*, Homily 34, checked in David Hurst, O.S.B., trans. and ed. (Gorgias Press, 2009), pp. 280–300; and St. Ambrose, *Exposition of the Gospel of Luke* VII.207–212, checked in Latin. The guide paraphrases their arguments and does not pass a working English translation off as original-language wording.
- Direct or explicit reuse of appointed material: Tertullian, *On Patience* 12; St. Gregory of Nyssa, *On Virginity* 12; and St. Augustine, *Confessions* VIII.3–4 and *Exposition on Psalm 54* 23. Augustine's psalm exposition directly treats the Gradual verse; the other works receive Gospel imagery or Luke's joy but do not comment on this Roman formulary.
- Related doctrinal illumination only: St. Irenaeus, *Against Heresies* IV.18.1–5, and St. Cyril of Jerusalem, *Mystagogical Catechesis* 5, especially 7 and 19–21. Neither is evidence for the Roman Secret's authorship, local history, or exact syntax.
- The online reception texts listed in the guide were consulted on 2026-07-10; their loci, direct-versus-related classifications, and paraphrase boundaries were re-audited on 2026-07-14. No patristic wording is presented as a verified direct quotation in the exposition.

## Cultural-reception search

- Exact and allusion searches on 2026-07-10 tested the Gradual's “cast thy burden” language, 1 Peter's “casting all your care,” Luke's joy over a repentant sinner, and the lost sheep across literature, film, television, hymns, classical music, and popular song.
- Two examples survived source verification. Felix Mendelssohn's *Elijah*, Op. 70 (1846), No. 15, sets “Cast thy burden upon the Lord” as a quartet; the [IMSLP work record](https://imslp.org/wiki/Elijah%2C_Op.70_%28Mendelssohn%2C_Felix%29) identifies the movement and the [CPDL score](https://www.cpdl.org/wiki/images/5/5d/Cast_thy_burden_upon_the_Lord_Mendelssohn.pdf) verifies the text. Morley Callaghan's *More Joy in Heaven* (Random House, 1937) places Luke 15:7 in its title and centers on an ex-convict's contested rehabilitation; the [first-edition record](https://openlibrary.org/works/OL3851049W/More_Joy_in_Heaven) and a [contemporary Kirkus review](https://www.kirkusreviews.com/book-reviews/a/morley-callaghan-2/more-joy-in-heaven/) verify the bibliographic and plot claims.
- Elizabeth Clephane's “The Ninety and Nine” and several direct devotional settings of 1 Peter 5:7 were reviewed but omitted to keep the interlude varied and because the retained Mendelssohn setting and Callaghan novel supply stronger, differently mediated examples.
- The two retained classifications, bibliographic locations, and supporting links were rechecked against the section standard on 2026-07-14; no additional example met a higher evidentiary threshold.

## Material limitations and unresolved questions

- The traditional dates for all four Davidic psalms are orientation dates, not demonstrated composition years. Psalm titles may signal authorship, collection, liturgical use, or association; Catholic tradition has not defined David as sole author of the Psalter.
- Psalm 7's Cush, Psalm 54's betrayer and city, and every psalm's writing site remain unidentified. No location was inferred merely from Zion, city, desert, or enemy imagery.
- Luke's Achaian composition place and c. AD 60 date come from separate inherited arguments. The city within Achaia is not named, and the modern c. AD 80–90 dating remains materially different.
- Luke itself supplies only the larger Jerusalem-journey frame and no town for the discourse; no narrower harmonized event location is asserted.
- First Peter's authorship and date remain contested. The table reports Rome and AD 63–64 because the requested metadata is traditional, not because the later Petrine-circle alternative has been resolved.
- No independent theological review is recorded. The working guide remains an aid to study rather than an official liturgical or critical edition.
- The 2026-07-14 full-document reevaluation corrected interpretive wording in the page-one map, gave the Collect its own clause-by-clause treatment, consolidated repeated patristic summaries, removed a duplicate full-text Secret display, completed the proper-reference cues, replaced a repeated exploratory proposal, and reclassified every source by evidentiary role. The resulting 13-page production snapshot passed two-pass compilation, clean-log, metadata, extraction, embedded-font, PDF-structure, every-page visual, and installed/build identity checks; that snapshot is superseded by the final production record below.
- Final clean-build review completed 2026-07-18: the settled 12-page PDF, SHA-256 `e2c15ab4f8dbc13c5bd7a93e2dc6632f8e485757fe3224e0a03df8847decaa08`, passed two-pass compilation, warning-free log, metadata, extraction, embedded-font, PDF-structure, and every-page visual checks; the installed PDF is byte-identical to the reviewed build.

## Interpretive-proposal audit

The proposal-specific search on 2026-07-18 tested each distinctive conjunction against the guide's checked psalm and Gospel reception, exact and near-exact New Advent searches, the official Antonian sermon, and Aquinas's treatment of temporal goods and solicitude. The labels classify the proposal's stated mechanism, not the existence of commentary on an individual appointed text.

| Proposal | Anchors / mechanism | Novelty classification | Bounded search / nearest analogue | Fruit | Controlling limit |
| --- | --- | --- | --- | --- | --- |
| Temporal goods become a road only by changing grammar | Collect’s passage through temporal goods; Offertory praise and poor; Secret oblation; Postcommunion everlasting mercy; goods move from use to gift to final end. | precedent located | Aquinas explicitly distinguishes temporal goods as relative goods from the last end and says they are good insofar as ordered to virtuous works leading to heaven ([*ST* I–II, q. 114, a. 10](https://www.newadvent.org/summa/2114.htm)); he also subordinates temporal solicitude to charity and spiritual goods ([II–II, q. 55, a. 6](https://www.newadvent.org/summa/3055.htm)). Searches did not locate this exact four-proper relay, but its governing means–end mechanism has clear precedent. | Orders economic and domestic stewardship toward worship, mercy, restitution, and the eternal end. | Matter is neither suspect nor automatically sanctified; offering cannot repair injustice without conversion and restitution. |
| The burden can be cast because the burdened is carried | Epistle and Gradual cast care/burden; Gospel shepherd bears sheep; surrender rests on divine pursuit. | near analogue located | Exact searches for 1 Peter 5:7, Psalm 54/55, and Luke 15’s shoulders found no witness stating the full reversal. Anthony directly coordinates 1 Peter 5 with Luke 15 ([official sermon, §§2, 15](https://www.santantonio.org/it/sermoni/sermoni-domenicali/domenica-iii-dopo-pentecoste)); Tertullian vividly describes the exhausted sheep borne on the Shepherd’s shoulders ([*On Repentance*](https://www.newadvent.org/fathers/0320.htm)). These are near analogues, not the three-text mechanism. | Permits honest naming of anxiety, communal help, and trust without fantasies of self-support. | No immediate emotional relief is promised; vigilance, resistance, and solidarity in suffering remain commanded. |
| Asking to be seen becomes learning how to see | Introit `Respice in me`; Secret `Respice ... munera`; Communion’s joy over another; attention moves from self to common gift to restored neighbor. | not located in the checked corpus | Searches for the two `Respice` petitions with Luke 15, angelic joy, gaze, and common offering across Augustine, Ambrose, Gregory, Anthony, and exact Latin/English queries found no treatment of this visual sequence. Ambrose’s account of Christ looking on the penitent is the nearest image ([*Concerning Repentance* II](https://www.newadvent.org/fathers/34062.htm)), but it does not supply the liturgical redistribution of attention. | Forms prayer that remains honest about affliction while becoming capable of another’s restoration as common good. | Communion has no `respice`; angels are not communicants; Secret grammar does not describe psychology. |
| Communion rehearses the affect murmuring resists | Gospel murmuring versus heaven’s joy; Communion voices the latter; performed counter-affect trains communal desire. | precedent located | Ambrose warns that envy toward a restored sinner excludes the envious brother from the Father’s feast ([*Concerning Repentance* I.15](https://www.newadvent.org/fathers/34061.htm)); Augustine directly explores why restoration produces heightened joy and connects it to the Church’s solemn service ([*Confessions* VIII.3–4](https://www.newadvent.org/fathers/110108.htm)). These precedents establish the proposal’s envy-versus-shared-joy mechanism, though not the Communion antiphon’s later placement. | Tests how communities receive converts, penitents, and restored offenders whose mercy unsettles status. | Singing does not prove envy conquered; prudence and safeguarding remain; the antiphon is not a diagnosis of every communicant. |
| The recovered object becomes an ecclesial subject | Gospel sheep/coin as sought objects; Offertory’s trust, praise, and poor; Secret common oblation; mercy restores agency for worship and service. | not located in the checked corpus | Searches joining Luke 15’s recovered sheep or coin with Psalm 9’s praising/remembering the poor and the common oblation found separate reception of each dossier but no source making the agency reversal. Chrysostom turns the found sheep into a demand that the hearer care for the vulnerable ([*Homily 59 on Matthew*](https://www.newadvent.org/fathers/200159.htm)); that is the nearest pastoral analogue, not precedent for the recovered person as offerer. | Supports reintegration that restores voice, responsibility, worship, and service rather than permanent patronization. | Luke does not narrate later conduct; repentance remains necessary; no historical arrangement claim. |

## 2026-07-18 Final Production Record

The settled two-pass build produced warning-free logs; generation metadata, PDF structure, embedded fonts, Unicode text, and extraction checks passed; all 13 pages were visually inspected; and the installed PDF is byte-identical to the reviewed build, SHA-256 `e72603e279c02cd339b095fd028f9408a2dbc281633213c3ce0d8899996b6ad1`. This exact snapshot remains in private review pending renewed exact-snapshot authorization.

## 2026-07-18 Interpretive-Discovery Production Record

The settled build completed with a warning-free final log. Generation-metadata validation, qpdf structural validation, embedded-font inspection (15 of 15 fonts embedded), and text extraction (41,415 bytes) passed, and all 13 full-size pages were visually inspected. The installed PDF at `doc/gpt/liturgy/roman-rite/1962/propers/temporal/43-third-after-pentecost.pdf` is byte-identical to the reviewed build at `build/gpt/liturgy/roman-rite/1962/propers/temporal/43-third-after-pentecost.pdf`, SHA-256 `9501541d375da13a0a93fd1932a23070d9de973f80b2687de25bc84c57536240`. This exact snapshot remains in private review pending renewed exact-snapshot authorization.

## Notable-and-quotable audit

The gallery was re-audited on 2026-07-19 under the cultural-afterlife rubric. Appointed wording was checked against `propers/verified.md`; later wording and context were checked in the identified public-domain editions. Patristic commentary was removed from the gallery and remains in detailed commentary.

| Proper phrase and later locus | Relationship and wording check | Later context and cultural payoff | Translation and rights status | Controlling limit |
| --- | --- | --- | --- | --- |
| Luke 15:1–2, “publicans and sinners”; *Punch* 73 (27 Oct. 1877), p. 182 | Exact English phrase, used with a deliberate lexical pun; checked in the [Heidelberg scan and OCR](https://doi.org/10.11588/diglit.17731#0186). | In an argument about public-house numbers, *publican* changes from tax collector to tavern keeper, and “sinners” become drinkers whom publicans tempt. | 1877 periodical and Heidelberg reproduction are marked public domain/CC0; excerpt is 11 words. | The joke exploits a later English sense and is not an interpretation of Luke's historical tax collectors. |
| Luke 15:4–7, the “lost sheep”; Harriet Beecher Stowe, *We and Our Neighbors* (1875), ch. 19, pp. 197–201 | Exact phrase as chapter title, with an explicit Good-Shepherd application later in the chapter; checked in the [Project Gutenberg text](https://www.gutenberg.org/files/48603/48603-h/48603-h.htm). | The rural recovery image becomes urban social-reform fiction about a destitute young woman, prostitution, refuge, class comfort, and Christian responsibility. | 1875 U.S.-public-domain novel; only the short title phrase is quoted and narrative is paraphrased. | The novel's application reflects its own social and gender assumptions; the woman is not reducible to an object lesson. |
| 1 Peter 5:8, “Be sober, be vigilant”; *The Temperance Text-Book* (Philadelphia, 1836), p. 93, “Testimony of Scripture” | Exact phrase and placement checked in the [National Library of Medicine / Internet Archive public-domain scan](https://commons.wikimedia.org/wiki/File:The_temperance_text-book_-_a_collection_of_facts_and_interesting_anecdotes,_illustrating_the_evils_of_intoxicating_drinks_(IA_101588361.nlm.nih.gov).pdf). | Peter's broad spiritual alertness before a prowling adversary is made a literal anti-alcohol proof text among warnings about drunkards and strong drink. | 1836 U.S.-public-domain work; four-word quotation. | The temperance manual narrows the verse's register and does not exhaust its devil, resistance, suffering, or vigilance context. |

Material negative results: the hymn “The Ninety and Nine” and other musical settings were rejected as bare settings; patristic shepherd and coin excerpts were returned to detailed commentary; generic “lost sheep” charities and unattributed quotation pages lacked a sufficiently verified semantic turn; roaring-lion entertainment analogies were rejected because dependence could not be shown. A Dutch confessional satire using the lost-sheep image was verified at the Rijksmuseum but omitted because the three selected examples offered clearer verbal loci and a more varied gallery.

## 2026-07-18 Notable-and-Quotable Production Record

The final 15-page build is settled and deterministic across two forced root rebuilds. Its warning-free log, generation metadata, qpdf structure, embedded Unicode fonts, text extraction, physical-page-2 boundary, and every-page full-size visual review passed. The installed PDF is byte-identical to the reviewed build, SHA-256 `0a6373cda6a22fdf0520be12bf6cf7d0b720b881bfb428b83240b3a714fd3d50`. This exact snapshot remains in private review pending renewed exact-current-snapshot authorization.

## Compact-rights-colophon production review — 2026-07-19

The settled 14-page PDF, SHA-256 `1d94bc69a5e5e90a478c4ad793d158e71cbd60e15940f874b57f2106c02e2336`, is the exact repository-reviewed build snapshot. Its Generation Metadata displays each model/configuration once per contiguous group while retaining every distinct Agent/runtime record. The final log was warning-free; generation metadata, `qpdf` structure, PDF title/page metadata, embedded and subsetted fonts with Unicode maps, and nonempty text extraction passed their gates. Repository production review inspected every physical page; the compact final-page reuse-and-rights colophon is readable, unclipped, non-overlapping, remains with terminal content, and creates no rights-only spill page. The installed PDF is byte-identical to this reviewed build. This reviewed build supersedes every earlier production snapshot. Because its bytes changed, exact-snapshot distribution approval remains pending.

## Corrected scriptural-afterlives production review — 2026-07-19

The final 13-page PDF, SHA-256 `14ef73ae5ea533999f0d00801a281db2614a2b3ea20493f79271f32992de4831`, is the exact clean-build snapshot for this revision. Its rebuilt Notable and Quotable gallery contains only source- and locus-audited, non-obvious cultural, humorous, ironic, idiomatic, literary, political, institutional, visual, scientific, commercial, or comparable afterlives of wording from appointed Scripture; the audit above records verbal strength, context, semantic turn, rights, limits, and rejected leads.

Generation metadata is singular and terminal. The warning-free clean build passed metadata, `qpdf`, title and subject, embedded and subsetted Unicode fonts, extraction, the page-2-only dossier boundary, and terminal-colophon gates; every rendered page was inspected at full size with no clipping, overlap, bad split, artificial padding, sparse spill, or rights-only page. The installed PDF is byte-identical to this reviewed build. This record supersedes earlier production snapshots; the exact current snapshot has release-specific distribution clearance under the 20 July 2026 dated rights supplement. That operational decision supplies no independent review or ecclesiastical approval.

## Research-staleness verdict — 2026-07-26

The current source, in-place candidate, and independent rewrite were compared
claim by claim against the changed Missal inputs. No material correction was
found; this exact study edition is ready for rebaselining.
