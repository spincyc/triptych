\section{Start here: choose one job}

\begin{familybox}{Use the short path first}
This book contains a child route and a complete coach reference. Do not begin
by assigning the child all fifty-seven action rows. First fill in ``Today's
Mass,'' let the child read the eight-scene map, and turn to only his own role
sheet. Rehearse one scene at a time. Use the complete chart later to settle an
exact cue, handoff, branch, or source question.
...
Use the contents to print ``Start here,'' ``Today's Mass,'' and the eight-scene
map. Under ``Learn your own role,'' start at the child's role heading and stop
at the next role heading; keep a shared boundary page when two headings land
on it. Keep the complete action chart for the coach. The response course is on
numbered pages \pageref{course:first}--\pageref{course:last}. The ready-to-cut
decks are: cue cards, numbered pages
\pageref{cards:cue-first}--\pageref{cards:cue-last}; pronunciation cards,
numbered pages \pageref{cards:pron-first}--\pageref{cards:pron-last}; and Missa Cantata
...
Child and assigned role & \MCGuideLine{0.61\linewidth}\\[5pt]
Priest / MC / adult trainer & \MCGuideLine{0.61\linewidth}\\[5pt]
...
you do inside each scene; the action IDs lead to the coach chart when an adult
needs the full detail.

\begin{description}[style=multiline,leftmargin=0.19\linewidth,labelwidth=0.16\linewidth]
  \item[1. Get ready.] Check the roster and objects, line up, process, land at
  the named places, and account for the priest's biretta (MC-001--007).
  \item[2. Begin.] The quiet response team serves the prayers at the foot; the
  team then follows the Introit, Kyrie, Gloria when appointed, and Collect
  posture (MC-008--018).
  \item[3. Listen.] Follow the Epistle route, prepare and serve the Gospel, sit
  for the sermon if one is given, and take the appointed Credo gestures
  (MC-019--029).
  \item[4. Prepare.] Serve wine and water, the selected incense route and
  Lavabo, and the quiet \textit{Suscipiat} (MC-030--035).
  \item[5. Adore.] Follow the Preface, bring torches, kneel for the Canon, and
  use the exact bell and elevation-incense cues (MC-036--042).
  \item[6. Receive.] Follow the Lord's Prayer and Agnus Dei; when the faithful
  receive, use the bell and plate route; then complete both ablutions and the
  book transfer (MC-043--050).
  \item[7. Finish.] Take the day's Postcommunion posture, dismissal branch,
  blessing when it belongs, and Last-Gospel route when it is said
  (MC-051--054).
  \item[8. Return.] Collect only the named exit objects, make the assigned
...
\begin{description}[style=multiline,leftmargin=0.25\linewidth,labelwidth=0.22\linewidth]
  ...
  shoulder to shoulder with a partner, at a prayerful pace. Never walk
  backward down steps.
...
\section{Coach reference: every action in order}

The following chart preserves the complete model route. A child may use an
action ID to check one detail, but the chart is chiefly for the adult trainer
and MC who must coordinate every person, object, branch, and waiting state.
...
stand. Then take this model order: TH with BB on the incense branch; CB between
A1 and A2, or A1 and A2 as a candle pair when there is no cross; T1 with T2;
T3 with T4; L on the alternate lesson branch; MC; priest. Omitted roles close
the gap without changing partners.}{The adult owns every flame handoff.
\LocalChoice: the church and sacristy route can change spacing.}
\end{actiontable}
...
turn together, and begin the procession without a gap.}{Hands joined unless
carrying an object.}
...
side, keep the plate level beneath each communicant, and watch the Host rather
...
and stand together from the Gospel-side altar stand, check that the center is
...
L remain empty-handed with hands joined. Form the same pair order used at
MC-004.}{The priest's biretta and every entrance object have now rejoined the
...
entrance.}{\MissalRule: \emph{Ritus servandus} XII.6 supplies the reserved/
