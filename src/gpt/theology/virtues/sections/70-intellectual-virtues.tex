...
\virtuedossier
  ...
  {\textbf{Atechnia} (Greek \emph{atechnia}, “lack of art”) is Aristotle's named contrary: a productive state involving false reasoning about what can be made (\oppositioncode{K}). It can miss the work's rule by defect or excess, so its placement on the defect side is lexical rather than quantitative.}
  {Art has no dedicated excess-side mate (\oppositioncode{U}). A false productive judgment can posit what the work's plan does not warrant, but that act-level failure is not superabundant art; virtuosity used for an evil end can remain technically excellent while morally corrupted in its use.}
  {Art perfects the work as made; prudence perfects the maker's action as human. Incompetence can be nonculpable and negligence belongs to a wider moral judgment. The fine arts are one subset of \emph{ars}, which also includes crafts and disciplined productive skills.}
