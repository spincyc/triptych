...
For weekly and ritual Mass guides, `Generation Metadata` is the terminal section of the document. The standalone sacramental reference work records the same three fields once on its title page instead. Keep the metadata compact—normally three short labeled lines with semicolon-separated key-value fragments rather than six repetitive fields or explanatory sentences:
...
The parallel non-Sunday pattern is `src/gpt/mNN-formulary-name/` with the same `main.tex`, `propers/`, and `research/` structure. The doctrinal reference work instead uses `src/gpt/sacraments/fragments/` for the shared matrix and lexicon, `src/gpt/sacraments/sections/` for long treatments and the initiation-practice table, `src/gpt/sacraments/summaries/` for the canonical reusable one-page appendices, and `src/gpt/sacraments/research/scope.md` for its authority and qualification audit. `src/gpt/sacraments-at-a-glance/main.tex` composes those canonical fragments and contains no independent doctrinal copy. Installed PDFs follow the same provider branch under `doc/gpt/`.
...
- the sacramental reference document begins with a title page containing its nonduplicated AI-generation metadata and a table of contents, keeps the metaphysical lexicon before the continuous main treatment, retains every chapter-specific analytical table in its relevant chapter, and places the complete matter–form–subject–minister–effect matrix, the single initiation-practice table for all twenty-four Catholic Churches *sui iuris*, and the seven consistently structured one-page summaries after `References` as terminal appendices in canonical order; every full treatment and summary explicitly distinguishes primary proper, intrinsic secondary, contingent, and ultimate ends where those distinctions apply;
...
- in weekly and ritual Mass guides, `Generation Metadata` is the terminal section; in the standalone sacramental reference work, the same fields appear once on the title page; in either location, the metadata normally uses three compact labeled lines and unambiguously gives the final generation date in `YYYY-MM-DD` form, the full verbatim AI model label, every exposed model qualifier, the AI agent and its role, every exposed and accurately labeled agent/client/build version, and the product/interface-level AI environment;
