...
`Generation Metadata` is always the terminal section of the document. It must state these three labeled fields unambiguously:

```text
Generation date: YYYY-MM-DD
AI model: exact model name or runtime identifier
AI environment: product and interface in which the model ran
```

Use the ISO date on which the final document was generated or last substantially regenerated. Record the exact model identifier exposed by the runtime; if the runtime does not expose one, state `Not exposed by the AI environment` rather than guessing. Describe the AI environment at the product/interface level, for example `OpenAI Codex CLI`, `ChatGPT with a Codex workspace`, or another comparably specific interface. If multiple models or AI environments materially contributed, list each and its role.
