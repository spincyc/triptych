...
## Codex sessions and retained runs

A Codex session may work directly in the current checkout, including the primary checkout, within the authority granted for its task. Inspect and preserve unrelated changes before mutation. A session already running in a launcher-managed linked worktree remains there and must not invoke the launcher again or create, remove, lock, unlock, prune, or enter another worktree.
