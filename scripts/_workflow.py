...
class WorkflowEngine:
    ...
    def advance(self, run_id: str, result_path: str | None = None,
        ...
        if stage["type"] == GATE:
            ...
            # Each lane was valid in isolation, but downstream identity is the
            # finding id alone.  Validate the engine's union as one result so
            # two lanes cannot mint the same id and leave routing, ownership,
            # and convergence to disagree about which finding survived.
            _validate_result(
                result,
    ...
    def _stale_reviews(self, workflow, state, review_input_stages=None):
        """Check selected seals in order; omission checks every sealed review."""
        latest = {record["stage"]: record for record in state["result_hashes"]}
        findings = []
        stages = workflow["stages"]
        if review_input_stages is not None:
            by_id = {other["id"]: other for other in stages}
            stages = [by_id[sid] for sid in review_input_stages]
        for other in stages:
    ...
    def _failure_budget_spent(
        ...
        # Two rules, from two runs that each blocked while converging, and
        # they compose.
        #
        # What charges the budget is a repair that was attempted and reported
        # failed, by the reviser this stage's findings were routed to. Where
        # no such report exists the comparison of finding ids is still the only
        # signal there is, and it stays: removing it everywhere would bound
        # such a stage by `max_total_iterations` alone and silently double
        # every limit an operator declared. Ground truth displaces the
        # heuristic exactly where it exists and nowhere else, and for the case
        # that prompted it -- `content-evaluation` routing to
        # `content-revision`, which reports -- ids are not read at all.
        #
        # And where ids are read, a repeat is the same defect coming back to
        # the *same owner*. An id that returns naming a different owner is the
        # second phase of a repair that is working: the first owner did its
        # half and the defect moved to whoever owns the rest. Run
        # ce4ecd514b64d2f9 died on exactly that -- CON-CIT-007 went to
        # `research`, research recorded the evidence, and the finding came back
        # as `authoring` because the leaf had still not cited it. Both sides of
        # that comparison are None for a finding with no owner, so a gate and
        # an evaluator that does not route by owner charge exactly as before.
        #
        # A gate keeps the comparison whatever a reviser reports. What makes an
        # id untrustworthy is that an AI lane mints it and cannot know what an
        # earlier iteration used; a gate's ids are its own check ids, so the
        # same check refusing again is the tool re-running, which is a
    ...
    def _run_gate(
        ...
        if stage.get("verify_review_inputs"):
            stale = self._stale_reviews(workflow, state, stage.get("review_input_stages"))
            if stale:
...
def _validate_workflow(data: dict[str, Any], path: Path) -> None:
    ...
    for i, stage in enumerate(data["stages"]):
        ...
        if "review_input_stages" in stage:
            selected = stage["review_input_stages"]
            if stype != GATE or stage.get("verify_review_inputs") is not True:
                raise WorkflowError(f"{path}: {sid}: review_input_stages requires a gate verifying review inputs")
            if (not isinstance(selected, list) or not selected
                    or any(not isinstance(value, str) or not value for value in selected)
                    or len(set(selected)) != len(selected)):
                raise WorkflowError(f"{path}: {sid}: review_input_stages must be a nonempty list of unique stage IDs")
    ...
    accepting = []
    stages_by_id = {stage["id"]: stage for stage in data["stages"]}
    for stage in data["stages"]:
        sid = stage["id"]
        for selected_id in stage.get("review_input_stages", []):
            selected = stages_by_id.get(selected_id)
            if (selected is None or selected["type"] != EVALUATOR
                    or not selected.get("review_input_command")):
                raise WorkflowError(f"{path}: {sid}: review_input_stages references an unknown or unsealed evaluator: {selected_id}")
        targets = [
            (tkey, stage[tkey])
...
def _validate_result(
    ...
    if "findings" in result:
        ...
        _validate_unique_result_ids(findings, "findings")
    ...
    if "finding_dispositions" in result:
        ...
        _validate_unique_result_ids(dispositions, "finding_dispositions")
...
def _validate_unique_result_ids(entries: list[Any], field: str) -> None:
    """Require one stable, nonempty identity per result collection.

    Findings may reuse an id in a later iteration: that recurrence is how the
    convergence budget recognizes a standing defect.  Inside one submitted
    result, however, every downstream consumer keys on id alone.  Accepting a
    duplicate would make a set keep one copy, a mapping keep the last owner,
    and route selection see both.  Dispositions form their own collection and
    may refer to findings from an earlier result, but they must be unique among
    themselves for the same reason.
    """
    seen: dict[str, int] = {}
    for index, entry in enumerate(entries):
        if not isinstance(entry, dict):
            # The collection-specific validator below owns this clearer error.
            continue
        identity = entry.get("id")
        if not isinstance(identity, str) or not identity.strip():
            raise WorkflowError(
                f"{field}[{index}].id must be a nonempty string"
            )
        if identity in seen:
            raise WorkflowError(
                f"{field}[{index}].id duplicates {field}[{seen[identity]}].id: "
                f"{identity!r}"
            )
        seen[identity] = index
