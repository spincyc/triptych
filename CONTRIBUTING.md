...
6. After reviewing the committed result, expressly authorize its integration and any resulting update to its recorded local target, including local `main` when `main` was the dispatch branch. Return to that target’s clean primary checkout and run `make integrate <run-id>` (or `scripts/triptych-codex --triptych-integrate <run-id>` directly). If needed, the command fast-forwards the target; after confirming that the exact worker commit is reachable, it attempts to remove the retained task checkout and worker branch. It refuses uncommitted work, a dirty primary checkout, the wrong target branch, or diverged history. The launcher does not invoke `git push`; locally configured Git hooks still apply.
