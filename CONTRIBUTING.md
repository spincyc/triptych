...
6. After reviewing the committed result, expressly authorize its integration and any resulting update to its recorded local target, including local `main` when `main` was the dispatch branch. Return to that target’s clean primary checkout and run `make integrate <run-id>` (or `scripts/triptych-codex --triptych-integrate <run-id>` directly). A result already based on the target is fast-forwarded unchanged. If the target advanced independently, the command rebases the audited worker commits onto its current tip and then fast-forwards, keeping the target history linear; this limited rewrite changes the unlanded worker commit IDs, never the target’s existing history. A conflict aborts the rebase and retains the original worker for reconciliation rather than creating a merge commit. After verifying the landed commit, the command attempts to remove the retained task checkout and worker branch. It refuses uncommitted work, a dirty primary checkout, the wrong target branch, or histories that no longer descend from the recorded base. The launcher does not invoke `git push`. It disables hooks for its internal worker rebase so they cannot alter the generated candidate; target-side merge hooks still apply, and checked-out state is rechecked before cleanup.
