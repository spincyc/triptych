...
**`build/tpt-runs/90dcdddcb6780e60` must not be.** No
`evaluations/blocking-findings-v1.toml` exists for this leaf — the record was
built during this work and no run has yet written one — so that directory's
`results/content-evaluation-0002.json` is the only record anywhere of the
seven blocking findings standing against the Fourteenth Sunday leaf, and its
`results/content-revision-0001.json` the only record of what the second
revision pass repaired. `make clean` and `wt tidy` each delete it without
asking. Preserve it until a run writes the tracked record for this leaf, or
until those findings are copied into one by hand.
