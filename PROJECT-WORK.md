...
judged the converter fit to release. Its two latent advisories are fixed in
0c5efacd4, and the reviewer's own probes confirm both resolved with no
regression:

- WEB-R-012: the audit now handles a leaf's macros before the built-ins,
  HTML entities, tables closed by a command and title pages;
- WEB-R-013: a blank line after a substituted dash or centered dot stays a
  paragraph break.
...
Study-review iteration 0 failed with five blocking findings. STU-001, owned
by research, concerns Hilary on Matthew: the study quoted him only as
Aquinas's *Catena* reports him, and his own text in PL 9 differs. The engine
therefore returned the run to research iteration 5. The four study-owned
