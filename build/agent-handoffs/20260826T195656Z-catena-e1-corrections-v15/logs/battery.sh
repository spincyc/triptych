...
if [ -n "${TRIPTYCH_CHROME:-}" ]; then
  GATE_ENV="TRIPTYCH_CHROME='$TRIPTYCH_CHROME' "
fi
run browser-gate "${GATE_ENV}node tools/tests/corpus_browser_gate.mjs --json-out '$LOG_ROOT/browser-gate-$SIDE.json' > /dev/null; gate=\$?; python3 '$HERE/gate-summary.py' '$LOG_ROOT/browser-gate-$SIDE.json'; echo 'report written: browser-gate-$SIDE.json beside this log, under this attempt root (the gate prints the same json report to stdout; discarded so the report ships once)'; exit \$gate"
if [ "$SIDE" = head ]; then
