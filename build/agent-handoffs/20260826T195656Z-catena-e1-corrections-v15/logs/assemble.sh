...
#
# V15, the V14 review. This line printed `executed-tools: 16
# shipped-executed, 11 shipped-not-executed` -- INVOCATION rows -- and three
# lines below it the verifier's tool table printed `classes : shipped-executed
# 4, shipped-not-executed 14` over 18 unique TOOLS. Both true, neither said
# which, and the two could not be reconciled by a reader. `sanitize-and-seal.py`
# is ONE tool and EIGHT invocations; the record now says both, under names
# that cannot be confused, and never as a combined `N/M`.
tools_by_class = {}
...
executed_tools = sum(1 for klass in tools_seen.values()
                     if klass in ("shipped-executed", "external-system-tool"))
executed_invocations = sum(
    1 for row in rows
    if row["class"] in ("shipped-executed", "external-system-tool"))
print(f"executed-tools: unique_tools {len(tools_seen)}, "
      f"invocations {len(rows)}, "
      f"executed_tools {executed_tools}, "
      f"executed_invocations {executed_invocations}"
      + (f", merged_battery_invocations {merged}" if merged else ""))
