...
does.
    ...
    {
      "schema":         "catena-final-authority/1",   # exact literal
...
BATTERY_STATES = {"started", "complete", "failed", "set-aside"}

# The only state a `set-aside` row may follow. Setting aside a battery that
# never completed is a different fact, and `failed` already has a word.
...
AUTHORITY_SCHEMA_ID = "catena-final-authority/1"
...
def scan_prose(text: str, winner: str, label: str, fault,
               package_name: str | None = None) -> int:
...
def check(args) -> list[str]:
    ...
    for row in ledger.get("rows", []):
        if row.get("record") == "state" \
                and str(row.get("status", "")) == "set-aside":
            one = canonical(str(row.get("attempt", "")))
