...
import catena_command as CATENA_COMMAND  # noqa: E402
...
class BatteryLedger(unittest.TestCase):
    ...
    def test_battery_writes_provenance_and_unique_indexed_logs(self):
        with tempfile.TemporaryDirectory() as scratch:
            ...
            # V16, THE V15 REVIEW: THE TOKEN IS `$CANDIDATE_REPO`, NOT
            # `$REPO`. `$REPO` meant the parent checkout in one field of a
            # V15 row and the candidate checkout in the next; the names are
            # disjoint now and `catena_command.py` refuses a record that
            # defines `REPO` at all.
            self.assertIn("cwd=" + "$" + "CANDIDATE_REPO", ledger,
                          "the cwd is the token, never the real path")
            self.assertNotIn("cwd=" + "$" + "REPO" + "\n", ledger,
                             "the overloaded V15 name is never written again")
            # AND THE ROOTS ARE DEFINED IN THE RECORD THAT USES THEM. V15's
            # rows named `$WORKSPACE` and `$REPO` with no definition anywhere
            # in the package.
            for root in ("CANDIDATE_REPO", "PARENT_REPO", "TOOLS_ANCHOR",
                         "EVIDENCE_ROOT"):
                self.assertIn("root=" + root + " ", ledger,
                              root + " is defined once, at run time")
            # EVERY STEP CARRIES A MACHINE-READABLE EXEC RECORD beside its
            # prose, and no recorded command hides a variable inside single
            # quotes -- which is the fault V15 shipped seven times.
            seen = 0
            for line in ledger.splitlines():
                if not line.startswith("CMDJSON: "):
                    continue
                seen += 1
                record = json.loads(line[len("CMDJSON: "):])
                self.assertEqual(record["schema"], "catena-exec-command/1")
                self.assertTrue(record["cwd"].startswith("$"))
                if record.get("shell"):
                    self.assertEqual(
                        CATENA_COMMAND.unexpandable_variables(
                            record["shell"]), [],
                        "a variable inside single quotes cannot expand")
            self.assertGreater(seen, 0,
                               "every step records an exec record")
...
class TokenLegend(unittest.TestCase):
    """TWO VOCABULARIES NAME ONE DIRECTORY, AND THE PACKAGE SAYS SO ITSELF.

    `battery.sh` and `assemble.sh` write REPLAY ROOTS into command records --
    `$CANDIDATE_REPO`, `$PARENT_REPO`, `$TOOLS_ANCHOR`, `$EVIDENCE_ROOT`,
    `$PACKAGE_ROOT` -- which are BINDINGS a replay supplies. The sealer writes
    SUBSTITUTION TOKENS into transcript prose at P2 -- `$REPO`, `$WORKSPACE`,
    `$EVIDENCE`, `$SCRATCH`, `$HOME` -- which stand where a private path was
    removed and which nothing binds.

    So the candidate checkout is `$CANDIDATE_REPO` in `commands.json` and
    `$REPO` in a transcript. That is NOT V15's defect -- V15's was one token
    meaning TWO directories inside a single row -- but it is exactly what the
    V15 review landed on when `verify-final.log` said `$WORKSPACE/...` and
    `executed-tools.json` said `$EVIDENCE` for one anchor. The legend answers
    it in the package's own bytes, and it is DERIVED: the roots from the
    ledgers, the tokens from the sealer's own tables.
    """

    ROOTS = {
        "CANDIDATE_REPO": "the implementation checkout under review",
        "PARENT_REPO": "the comparison checkout",
        "TOOLS_ANCHOR": "the handoff-tools checkout this lane ran from",
        "EVIDENCE_ROOT": "the staged package source directory",
    }

    def test_the_substitution_vocabulary_is_read_from_the_sealer(self):
        """Not typed here and not typed there: three independent readings of
        the sealer's own tables, unioned."""
        found = checks.sanitization_tokens()
        self.assertEqual(
            ["$EVIDENCE", "$HOME", "$REPO", "$SCRATCH", "$WORKSPACE"], found)

    def test_the_legend_covers_both_vocabularies_and_states_the_mapping(self):
        lines, problems = checks.token_legend(self.ROOTS, set())
        self.assertEqual([], problems)
        body = "\n".join(lines)
        self.assertIn("REPLAY ROOTS", body)
        self.assertIn("SANITIZATION TOKENS", body)
        for name in self.ROOTS:
            self.assertIn(f"${name} --", body)
        # The one correspondence there is, written as one.
        self.assertIn("$REPO == $CANDIDATE_REPO", body)
        # And the one the coordinator's draft got wrong: the sealer's
        # `$EVIDENCE` rule matches `.agents/<lane>`, not the tools anchor.
        self.assertIn("$EVIDENCE -- the lane's agent evidence directory",
                      body)
        self.assertNotIn("$EVIDENCE == $TOOLS_ANCHOR", body)

    def test_a_root_a_record_uses_and_no_ledger_defines_is_named(self):
        """`$PACKAGE_ROOT` is bound by the package-phase records and declared
        by no battery ledger; the legend derives it rather than dropping it."""
        lines, problems = checks.token_legend(self.ROOTS, {"PACKAGE_ROOT"})
        self.assertEqual([], problems)
        self.assertIn("$PACKAGE_ROOT -- the package directory itself",
                      "\n".join(lines))

    def test_an_unknown_root_a_record_uses_is_refused(self):
        """THE MATCHED NEGATIVE. A legend a reader cannot bind is not a
        legend."""
        _lines, problems = checks.token_legend(self.ROOTS, {"MYSTERY_ROOT"})
        self.assertTrue(any("references $MYSTERY_ROOT and no ledger" in one
                            for one in problems), problems)

    def test_a_substitution_token_with_no_legend_entry_is_refused(self):
        """The property the coordinator asked for, driven directly: a token
        the sealer can emit and the legend cannot explain is a refusal."""
        kept = dict(checks.SANITIZATION_MEANINGS)
        checks.SANITIZATION_MEANINGS.pop("$SCRATCH")
        try:
            _lines, problems = checks.token_legend(self.ROOTS, set())
        finally:
            checks.SANITIZATION_MEANINGS.clear()
            checks.SANITIZATION_MEANINGS.update(kept)
        self.assertTrue(any("the sanitizer can emit $SCRATCH and this legend "
                            "has no entry" in one for one in problems),
                        problems)

    def test_one_name_in_both_vocabularies_is_refused(self):
        """V15's DEFECT ITSELF, one level up: a name that is a binding in a
        record and a substitution in a transcript names two directories."""
        roots = dict(self.ROOTS)
        roots["REPO"] = "a replay root that reuses a substitution token"
        _lines, problems = checks.token_legend(roots, set())
        self.assertTrue(any("$REPO is BOTH a replay root and a sanitization "
                            "token" in one for one in problems), problems)

    def test_the_disjointness_is_stated_against_the_reserved_names(self):
        body = "\n".join(checks.token_legend(self.ROOTS, set())[0])
        for name in sorted(CATENA_COMMAND.RESERVED_VARIABLES):
            self.assertIn(f"${name}", body)
        self.assertIn("no name in this package means two directories", body)


class FencedReferences(unittest.TestCase):
    """A FENCE IS NOT A BACKTICK PAIR, AND THE INDEX CHECK MUST SEE PAST IT.

    V16: `_reference_candidates` paired backticks across the whole document,
    so a ``` fence -- three of them -- inverted the pairing of everything
    after it. Thirty of fifty-four references in this lane's own HANDOFF.md
    were invisible to the check that exists to read them. The index happened
    to be complete, so nothing shipped wrong; it was passing by parser
    accident rather than by coverage, which is the shape this protocol
    refuses everywhere else.
    """

    def candidates(self, text):
        return sealer._reference_candidates(text)

    def test_a_reference_after_a_fenced_block_is_still_seen(self):
        text = ("`before.log` is named.\n\n"
                "```js\n"
                "const x = 1;\n"
                "```\n\n"
                "`after.log` is named too.\n")
        self.assertEqual(self.candidates(text), {"before.log", "after.log"})

    def test_the_pairing_was_inverted_without_the_fix(self):
        """The control: strip the guard and the second name disappears."""
        text = ("`before.log` is named.\n\n"
                "```js\n"
                "const x = 1;\n"
                "```\n\n"
                "`after.log` is named too.\n")
        import re
        naive = {one for one in re.findall(r"`([^`]+)`", text)
                 if sealer.REFERENCE_SHAPE.match(one)}
        self.assertNotIn("after.log", naive)

    def test_two_fenced_blocks_do_not_hide_the_text_between_them(self):
        text = ("```\nfirst\n```\n\n`between.log`\n\n```\nsecond\n```\n\n"
                "`tail.log`\n")
        self.assertEqual(self.candidates(text), {"between.log", "tail.log"})

    def test_a_filename_inside_a_fence_is_an_illustration_not_a_claim(self):
        """A fenced block is a transcript or a command sample.

        A filename inside one shows what a tool printed; it is not this
        document asserting the package contains it, and checking it would
        refuse every honest transcript that names a file it does not ship.
        """
        text = "```\nwrote logs/not-a-member.log\n```\n\n`real.log`\n"
        self.assertEqual(self.candidates(text), {"real.log"})

    def test_an_unterminated_fence_does_not_swallow_the_document(self):
        text = "`kept.log`\n\n```\ndangling\n"
        self.assertIn("kept.log", self.candidates(text))
