...
#
# THE ATTEMPT STATE MODEL: TWO AXES, NEVER ONE. This is the one statement of
# it; `battery.sh` and `logs/checks.py` refer here rather than restating it.
#
# V16 SEPARATED WHAT V13 HAD RUN TOGETHER. `checks.py` carried
# `"complete": ("set-aside",)` in its battery table, which said that declining
# a cohort's figures is a TRANSITION OF THE EXECUTION MACHINE. It is not. How
# an attempt terminated and whether a completed attempt's measurements remain
# authoritative are two facts about two different things, and one table over
# both is the same "one word for two facts" defect that made V11's
# authoritative count uncountable, one level down: read through the collapsed
# table, an attempt that ran to completion and was later declined printed
# `set-aside`, and the fact that it completed appeared nowhere.
#
#   AXIS 1 -- THE EXECUTION DISPOSITION. How the attempt itself terminated.
#   Terminal, irreversible, EXACTLY ONE per attempt, carried by the one
#   `record=attempt` row. Nothing that follows it moves it.
#
#     BATTERY ATTEMPT   (side=head, side=parent)
#         started -> complete
#         started -> failed
#         started -> abandoned
#       `started` is optional -- a battery that runs to its terminal row in
#       one go writes only that row. `failed` is the run that reached a
#       DECISION: a step it ran returned non-zero, or one of its own guards
#       refused. A GUARD REFUSAL IS A CAUSE AND NOT A DISPOSITION: there is no
#       `refused` state, and the cause belongs in the reason. `abandoned` is
#       the run something OUTSIDE it stopped -- a killed process, a lost
#       handle, an operator interrupt -- with no step having failed at all;
#       it is not `failed`, because collapsing it there asserts a decision
#       nothing made. A battery attempt may NEVER be `sealed`, `sealing` or
#       `discarded`: those are package words.
#
...
#
#   AXIS 2 -- THE EVIDENCE DISPOSITION. Whether a COMPLETED attempt's
#   measurements remain authoritative. Only meaningful for an attempt whose
#   execution disposition is `complete` (battery) or `sealed` (package), and
#   carried by a LATER `record=state` row that leaves the terminal row exactly
#   where it is. An attempt with no such row reads `unevidenced`.
#
#     BATTERY COHORT    complete -> authoritative | set-aside
#       Irreversible: one evidence disposition, ever. `authoritative` says
#       this package reports these figures; `set-aside` says it does not, and
#       says why. As many cohorts as a package ships may be authoritative --
#       that is a different claim from being the package to review.
#
#     PACKAGE ATTEMPT   sealed -> authoritative -> superseded
#                       sealed -> superseded
#       `authoritative` here is the POST-P8 verdict, carried by a
#       `record=state` row that exists only OUTSIDE the package, appended at
#       P9 -- see THE V13 CORRECTION, ONE. AT MOST ONE package attempt in a
#       ledger may hold it, so superseding an attempt is what makes room for
#       the next one. THIS SCRIPT NEVER SUPERSEDES ANOTHER ATTEMPT. Sealing a
...
#       operator appends that supersession -- one row, one reason -- rather
#       than an assembly script making it as a side effect of running again.
#
#   AN ATTEMPT THAT MEASURED NOTHING IS `unevidenced`, PERMANENTLY. `failed`,
#   `abandoned` and `discarded` carry no evidence disposition and none may be
#   recorded on them: there is nothing to carry or to decline, and applying an
#   evidence word to one would cover what its disposition says. Such an
#   attempt is nonetheless RESOLVED -- its history is closed, the audit counts
#   it, and it is in no successful and no authoritative tally.
#
#   Exactly one `record=attempt` row per attempt carries the terminal
#   EXECUTION disposition (`complete`/`failed`/`abandoned`/`sealed`/
#   `discarded`). The non-terminal states and every EVIDENCE disposition
#   (`authoritative`/`set-aside`/`superseded`) are `record=state` rows, so
#   nothing can be described as authoritative and discarded at once, and no
#   evidence row overwrites the disposition it follows.
#
...
# silent.
# `$INVENTORY_LOG` IS NOT IN THIS LIST, AND CANNOT BE. P11 sanitizes and
# rescans the outer siblings; the completeness verdict is taken at P12, AFTER
# that, precisely so it inspects the final state -- which is the V15 defect
# being corrected, where the verdict was taken at P10 and went stale when P11
# wrote two more authoritative transcripts beside the package. The inventory
# transcript therefore does not exist when P11 runs, and naming it here made
# P11 refuse a file that by design was not there yet, then record the raw
# absolute path of the missing file in its own transcript and refuse again for
# the private tokens it had just written. P12 writes that transcript and
# sanitizes and rescans it itself, immediately, which is the only order in
# which a pass can cover its own output.
...
#
# V15, THE V14 REVIEW, AND THE ONE LINE THAT LOST FOUR ROWS. The V14 record's
# earliest row was from P2: `checks.py --allocate-ordinal` (P0),
# `compare-gate.py` (P1), the `checks.py` compose (P1) and
# `test-sanitize-and-seal.py` (P1) were all invoked through `run_tool` and
# none of them reached the record, so `compare-gate.py` shipped classed
# `shipped-not-executed` beside its own P1 transcript.
#
# The cause was NOT that those phases bypass `run_tool` -- they never did.
# `RUNS` was `${RUNS:-$(mktemp ...)}`, an UNCONDITIONAL inheritance from the
# environment, and the outer stage installs `trap 'rm -f "$RUNS"' EXIT`. At
# P1 this pipeline runs `test-sanitize-and-seal.py`, whose `ProtocolRefusals`
# tests spawn `assemble.sh` against throwaway repositories to prove it
# refuses an existing target. That harness scrubs the pipeline's own names
# from the child environment -- but its list did not carry `RUNS`. So the
# spawned assembly inherited the REAL run's scratch file, ran in OUTER mode
# (`ASSEMBLE_INNER` having been scrubbed), installed that trap, refused the
# stale target, exited 1 -- and deleted the live run's rows on the way out.
# P2's first append silently re-created the file, which is exactly why the
# record began at P2 and no phase reported an error.
#
# TWO FIXES, BECAUSE ONE OF THEM IS THE HARNESS'S. The harness list gained
# `RUNS` and `TOOLRUNS`. This line stops depending on that list being right:
# `RUNS` is inherited ONLY together with `ASSEMBLE_INNER`, which is the only
# way this script ever passes it -- the two are exported on the same three
# lines below. Any other process holding a `RUNS` in its environment gets a
# fresh file of its own, appends its own rows to it, and deletes only that.
if [ -n "${ASSEMBLE_INNER:-}" ] && [ -n "${RUNS:-}" ]; then
  : # the inner stage, continuing the outer stage's file. Never deleted here.
else
  RUNS=$(mktemp -t assemble-runs.XXXXXXXX)
fi

# THE BATTERIES' OWN CONTEMPORANEOUS DIGESTS, appended by `battery.sh` as it
# runs. Lane-wide and append-only, beside the attempt ledger and on the same
# terms: it spans attempts, because the batteries are separate invocations
# that ran before this one. `emit_executed_tools` merges the rows whose
# transcripts this package actually ships. See the note there. It is derived
# from `$ATTEMPTS` rather than spelled out, so the two records cannot be
# pointed at different directories by an operator who overrides one of them.
TOOLRUNS=${TOOLRUNS:-$(dirname "$ATTEMPTS")/executed-tools.jsonl}

# THE DRIVER RECORDS ITSELF.
#
# V16, THE V15 REVIEW: "both `assemble.sh` and `battery.sh` are marked not
# executed although they drove the build". They were, and the cause is
# structural rather than an oversight: `run_tool` hashes the file it is about
# to hand to an interpreter, and nothing hands THIS file to an interpreter --
# bash is executing it. So the one tool that drove every row in the record was
# the one tool the record could not see, and it landed in the
# `shipped-not-executed` roster beside its own invocation log.
#
# The row is contemporaneous on the same terms as every other: the sha256 of
# the exact bytes bash is running, taken at P0, before the first phase. Its
# `kind` is `driver`, and `emit_executed_tools` counts driver rows as
# EXECUTED invocations -- because they are -- while keeping them separately
# named so a reader can see which invocations are drivers and which are the
# steps the drivers drove.
record_driver() {  # logical-name absolute-path phase log
  local name=$1 path=$2 phase=$3 log=$4 digest
  [ -f "$path" ] || return 0
  digest=$(sha256sum "$path" | cut -d' ' -f1) || return 0
  python3 - "$name" "$digest" "$(date -Is)" "$phase" "$log" driver \
      <<'PY' >> "$RUNS"
import json, sys
...
emit_executed_tools() {  # phase
  python3 - "$RUNS" "$EXECUTED" "$ATTEMPT" "$EVIDENCE" "$PKG" "$1" \
      "$(basename "$RUN_LOG")" "$TOOLRUNS" <<'PY'
import datetime, hashlib, json, pathlib, sys

(runs_path, out, attempt, anchor, package, phase, outer,
 toolruns_path) = sys.argv[1:9]
logs = pathlib.Path(package) / "logs"

# The ordinal an attempt id carries: `<side>-<stamp>-<NN><suffix>`.
ATTEMPT_ROOT = __import__("re").compile(r"^[a-z]+-\d{8}T\d{6}Z-(\d{2})")
...
def agree(name, sha, where):
    """One logical tool, one set of bytes -- across BOTH sources."""
    first = digests.setdefault(name, sha)
    if first != sha:
        problems.append(f"{name}: executed as {first} and as {sha} "
                        f"({where}); one logical tool, two sets of bytes")
...
for line in pathlib.Path(runs_path).read_text(encoding="utf-8").splitlines():
    ...
    row = json.loads(line)
    name = row["tool"]
    agree(name, row["sha256"], "this attempt")
    ...
    #
    # `path` IS A PACKAGE-RELATIVE PATH OR IT IS EMPTY. It is never prose and
    # never an out-of-package location. V15, found at seal time: this branch
    # wrote an explanatory sentence into `path` for a system tool and the
    # anchor-relative location for a tool the package does not ship, and
    # `verify-final-package.py` refused both, in its own words -- "an
    # external system tool ships nothing, so its path is ''" -- on check 0,
    # and again on check 11 as a path the package does not ship. The verifier
    # is right and the writer was wrong: `path` is the field a reader
    # compares against the archive's member list, so a value that is not a
    # member name makes that comparison impossible.
    #
    # NOTHING IS LOST BY EMPTYING IT. What the sentence said is said by the
    # CLASS, which is the vocabulary's whole purpose: `external-system-tool`
    # means "ran, not ours to ship", and the verifier renders such a row's
    # logical path as "(external system tool)" for the reader. Where the
    # out-of-package copy lives is said once, at the top level, by `anchor`
    # -- repeating it per row said nothing the record did not already say.
    if row.get("kind") == "system":
        ...
        # Ours, executed, and NOT shipped under `logs/`. The only class the
        # verifier accepts for a tool the package does not ship is
        # `external-system-tool`, and it accepts it only with an empty path.
        path = ""
        klass = "external-system-tool"
...
# ---- the OTHER attempts' invocations, vouched for by this package ---------
#
...
# transcripts. The reported cause -- that the record's rows start at P2 -- is
# a real defect and is fixed where `RUNS` is created, but it is NOT the cause
# of these three: `assemble.sh` NEVER invokes them. `battery.sh` does, in a
# different attempt, and this script cannot record a digest for bytes another
# process handed to an interpreter minutes or days ago.
#
# So `battery.sh` records them itself, contemporaneously, into the same
# append-only lane file this reads. The ADMISSION TEST is not an attempt id
# and not a name match: it is whether THIS PACKAGE SHIPS THE TRANSCRIPT the
# row names. A row therefore enters the record only when the package carries
# the evidence for it, which is exactly the pair a reviewer needs -- "it ran,
# here are the bytes, here is the log, open it" -- and a battery whose logs
# were never staged contributes nothing, however many rows it wrote.
#
# The digest agreement rule spans both sources: if the battery ran different
# bytes than this assembly shipped, that is the same fault as a mid-run byte
# change and it refuses here rather than shipping a record that names one
# `logs/<tool>` for two sets of bytes.
merged = 0
toolruns = pathlib.Path(toolruns_path)
if toolruns.is_file():
    seen = set()
    for line in toolruns.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        try:
            row = json.loads(line)
        except json.JSONDecodeError:
            # A torn line in a best-effort side record is not a reason to
            # refuse a package; the ledger row and the transcript are the
            # primary evidence and they are written by another path.
            problems.append("the battery tool-run record has an unparsable "
                            "line; it is append-only and something wrote a "
                            "partial row")
            continue
        if not isinstance(row, dict):
            continue
        name, log, sha = (str(row.get("tool") or ""), str(row.get("log") or ""),
                          str(row.get("sha256") or ""))
        if not name or not log or not sha:
            continue
        if not (logs / name).is_file():
            continue                      # not a tool this package ships
        if not (pathlib.Path(package) / log).is_file():
            continue                      # not a transcript this package ships
        # AND IT MUST BE *THAT ATTEMPT'S* TRANSCRIPT.
        #
        # V16, found at P8 on the first assembly that got there. A battery
        # records its own driver row against `logs/order-<side>.txt`, which is
        # a PER-SIDE file: every battery attempt on a side names the same
        # path, and the package ships exactly one of them -- the winning
        # attempt's. The existence test above therefore admitted rows from
        # every attempt on that side, including a battery that FAILED before
        # it recorded anything and one whose cohort was later SET ASIDE. Their
        # digests were then compared against the shipped cohort's, and the
        # merge refused the package because one logical tool had been executed
        # as two sets of bytes -- which was true of the lane's history and
        # false of its evidence, since a fix lane had corrected the driver
        # between those attempts, exactly as a fix lane is supposed to.
        #
        # The stated admission test is "it ran, here are the bytes, here is
        # the log, open it". A per-side ordering file cannot satisfy it for an
        # attempt whose own root is not here, so the ATTEMPT'S OWN ROOT is
        # what is asked for. That root is per-attempt by construction, it is
        # staged only for a cohort this package carries, and it is the same
        # evidence a reviewer opens. A row from an attempt whose transcripts
        # are not shipped contributes nothing, which is what the comment above
        # already said and what the file check could not enforce.
        root = ATTEMPT_ROOT.match(str(row.get("attempt") or ""))
        if not root or not (logs / f"attempt-{root.group(1)}").is_dir():
            continue
        if (name, log) in seen:
            continue
        seen.add((name, log))
        agree(name, sha, f"battery attempt {row.get('attempt') or '(unnamed)'}")
        rows.append({"tool": name, "path": f"logs/{name}",
                     "attempt": str(row.get("attempt") or attempt),
                     "sha256": sha, "at": str(row.get("at") or ""),
                     "phase": str(row.get("phase") or ""), "log": log,
                     "kind": str(row.get("kind") or "battery"),
...
# ---- THE ROSTER, WHICH IS NOT A LIST OF INVOCATIONS -----------------------
#
# V16, THE V15 REVIEW: "six supposed invocations are manufactured
# not-executed placeholders". They were, exactly: this loop fabricated an
# `at` (the render instant), a `phase` ("P10 the post-seal gates") and a `log`
# (the assemble log) for a tool that had not run, appended the result to
# `runs` beside twenty-seven real invocations, and the epilogue then counted
# `invocations {len(rows)}` over all thirty-three. Six of those thirty-three
# describe nothing that happened, and only the `class` field distinguished
# them.
#
# A NOT-EXECUTED TOOL HAS NO INSTANT, NO PHASE AND NO TRANSCRIPT, so the
# roster row carries none of the three. What it carries is what is true: the
# package ships this file, here are its bytes, this attempt did not run it.
# It lives in its own list, `shipped_not_executed`, and is counted under its
# own name. Nothing is lost -- every tool the package ships still appears
# with a class, which is the requirement -- and nothing is manufactured.
shipped_not_executed = []
if logs.is_dir():
    for one in sorted(logs.iterdir()):
        ...
        if one.name in executed:
            continue
        shipped_not_executed.append({
            "tool": one.name,
            "path": f"logs/{one.name}",
            "attempt": attempt,
            "sha256": hashlib.sha256(one.read_bytes()).hexdigest(),
            "class": "shipped-not-executed",
            "why": ("the package ships this file and this attempt did not "
                    "execute it; it has no instant, no phase and no "
                    "transcript because it did not run"),
        })

record = {
    # /2 ADDS: `phase_view`, `counts`, and `shipped_not_executed` as its own
    # list. See the counting block below for why each exists.
    "schema": "catena-executed-tools/2",
    "phase_view": phase,
    "attempt": attempt,
    ...
    "anchor": anchor,
    "runs": rows,
    "shipped_not_executed": shipped_not_executed,
}
# FIVE FIGURES, FIVE NAMES, NEVER COMBINED AND NEVER UNLABELLED.
#
# V16, THE V15 REVIEW, TWO FAULTS IN ONE LINE.
#
# ONE: `invocations` counted the roster. V15 printed `unique_tools 20,
# invocations 33, executed_tools 14, executed_invocations 27` where the 33
# includes six roster rows that describe no invocation at all. A reader
# cannot subtract what they cannot see.
#
# TWO: THE SAME QUANTITY WAS SHIPPED TWICE, UNDER TWO VALUES, WITH NEITHER
# LABELLED BY PHASE. `.tool-bytes.json` and the P8 table say `executed_tools
# 11, executed_invocations 24`; the P10 line of `.assemble.log` says
# `executed_tools 14, executed_invocations 27, merged_battery_invocations 6`.
# Both are true OF DIFFERENT INSTANTS -- this record is rendered before P8,
# because P8 receives it as `--executed`, and re-rendered after P10 with the
# gates in it -- and nothing anywhere said so. `phase_view` now labels every
# rendering with the phase it was taken at, and this line prints it, so two
# figures for one quantity are two answers to two questions rather than a
# contradiction.
#
# THE FIVE, DEFINED:
#
#   unique_referenced_tools   every tool this record names at all: executed,
#                             shipped-and-not-executed, and external.
#   unique_executed_tools     tools with at least one OBSERVED invocation.
#   execution_invocations     observed invocations. Never a roster row.
#   shipped_tools             files under `logs/` the package ships.
#   trusted_not_executed_tools  shipped, and this attempt did not run them.
#
# AND TWO MORE THAT ARE NOT FOLDED INTO ANY OF THEM:
#
#   driver_invocations        `assemble.sh` and `battery.sh`, which drove the
#                             rows rather than being driven.
#   external_system_tool_rows RECORDED ONCE PER TOOL AT FIRST USE, not per
#                             invocation. python3 and git run dozens of
#                             times; this is a REPRESENTATIVE row and the
#                             field below says so in as many words, because
#                             V15's "repeated Python/Git uses are sampled
#                             rather than counted" is a true criticism of a
#                             figure that did not admit it.
tools_seen = {}
for row in rows:
    tools_seen.setdefault(row["tool"], row["class"])
roster_tools = {one["tool"] for one in shipped_not_executed}

execution_invocations = len(rows)
unique_executed_tools = len(tools_seen)
driver_invocations = sum(1 for row in rows if row.get("kind") == "driver")
external_rows = sum(1 for row in rows
                    if row["class"] == "external-system-tool")
shipped_tools = sum(1 for one in logs.iterdir()
                    if one.is_file() and one.suffix in (".py", ".sh")) \
    if logs.is_dir() else 0

record["counts"] = {
    "unique_referenced_tools": len(set(tools_seen) | roster_tools),
    "unique_executed_tools": unique_executed_tools,
    "execution_invocations": execution_invocations,
    "shipped_tools": shipped_tools,
    "trusted_not_executed_tools": len(roster_tools),
    "driver_invocations": driver_invocations,
    "external_system_tool_rows": external_rows,
    "merged_battery_invocations": merged,
}
record["counting_notes"] = {
    "phase_view": (
        "every figure here describes the record AS OF " + phase + ". This "
        "record is rendered before P8, because P8 receives it as --executed, "
        "and re-rendered after the post-seal gates; the two renderings carry "
        "different figures for the same quantities and each says which phase "
        "it is the view from."),
    "execution_invocations": (
        "OBSERVED invocations only. A tool the package ships and this "
        "attempt did not run is in shipped_not_executed, which is a roster "
        "and not a list of invocations; it carries no instant, no phase and "
        "no transcript, because it did not run."),
    "external_system_tool_rows": (
        "REPRESENTATIVE, NOT EXHAUSTIVE. python3 and git are recorded once "
        "each, at the phase that first used them, and run many more times "
        "than that. This figure is the number of ROWS and is never a count "
        "of invocations of those tools."),
    "driver_invocations": (
        "assemble.sh and battery.sh drove the rows rather than being driven. "
        "Each hashes its own bytes at its own preflight and appends one row; "
        "they are EXECUTED and are counted in execution_invocations, and are "
        "named separately here so a reader can tell a driver from a step."),
}
pathlib.Path(out).write_text(
    json.dumps(record, indent=2, sort_keys=True) + "\n", encoding="utf-8")

tools_by_class = {}
invocations_by_class = {}
for row in rows:
    invocations_by_class[row["class"]] = (
        invocations_by_class.get(row["class"], 0) + 1)
for klass in tools_seen.values():
    tools_by_class[klass] = tools_by_class.get(klass, 0) + 1
if roster_tools:
    tools_by_class["shipped-not-executed"] = len(roster_tools)


def tally(counted):
    return ", ".join(f"{name} {number}"
                     for name, number in sorted(counted.items())) or "(none)"


# THE PHASE IS ON THE LINE. A figure without one is the V15 contradiction.
print(f"executed-tools [{phase}]: "
      + ", ".join(f"{name} {number}"
                  for name, number in sorted(record["counts"].items())))
print("executed-tools: tools_by_class " + tally(tools_by_class))
print("executed-tools: invocations_by_class " + tally(invocations_by_class))
print("executed-tools: shipped_not_executed is a ROSTER, not invocations: "
      + ", ".join(sorted(roster_tools) or ["(none)"]))
PY
}
...
# THE EXEC RECORD FOR A PIPELINE STEP.
#
# V16, THE V15 REVIEW: "the package-comparison row omits its required package
# cwd and conflicts with the ledger". It did. `python3 logs/compare-gate.py
# logs/attempt-01/... logs/attempt-02/...` is three package-relative paths
# with nothing anywhere saying what they are relative to, recorded on a row
# whose `cwd` field said `$REPO` -- the implementation checkout, which is not
# where it ran. It ran with `cd "$PKG"`. `$PACKAGE_ROOT` is that directory and
# is the cwd every pipeline row now carries.
#
# Set `STEP_EXEC` (a JSON exec record) and/or `STEP_CWD` before calling
# `step_row`; both are consumed and cleared by it, so a later row cannot
# inherit an earlier row's context by accident.
STEP_EXEC=""
STEP_CWD=""
pkg_exec_record() {  # cwd-token -- argv...
  local cwd=$1
  shift
  [ "${1:-}" = "--" ] && shift
  python3 - "$TOOLS" "$cwd" "$@" <<'PY'
import json, sys
sys.path.insert(0, sys.argv[1])
import catena_command as CC
print(json.dumps(CC.make_record(sys.argv[2], argv=sys.argv[3:]),
                 sort_keys=True))
PY
}
...
step_row() {  # phase command exit start end log
  ...
  attempt_row attempt="$ATTEMPT" attempt_no="$ATTEMPT_NO" record=step \
    side=package sha="${HEAD_SHA:-unknown}" \
    cwd="${STEP_CWD:-\$PACKAGE_ROOT}" phase="$1" \
    order="$ORDER" command="$2" exit="$3" result="exit $3" start="$4" \
    end="$5" log="$6" exec_record="${STEP_EXEC:-}"
  STEP_EXEC=""
  STEP_CWD=""
  # AND INTO THE SLICE, ONCE THE SLICE EXISTS.
  #
  # V15, THE V14 REVIEW: the post-seal gates never reached the shipped
  # history. The IN-PACKAGE `logs/attempts.json` is frozen at P5, manifested
  # at P6 and archived at P7, so P10's rows genuinely cannot enter it -- they
  # do not exist yet, and putting them there afterwards would mean rewriting
  # a sealed archive, which this lane does not do at any price. That half of
  # the limitation is structural and stays.
  #
  # The OUTER SLICE beside the archive is a different file with different
  # rules: P9 derives it from the lane ledger, and it is not inside anything
  # sealed. It could always have carried the gates and did not, purely
  # because it was derived once and never appended to -- and `discard()` and
  # P11 had each already worked around that with their own one-line mirror.
  # Doing it HERE covers every post-P9 row there will ever be, including ones
  # a later phase adds, instead of three call sites remembering to.
  #
  # Before P9 the slice does not exist and this is a no-op, so the file P8
  # and P9 read is exactly the file they read before. The gates at P10 read
  # it BEFORE their own rows are appended, so no gate is ever handed a row
  # about itself. The rows are `record=step` and carry no `status`, which is
  # what `authority-coherence.py`'s external-ledger fold selects on, so the
  # disposition fold sees nothing new. And they arrive before P11's sanitize
  # pass, so they are sanitized and scanned with the rest of the file.
...
overwritten or deleted, and no package was created"
  ...
  # One file, so the record is one record. `RUNS` is inherited by the inner
  # stage ONLY in company with `ASSEMBLE_INNER`, exported on the line above --
  # see the note where `RUNS` is created for the V14 row loss that pairing
  # closes. `TOOLRUNS` travels too, so an override reaches both stages.
  export RUNS TOOLRUNS
...
on this row because this pass rewrites the executed-tool record itself)" \
    ...
    if [ "$SANITIZED" -ne 0 ] || [ "$SCANNED" -ne 0 ] \
      ...
      python3 "$TOOLS/sanitize-and-seal.py" --repo "$REPO" --sanitize-files \
        "$REPO/build/agent-handoffs/$STAMP-$NAME.SUPERSEDED.txt" \
        > /dev/null 2>&1 || true
...
token, or would still be rewritten by the sanitizer" || true
      # AND SAY WHERE THIS ATTEMPT'S TRANSCRIPTS WENT, for the same reason a
      # discard does. A superseded attempt's log root is no more staged into a
      # later package than a discarded one's -- `checks.py` refuses a logs
      # directory carrying either marker -- so the next assembly is refused by
      # the attempt-log audit for a root this one named and never shipped.
      # The discard path already records this; the supersession path did not,
      # and the difference cost an assembly. What is known here is the same:
      # which phase superseded it, where the tree is, and that nothing was
      # staged.
      python3 "$TOOLS/checks.py" --log-root-elsewhere "$ATTEMPT" \
        --attempts "$ATTEMPTS" --lane "$LANE" --reason \
        "this assembly reached final authority and was superseded at P11 when \
the outer siblings did not reach a clean sanitizer fixpoint; its package tree \
is kept unaltered beside its supersession marker under build/agent-handoffs in \
the implementation checkout, it was never staged into any package, and none of \
its bytes is evidence for any claim" >/dev/null 2>&1 || true
      ...
      # ---- P12 the completeness verdict, at the TRUE final state --------
      #
      # THE REQUIRED ORDER, AND THIS IS ITS LAST STEP:
      #
      #   every authoritative log exists -> outer sanitize complete
      #                                  -> outer scan complete
      #                                  -> every required log named
      #                                  -> COMPLETENESS CHECKER RUNS
      #                                  -> final result
      #                                  -> no evidence mutation afterwards
      #
      # Everything above this line has finished. `$SANITIZE_LOG` and
      # `$SCAN_LOG` exist and are clean, the ledger slice has been rescanned,
      # and the sibling set beside the package is closed except for this
      # gate's own transcript.
      #
      # THE ONE EXCEPTION, DECLARED RATHER THAN HIDDEN. `$INVENTORY_LOG` is
      # written BY this pass, so it cannot be an input to it, and it must be
      # sanitized afterwards or a tracked file ships unscanned. That is the
      # only mutation after the verdict, the gate is told about it with
      # `--pending-sibling`, and the re-run below proves the verdict does not
      # move because of it.
      P12_START=$(date -Is)
      INVENTORY_SHA=$(sha256sum "$TOOLS/handoff-inventory.py" | cut -d' ' -f1)
      set +e
      python3 "$TOOLS/handoff-inventory.py" --package "$PKG" \
        --pending-sibling "$(basename "$INVENTORY_LOG")" \
        > "$INVENTORY_LOG" 2>&1
      INVENTORY=$?
      set -e
      ...
      ORDER=$((ORDER + 1))
      attempt_row attempt="$ATTEMPT" attempt_no="$ATTEMPT_NO" record=step \
        side=package sha="${HEAD_SHA:-unknown}" cwd='$PACKAGE_ROOT' \
        phase="P12 final completeness" order="$ORDER" \
        tool=handoff-inventory.py tool_sha256="$INVENTORY_SHA" \
        command="handoff-inventory.py --package PKG --pending-sibling \
$(basename "$INVENTORY_LOG") (the verdict is taken AFTER P11, at the final \
state; the one artifact still to arrive is this gate's own transcript and it \
is declared)" \
        exit="$INVENTORY" result="exit $INVENTORY" \
        start="$P12_START" end="$(date -Is)" \
        log="$(basename "$INVENTORY_LOG")" || true
      # AND THE GATE'S OWN TRANSCRIPT IS SANITIZED AND SCANNED. The declared
      # post-verdict step, and the only one.
      set +e
      python3 "$TOOLS/sanitize-and-seal.py" --repo "$REPO" \
        --sanitize-files "$INVENTORY_LOG" >> "$SANITIZE_LOG" 2>&1
      python3 "$TOOLS/sanitize-and-seal.py" --repo "$REPO" \
        --scan-files "$INVENTORY_LOG" >> "$SCAN_LOG" 2>&1
      INVENTORY_CLEAN=$?
      set -e
      [ -f "$PKG_LEDGER" ] && tail -1 "$ATTEMPTS" >> "$PKG_LEDGER"
      if [ "$INVENTORY" -eq 2 ]; then
        echo "HANDOFF INVENTORY COULD NOT RUN" >&2
        tail -20 "$INVENTORY_LOG" >&2 || true
        STATUS=1
      elif [ "$INVENTORY" -ne 0 ] || [ "$INVENTORY_CLEAN" -ne 0 ]; then
        echo "P12 FAILED: the package is not complete at its final state" >&2
        tail -20 "$INVENTORY_LOG" >&2 || true
        {
          echo "SUPERSEDED ATTEMPT -- NOT THE PACKAGE TO REVIEW"
          echo "state   : superseded"
          echo "attempt : $ATTEMPT"
          echo "stamp   : $STAMP"
          echo "phase   : P12 final completeness"
          echo "reason  : the completeness verdict taken AT THE FINAL STATE"
          echo "          is not COMPLETE (inventory $INVENTORY, transcript"
          echo "          scan $INVENTORY_CLEAN)"
          echo ""
          echo "V15 took this verdict at P10 and P11 then wrote two more"
          echo "authoritative transcripts beside the package, so the shipped"
          echo "COMPLETE described a state the package was no longer in. The"
          echo "verdict is taken here, last, and an attempt that cannot reach"
          echo "COMPLETE here is superseded rather than shipped."
        } > "$REPO/build/agent-handoffs/$STAMP-$NAME.SUPERSEDED.txt" || true
      python3 "$TOOLS/sanitize-and-seal.py" --repo "$REPO" --sanitize-files \
        "$REPO/build/agent-handoffs/$STAMP-$NAME.SUPERSEDED.txt" \
        > /dev/null 2>&1 || true
        attempt_row attempt="$ATTEMPT" attempt_no="$ATTEMPT_NO" record=state \
          side=package sha="${HEAD_SHA:-unknown}" cwd='$PACKAGE_ROOT' \
          phase="P12 final completeness" order="$ORDER" \
          start="$P12_START" end="$(date -Is)" exit=1 result=superseded \
          status=superseded package="$STAMP-$NAME" \
          head="${HEAD_SHA:-unknown}" log="$(basename "$INVENTORY_LOG")" \
          reason="the final-state completeness verdict is not COMPLETE \
(inventory exit $INVENTORY, own-transcript scan exit $INVENTORY_CLEAN)" || true
      # AND SAY WHERE THIS ATTEMPT'S TRANSCRIPTS WENT. Third exit path, same
      # debt: P2's discard records it, P11's supersession records it, and P12's
      # did not -- so the next assembly was refused by the attempt-log audit
      # for the root this one named and never shipped. Every path that ends an
      # attempt owes the record the same statement, because every one of them
      # leaves a log root the next package cannot stage.
      python3 "$TOOLS/checks.py" --log-root-elsewhere "$ATTEMPT" \
        --attempts "$ATTEMPTS" --lane "$LANE" --reason \
        "this assembly reached final authority and was superseded at P12 when \
the final-state completeness verdict was not COMPLETE; its package tree is kept \
unaltered beside its supersession marker under build/agent-handoffs in the \
implementation checkout, it was never staged into any package, and none of its \
bytes is evidence for any claim" >/dev/null 2>&1 || true
        [ ! -f "$PKG_LEDGER" ] || tail -1 "$ATTEMPTS" >> "$PKG_LEDGER" || true
        STATUS=1
      else
        # ---- THE DEFERRED BINDINGS, FILLED --------------------------------
        #
        # P9 wrote the authority record with `outer_scan_result`,
        # `completeness_result` and `completeness_log` empty and DECLARED
        # deferred, because neither verdict existed then. Both exist now.
        # This is the one protocol-permitted mutation after the completeness
        # verdict, and it is accounted for: the fields it fills are the ones
        # the record itself said it would fill, `authority-coherence.py`
        # requires them non-empty on a finished record, and the sanitize and
        # rescan below leave the file at the same fixpoint P11 proved.
        set +e
        python3 - "$AUTHORITY" "$SCAN_LOG" "$(basename "$INVENTORY_LOG")" \
            "$PKG_LEDGER" <<'PY'
import hashlib, json, pathlib, sys

record_path, scan_log, inventory_log, ledger = sys.argv[1:5]
path = pathlib.Path(record_path)
record = json.loads(path.read_text(encoding="utf-8"))

# READ BACK FROM THE TRANSCRIPT, never carried forward from a variable. A
# value carried forward proves the variable, not the pass.
scan = pathlib.Path(scan_log).read_text(encoding="utf-8", errors="replace")
clean = "CLEAN" if "REFUSING" not in scan and "FOUND" not in scan else "DIRTY"

record["outer_scan_result"] = clean
record["completeness_result"] = "COMPLETE"
record["completeness_log"] = inventory_log
# AND THE SLICE, WHOSE BYTES ONLY SETTLE HERE. P9 derived it after writing this
# record, and P10 and P11 each appended their own row to it, so its digest was
# unknowable then and is knowable now. Taken from the file, not carried.
ledger_path = pathlib.Path(ledger)
if ledger_path.is_file():
    record["ledger_sha256"] = hashlib.sha256(ledger_path.read_bytes()).hexdigest()
record["deferred_bindings"] = {}
path.write_text(json.dumps(record, indent=2, sort_keys=True) + "\n",
                encoding="utf-8")
print(f"authority record: outer scan {clean}, completeness COMPLETE, "
      f"log {inventory_log}, ledger {record.get('ledger_sha256', '')[:12]}")
PY
        BOUND=$?
        python3 "$TOOLS/sanitize-and-seal.py" --repo "$REPO" \
          --sanitize-files "$AUTHORITY" >> "$SANITIZE_LOG" 2>&1
        python3 "$TOOLS/sanitize-and-seal.py" --repo "$REPO" \
          --scan-files "$AUTHORITY" >> "$SCAN_LOG" 2>&1
        BOUND_CLEAN=$?
        set -e
        ORDER=$((ORDER + 1))
        attempt_row attempt="$ATTEMPT" attempt_no="$ATTEMPT_NO" record=step \
          side=package sha="${HEAD_SHA:-unknown}" cwd='$PACKAGE_ROOT' \
          phase="P12 bind the deferred authority fields" order="$ORDER" \
          command="fill outer_scan_result, completeness_result and \
completeness_log in $(basename "$AUTHORITY") -- the three fields P9 declared \
deferred because neither verdict existed then -- then sanitize and rescan it" \
          exit="$BOUND/$BOUND_CLEAN" \
          result="bind $BOUND, rescan $BOUND_CLEAN" \
          start="$P12_START" end="$(date -Is)" \
          log="$(basename "$SCAN_LOG")" || true
        [ -f "$PKG_LEDGER" ] && tail -1 "$ATTEMPTS" >> "$PKG_LEDGER"
        if [ "$BOUND" -ne 0 ] || [ "$BOUND_CLEAN" -ne 0 ]; then
          echo "P12 FAILED: the deferred authority bindings were not filled" >&2
          STATUS=1
        else
          echo "P12: COMPLETE at the final state; the deferred authority"
          echo "     bindings are filled and the record rescanned. The only"
          echo "     artifacts written after the verdict are this gate's own"
          echo "     transcript and the three fields the record itself said"
          echo "     it would fill, both of which it accounts for."
        fi
      fi
    fi
  fi
...
# AND THE DRIVER ITSELF, which drove every row above this line and every row
# below it. V15 shipped this file classed `shipped-not-executed` beside its
# own invocation log.
record_driver assemble.sh "$0" "$PHASE" "$(basename "$RUN_LOG")"
...
# EVERY MARKER THIS PIPELINE WRITES GOES THROUGH THE SANITIZER.
#
# V16, THE V15 REVIEW: "locally retained discarded markers and raw retained
# ledgers are outside that scan and preserve builder-local offsets or absolute
# paths, so no broader all-retained-artifacts privacy claim is accepted."
#
# The reported exposure is real and small: a marker carries three `date -Is`
# timestamps with a LOCAL UTC OFFSET, the free-text `reason` composed at the
# failure site, and -- on the failure paths that name a path -- whatever the
# failing phase put in it. None of it is published; all of it sits under
# `build/agent-handoffs/` on the builder's machine. The sanitizer has a rule
# for every one of those classes and simply was never pointed at a marker,
# because `OUTER_SIBLINGS` does not list them and P11 sanitizes that list.
#
# BEST-EFFORT, AND DELIBERATELY SO. A discard path must not fail harder than
# the failure that reached it: if the sanitizer is missing, unreadable or
# itself broken, the marker still exists and still says what happened, which
# is the whole reason it is written. The exit is ignored and the pass is one
# line.
sanitize_marker() {  # path
  [ -f "$1" ] || return 0
  python3 "$TOOLS/sanitize-and-seal.py" --repo "$REPO" \
    --sanitize-files "$1" > /dev/null 2>&1 || true
  return 0
}
...
discard() {
  ...
  # AND THROUGH THE SANITIZER, all three, before anything else happens.
  sanitize_marker "$PKG/DISCARDED.txt"
  sanitize_marker "$PKG/logs/DISCARDED-$ATTEMPT.txt"
  sanitize_marker "$ZIP.DISCARDED.txt"
  # AND SAY WHERE THIS ATTEMPT'S TRANSCRIPTS WENT, NOW, WHILE IT IS KNOWN.
  #
  # The attempt-log audit refuses a package whose shipped rows name a log root
  # the package does not contain -- the V15 defect where a discarded
  # predecessor's root was named and never staged. A discarded attempt is
  # exactly that case: its tree is kept in place beside these markers and is
  # never staged into any package, by construction of the refusal above.
  #
  # Until this ran here, every discard left the NEXT assembly to be refused
  # for the previous one, and an operator had to write the explanation by
  # hand before the pipeline could proceed -- which meant the honest answer
  # arrived only if someone remembered it. The assembler knows all of it at
  # the moment it discards: which phase refused, where the tree is, and that
  # nothing was staged. It is not an exemption from staging; it is the
  # statement that the root is somewhere a reviewer can follow, and a package
  # that simply omitted a root with no such row is still refused.
  python3 "$TOOLS/checks.py" --log-root-elsewhere "$ATTEMPT" \
    --attempts "$ATTEMPTS" --lane "$LANE" --reason \
    "this assembly was discarded at $phase; its partially built tree is kept \
unaltered beside its discard markers under build/agent-handoffs in the \
implementation checkout, it was never staged into any package because \
logs/checks.py refuses a logs directory carrying a discard marker, and none \
of its bytes is evidence for any claim" >/dev/null 2>&1 || true
  if [ -n "$SEALED" ]; then
    attempt_row attempt="$ATTEMPT" attempt_no="$ATTEMPT_NO" record=state \
...
# THE ATTEMPT NAMES ITSELF, AND ONLY THIS PHASE CAN WRITE IT.
#
# The authority gate requires a prose member to name the attempt that reached
# final authority: a package whose documents are silent on the one question it
# exists to settle is refused, and rightly. But the attempt id is minted at P0
# of THIS run -- it carries a random suffix precisely so two attempts can never
# collide -- so no author could have typed it into the source, and a document
# that guessed would be wrong on every run but one.
#
# So the source carries the token `<attempt>` and this phase resolves it, once,
# into the staged copy, before the freeze that makes those bytes evidence. The
# source keeps the token; the package gets the id. It is the same shape as the
# package stem, one field along, and for the same reason: a name that is chosen
# per attempt is written per attempt, by whatever knows it.
if [ -n "${ATTEMPT:-}" ]; then
  for one in "$PKG"/*.md; do
    [ -f "$one" ] || continue
    python3 - "$one" "$ATTEMPT" <<'SUB'
import pathlib, sys
path, attempt = pathlib.Path(sys.argv[1]), sys.argv[2]
text = path.read_text(encoding="utf-8")
if "<attempt>" in text:
    path.write_text(text.replace("<attempt>", attempt), encoding="utf-8")
SUB
  done
fi
...
# THE ROW SAYS WHERE IT RAN, AND THE RECORD IS THE ARGV THAT RAN. Both were
# missing in V15; the paths below are exactly the two the comparison was
# handed, discovered from the staged tree rather than typed.
if [ -n "$GATE_PARENT" ] && [ -n "$GATE_HEAD" ]; then
  STEP_EXEC=$(pkg_exec_record '$PACKAGE_ROOT' -- python3 logs/compare-gate.py \
    "$GATE_PARENT" "$GATE_HEAD") || STEP_EXEC=""
fi
STEP_CWD='$PACKAGE_ROOT'
...
run_tool checks.py "$PKG/logs/checks.py" "$PHASE" \
  ...
  -- python3 -B "$PKG/logs/checks.py" --audit-logs --package "$PKG" \
...
invocation log is the transcript)" \
  -- python3 -B "$PKG/logs/checks.py" --seal-ledger --package "$PKG" \
  --attempts "$ATTEMPTS" --attempt "$ATTEMPT" --attempt-no "$ATTEMPT_NO" \
...
python3 - "$ZIP" "$VERIFY_LOG" "$AUTHORITY" "$ATTEMPT" "$STAMP-$NAME" \
    "$HEAD_SHA" "$LANE" "$PARENT" "$REVIEW" "$PKG_LEDGER" <<'PY'
import datetime, hashlib, json, pathlib, re, sys, zipfile

(zip_path, verify_log, out, attempt, package, head,
 lane, parent, review, ledger) = sys.argv[1:11]
...
# THE LEDGER SLICE'S OWN BYTES. V15 named the ledger and never hashed it, so
# a reader holding two slices could not tell which one the record meant.
ledger_path = pathlib.Path(ledger)
ledger_sha = ""
if ledger_path.is_file():
...
record = {
    "schema": "catena-final-authority/2",
    "attempt": attempt,
    "package": package,
    "head": head,
    ...
    #
    # V16, THE V15 REVIEW: the parent and the predecessor review were "bound
    # transitively through the archive and the final evidence commit". Each
    # of these now names the thing itself, in the record a reader opens
    # first.
    "lane": lane,
    "parent": parent,
    "review_commit": review,
    "ledger_name": ledger_path.name,
    # ---- /2: DECLARED HERE, FILLED AT P12 --------------------------------
    #
    # This is P9. The outer scan is P11 and the final completeness verdict is
    # P12, and a record written here carrying either value would carry a
    # verdict that did not exist -- which is precisely the fault the V13
    # phase order was built to end. They are empty, they are DECLARED empty,
    # and `--bind-final` fills them once both verdicts exist.
    "outer_scan_result": "",
    "completeness_result": "",
    "completeness_log": "",
    "ledger_sha256": "",
    "deferred_bindings": {
        "ledger_sha256":
            "the per-package ledger slice is DERIVED after this record is "
            "written, and P10 and P11 each append their own row to it, so "
            "its bytes do not settle until the last of them has run; a "
            "digest taken here would name a file that does not exist yet "
            "and would be wrong about the one that eventually does",
        "outer_scan_result":
            "P11 sanitizes and rescans the outer siblings AFTER this record "
            "is written; its verdict does not exist yet",
        "completeness_result":
            "P12 takes the completeness verdict at the final state, AFTER "
            "P11; V15 took it at P10 and it went stale when P11 wrote two "
            "more authoritative transcripts beside the package",
        "completeness_log":
            "the transcript P12 writes; it cannot be an input to the pass "
            "that writes it",
    },
    # ---- /2: THE ONE THAT CANNOT BE BOUND DIRECTLY -----------------------
    "evidence_commit": "",
    "evidence_commit_note":
        "The commit that carries this record cannot be named inside it: "
        "naming it would change these bytes and therefore the commit. What "
        "is bound directly is everything that commit contains -- the "
        "archive's basename, byte size and SHA-256, the package basename, "
        "the exact head, the exact parent, the review commit this package "
        "answers, and the attempt ledger slice's own digest -- so a reader "
        "resolves the evidence commit as the commit whose tree carries this "
        "file, and nothing about the package's identity depends on finding "
        "it.",
}
pathlib.Path(out).write_text(
    json.dumps(record, indent=2, sort_keys=True) + "\n", encoding="utf-8")
...
run_tool authority-coherence.py "$TOOLS/authority-coherence.py" "$PHASE" \
  ...
  --lane "$LANE" --parent "$PARENT" ${REVIEW:+--review "$REVIEW"} \
  --bindings-pending \
  > "$COHERENCE_LOG" 2>&1
COHERENCE=$?
set -e
...
# THE COMPLETENESS GATE IS NOT HERE ANY MORE. IT IS P12, IN THE OUTER STAGE.
#
...
#
# The mechanism, exactly. `handoff-inventory.py` DISCOVERS the artifacts
# beside the package rather than being told them, which is right and is the
# fix for a V12 defect. It ran HERE, at P10, in the inner stage. P11 then runs
# in the OUTER stage -- it has to, because `$RUN_LOG` is held open by the
# `tee` the inner stage runs under -- and writes two more transcripts beside
# the package. At the instant the verdict was taken there were ten siblings
# and all ten were named; a minute later there were twelve and two were not.
# The verdict was true when it was taken and false by the time it shipped.
#
# A verdict about a final state has to be taken AT the final state. So the
# gate moved to P12, after P11's sanitize and scan, where the sibling set is
# closed. What follows P12 is exactly one thing -- the sanitize and scan of
# P12's OWN transcript, which cannot be an input to the pass that writes it --
# and the gate accounts for that itself: it is handed `--pending-sibling` for
# its own log, and re-running it afterwards yields the same verdict. That
# re-run is the test.
