...
# V16, /2: the exact parent, the exact review commit and the lane are
# bound DIRECTLY. V15 bound the first two only transitively, through
# the archive and the final evidence commit.
PARENT_HEAD = "3333333333333333333333333333333333333333"
REVIEW_COMMIT = "4444444444444444444444444444444444444444"
LANE = "V13"
...
def sound_authority(digest, size, ledger_digest="0" * 64):
    ...
    return {
        ...
        "ledger_name": f"{NAME}.attempts.jsonl",
        "ledger_sha256": ledger_digest,
        # ---- /2: FILLED AFTER P11 AND P12, BY --bind-final -------------
        "outer_scan_result": "CLEAN",
        "completeness_result": "COMPLETE",
        "completeness_log": f"{NAME}.handoff-inventory.log",
        # ---- /2: THE ONE THAT CANNOT BE BOUND, AND SAYS SO -------------
        "evidence_commit": "",
        "evidence_commit_note": (
            "The commit that carries this record cannot be named inside it: "
            "naming it would change these bytes and therefore the commit. "
            "What is bound directly is everything that commit contains -- the "
            "archive's digest, the package basename, the head, the parent, "
            "the review commit and the ledger's digest -- and a reader "
            "resolves the evidence commit as the commit whose tree carries "
            "this file."),
        "deferred_bindings": {},
    }
...
class Gate(unittest.TestCase):
    ...
    def build(self, ledger=None, outer=None, prose=None, marker=None,
        ...
        if verify is not OMIT:
            (root / f"{NAME}.verify-final.log").write_text(
                ...
                encoding="utf-8")
        # THE LEDGER IS WRITTEN FIRST, because the /2 record BINDS ITS
        # DIGEST. V15 named the ledger and never its bytes, so a reader
        # holding two slices could not tell which one the record meant.
        ledger_digest = "0" * 64
        if external is not OMIT:
            ...
            (root / f"{NAME}.attempts.jsonl").write_text(
                body, encoding="utf-8")
            ledger_digest = hashlib.sha256(body.encode("utf-8")).hexdigest()

        if authority is not OMIT:
            record = sound_authority(digest, size, ledger_digest)
            if authority:
    ...
    # -- V16, DEFECT 10: EVERY /2 FIELD IS GATED -------------------------
    #
    # "The V16 authority sidecar must DIRECTLY bind: exact V16 head; exact
    # V15 parent; the V15 review commit; the evidence commit; package
    # basename; size; SHA-256; P8 verdict; final completeness verdict;
    # outer-scan verdict; authoritative attempt/ledger identity. Avoid
    # transitive-only binding."
    #
    # One positive and one matched negative per field.

    def test_the_v1_schema_is_refused_and_the_reason_is_named(self):
        code, said = self.run_gate(self.build(
            authority={"schema": "catena-final-authority/1"}))
        self.assertEqual(code, 1, said)
        self.assertIn("bound the parent and the predecessor review only "
                      "transitively", said)

    def test_a_record_that_binds_no_parent_is_refused(self):
        for value in (OMIT, "", "not-a-sha", "3" * 39):
            with self.subTest(value=value):
                code, said = self.run_gate(self.build(
                    authority={"parent": value}))
                self.assertEqual(code, 1, said)

    def test_a_record_binding_the_wrong_parent_is_refused(self):
        code, said = self.run_gate(
            self.build(authority={"parent": "9" * 40}),
            "--parent", PARENT_HEAD)
        self.assertEqual(code, 1, said)
        self.assertIn("authoritative against parent", said)

    def test_a_record_binding_the_right_parent_passes(self):
        code, said = self.run_gate(self.build(), "--parent", PARENT_HEAD)
        self.assertEqual(code, 0, said)

    def test_a_record_that_binds_no_review_commit_is_refused(self):
        code, said = self.run_gate(self.build(
            authority={"review_commit": ""}))
        self.assertEqual(code, 1, said)
        self.assertIn("bound HERE and not transitively", said)

    def test_a_record_binding_the_wrong_review_is_refused(self):
        code, said = self.run_gate(
            self.build(authority={"review_commit": "9" * 40}),
            "--review", REVIEW_COMMIT)
        self.assertEqual(code, 1, said)
        self.assertIn("answers review", said)

    def test_a_record_binding_the_right_review_passes(self):
        code, said = self.run_gate(self.build(), "--review", REVIEW_COMMIT)
        self.assertEqual(code, 0, said)

    def test_a_record_naming_no_lane_is_refused(self):
        code, said = self.run_gate(self.build(authority={"lane": ""}))
        self.assertEqual(code, 1, said)
        self.assertIn("names no lane", said)

    def test_a_record_binding_the_wrong_lane_is_refused(self):
        code, said = self.run_gate(self.build(authority={"lane": "V9"}),
                                   "--lane", LANE)
        self.assertEqual(code, 1, said)
        self.assertIn("is not 'V13'", said)

    def test_a_ledger_digest_that_does_not_hash_is_refused(self):
        """V15 named the ledger and never its bytes, so a reader holding two
        slices could not tell which one the record meant."""
        code, said = self.run_gate(self.build(
            authority={"ledger_sha256": "a" * 64}), "--ledger", "IGNORED")
        self.assertEqual(code, 1, said)

    def test_the_ledger_digest_is_recomputed_and_matches(self):
        package = self.build()
        ledger = package.parent / f"{NAME}.attempts.jsonl"
        code, said = self.run_gate(package, "--ledger", str(ledger))
        self.assertEqual(code, 0, said)

    def test_a_record_naming_the_wrong_ledger_is_refused(self):
        package = self.build(authority={"ledger_name": "somebody-else.jsonl"})
        ledger = package.parent / f"{NAME}.attempts.jsonl"
        code, said = self.run_gate(package, "--ledger", str(ledger))
        self.assertEqual(code, 1, said)
        self.assertIn("names ledger", said)

    def test_a_still_deferred_completeness_verdict_is_refused(self):
        """P9 declares it deferred; a FINISHED record has it bound. V15's
        completeness verdict was taken at P10 and went stale at P11."""
        code, said = self.run_gate(self.build(authority={
            "completeness_result": "",
            "deferred_bindings": {
                "completeness_result": "P12 has not run"}}))
        self.assertEqual(code, 1, said)
        self.assertIn("is still deferred", said)

    def test_an_empty_undeclared_binding_is_refused(self):
        """A binding is present or explained, never absent and silent."""
        code, said = self.run_gate(self.build(
            authority={"outer_scan_result": ""}))
        self.assertEqual(code, 1, said)
        self.assertIn("is not declared deferred", said)

    def test_an_incomplete_verdict_cannot_be_authoritative(self):
        """A record does not get to be authoritative with an excuse."""
        for key, value in (("completeness_result", "INCOMPLETE"),
                           ("outer_scan_result", "DIRTY")):
            with self.subTest(key=key):
                code, said = self.run_gate(self.build(
                    authority={key: value}))
                self.assertEqual(code, 1, said)
                self.assertIn("does not get to be authoritative with an "
                              "excuse", said)

    def test_a_bound_verdict_pair_passes(self):
        """THE MATCHED POSITIVE for the two above."""
        code, said = self.run_gate(self.build())
        self.assertEqual(code, 0, said)

    def test_a_filled_evidence_commit_is_refused(self):
        """Naming the commit inside the record changes the bytes and
        therefore the commit; it is always empty."""
        code, said = self.run_gate(self.build(
            authority={"evidence_commit": "5" * 40}))
        self.assertEqual(code, 1, said)
        self.assertIn("it is always empty", said)

    def test_an_unexplained_evidence_commit_gap_is_refused(self):
        """The gap is stated, not silent."""
        code, said = self.run_gate(self.build(
            authority={"evidence_commit_note": "n/a"}))
        self.assertEqual(code, 1, said)
        self.assertIn("evidence_commit_note is empty or too short", said)

    def test_bindings_pending_accepts_a_declared_deferral(self):
        """THE P10 RUN, which happens before P11 and P12 exist."""
        code, said = self.run_gate(self.build(authority={
            "outer_scan_result": "",
            "completeness_result": "",
            "completeness_log": "",
            "deferred_bindings": {
                "outer_scan_result": "P11 runs after this record is written",
                "completeness_result": "P12 runs after this record is written",
                "completeness_log": "written by P12"}}), "--bindings-pending")
        self.assertEqual(code, 0, said)

    def test_bindings_pending_still_refuses_a_silent_gap(self):
        """THE MATCHED NEGATIVE: pending excuses an UNWRITTEN binding, never
        an undeclared one."""
        code, said = self.run_gate(self.build(authority={
            "outer_scan_result": "",
            "deferred_bindings": {}}), "--bindings-pending")
        self.assertEqual(code, 1, said)
        self.assertIn("is not declared deferred", said)

    def test_bindings_pending_still_refuses_an_illegal_written_value(self):
        """A value that IS written must be a legal one; only its ABSENCE is
        what pending excuses."""
        code, said = self.run_gate(self.build(authority={
            "completeness_result": "INCOMPLETE"}), "--bindings-pending")
        self.assertEqual(code, 1, said)
        self.assertIn("does not get to be authoritative with an excuse", said)

    def test_the_pre_p8_reading_allows_the_deferred_fields(self):
        """A record written at P9 legitimately carries neither verdict: the
        outer scan is P11 and the completeness verdict is P12."""
        code, said = self.run_gate(self.build(authority={
            "outer_scan_result": "",
            "completeness_result": "",
            "completeness_log": "",
            "deferred_bindings": {
                "outer_scan_result": "P11 runs after this record is written",
                "completeness_result": "P12 runs after this record is written",
                "completeness_log": "written by P12"}}), "--pre-p8")
        self.assertNotIn("is still deferred", said)

    # -- V16: THE TWO AXES, IN SHIPPED BYTES -----------------------------
    #
    # This gate's vocabulary was one set per side, so it could not tell an
    # attempt's EXECUTION disposition -- how the run ended -- from its
    # EVIDENCE disposition -- whether a completed cohort's measurements are
    # the ones this package reports. Two consequences, both real:
    #
    #   * `abandoned` was in no vocabulary at all, so a package shipping the
    #     honest record of a run something outside it stopped was REFUSED
    #     for shipping it;
    #   * a battery cohort recorded `authoritative` -- "these are the figures
    #     this package reports", which is knowable at P5 because the package
    #     is shipping them -- was refused as though it had claimed to be the
    #     package under review.
    #
    # The package-side meaning of the word is untouched: see the four tests
    # above this block, which still refuse a held claim of final authority in
    # bytes frozen before P8.

    def _battery_evidence(self, status, terminal="complete", reason="stated"):
        ledger = sound_ledger()
        for row in ledger["rows"]:
            if row["attempt"] == BATTERY:
                row["status"] = terminal
                if terminal in ("failed", "abandoned"):
                    row["reason"] = "the run this row is about"
        ledger["rows"].append(state_row(BATTERY, "head", status,
                                        reason=reason))
        for one in ledger["attempts"]:
            if one["attempt"] == BATTERY:
                one["status"] = status
                one["reason"] = reason
                if terminal in ("failed", "abandoned"):
                    one["terminal_reason"] = "the run this row is about"
        return ledger

    def test_a_battery_cohort_recorded_authoritative_is_accepted(self):
        """THE POSITIVE THIS GATE HAD NO WAY TO EXPRESS. A cohort whose
        figures the package reports is authoritative ON ITS OWN AXIS, and as
        many cohorts as a package ships may be."""
        code, said = self.run_gate(self.build(
            self._battery_evidence("authoritative",
                                   reason="this cohort measured the checkout "
                                          "this package reports")))
        self.assertEqual(code, 0, said)
        self.assertIn("PASS (0 problems)", said)
        self.assertIn("one authoritative attempt", said)

    def test_a_battery_written_authoritative_as_its_disposition_is_refused(
            self):
        """THE MATCHED NEGATIVE, and V11's root defect: `authoritative` as the
        battery's own TERMINAL row is one word for two facts."""
        ledger = sound_ledger()
        for row in ledger["rows"]:
            if row["attempt"] == BATTERY:
                row["status"] = "authoritative"
    ...
    def test_battery_evidence_on_a_failed_attempt_is_refused(self):
        for status in ("authoritative", "set-aside"):
            for terminal in ("failed", "abandoned"):
                with self.subTest(status=status, terminal=terminal):
                    code, said = self.run_gate(self.build(
                        self._battery_evidence(status, terminal=terminal,
                                               reason="a stated reason")))
                    self.assertEqual(code, 1, said)
                    self.assertIn(terminal, said)

    def test_an_abandoned_battery_is_accepted_as_resolved_history(self):
        """A run something outside the battery stopped is a real disposition
        with a real reason, and a package that ships it is more honest than
        one that deleted it. This gate refused such a package outright."""
        ledger = sound_ledger()
        why = ("the process running this battery was stopped from outside it "
               "and no step failed")
        for row in ledger["rows"]:
            if row["attempt"] == BATTERY:
        ...
        for one in ledger["attempts"]:
            if one["attempt"] == BATTERY:
                one["status"] = "abandoned"
                one["reason"] = why
        code, said = self.run_gate(self.build(ledger))
        self.assertEqual(code, 0, said)
        self.assertIn("PASS (0 problems)", said)

    def test_an_abandoned_battery_with_no_reason_is_refused(self):
        """THE MATCHED NEGATIVE. Abandonment being expressible does not make
        it free: it is not a success, so it says why."""
        ledger = sound_ledger()
        for row in ledger["rows"]:
            if row["attempt"] == BATTERY:
                row["status"] = "abandoned"
        ...
        code, said = self.run_gate(self.build(ledger))
        self.assertEqual(code, 1, said)
        self.assertIn("abandoned with no reason", said)
