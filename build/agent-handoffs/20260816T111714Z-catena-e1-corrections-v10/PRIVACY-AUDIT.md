...
semantics unchanged through V9 and V10, its ordering as V9 corrected it —
runs over the package in the phases `logs/assemble.sh` states:
...
the binding header naming the exact ZIP basename, byte size and SHA-256,
then sidecar digest and size, layout, manifest, every claimed row against
the delivered bytes, the rows/derived partition with duplicate-row checks,
the derived final-byte totals, the re-rendered claims prose, and the
replayed consistency and sanitization audits over the extraction. Both are
read-only; running the verifier twice is proved to change nothing.
