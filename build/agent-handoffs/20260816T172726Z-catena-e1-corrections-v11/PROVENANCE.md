...
another. The enumerated authority is `logs/attempts.json`, not this prose: it names
every battery this package was built from AND every assembly attempt made
under this lane's name, discarded ones included, and gives each one a
terminal disposition with exactly one reason. Where this prose and that
member could disagree, the member is the record.

1. **Four assembly attempts were discarded, and no stamp was reused.** Each
   refused rather than overwrote, each left a partially built directory
   carrying its own discard marker naming the attempt, the phase, the one
   reason and the exact head, and each was superseded by a fresh UTC stamp:

   - `package-20260816T171438Z-03g4tknr`, stopped at P1 — the sealer's own
     test suite failed, and a package sealed by an unproven sealer is not
     evidence of privacy;
   - `package-20260816T171652Z-046xn6k4`, stopped at P2 — the first
     normalize pass failed, because `screenshots/INDEX.md` named its own
     ...
     thirty-one missing ones;
   - `package-20260816T172004Z-05jnhq7t`, stopped at P2 — the same audit,
     one reference further: `PRIVACY-AUDIT.md` named a sidecar by its bare
     suffix in prose;
   - `package-20260816T172038Z-069xq446`, stopped at P2 — the same audit
     again: this file named three artifacts that live outside the package or
     under a different member path.

   `package-20260816T172114Z-07835xnr` is the attempt that survived, and it
   is the one this package was built from. Every one of the five is in
   `logs/attempts.json` with its own terminal disposition and its one reason.

2. **The first of those four was a seam, not a guard failure, and is worth
...
3. **The attempt ledger was cleaned once, and the raw one is kept.** The
   ...
   deleted: `logs/attempts.json` carries this lane's attempts — the head
   battery, the parent battery, and every assembly attempt — and the
