#!/usr/bin/env bash
# Assemble, derive, audit and seal the V8 handoff package.
#
...
PKG="$REPO/build/agent-handoffs/$STAMP-catena-e1-corrections-v8"
...
( cd "$REPO/build/agent-handoffs" \
  && python3 - "$STAMP-catena-e1-corrections-v8" <<'PY'
...
( cd "$REPO/build/agent-handoffs" \
  && sha256sum "$STAMP-catena-e1-corrections-v8.zip" \
     > "$STAMP-catena-e1-corrections-v8.zip.sha256" \
  && cat "$STAMP-catena-e1-corrections-v8.zip.sha256" )
