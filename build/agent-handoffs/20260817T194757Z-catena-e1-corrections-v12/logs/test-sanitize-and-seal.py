    ...
    def test_a_battery_terminates_complete_and_never_authoritative(self):
        ...
        with tempfile.TemporaryDirectory() as scratch:
            ...
            attempts = Path(scratch) / "attempt-ledger.jsonl"
                ...
                ["bash", str(HERE / "battery.sh"), str(repo), str(logs),
                 "head"],
                env=unpiped(ATTEMPTS=str(attempts)),
                capture_output=True, text=True)
            self.assertEqual(0, done.returncode, done.stderr)
    ...
    def test_a_second_battery_run_gets_a_new_root_and_refuses_the_old_one(self):
        ...
        with tempfile.TemporaryDirectory() as scratch:
            ...
            (second / "order-head.txt").unlink()
                ...
                capture_output=True, text=True)
            self.assertNotEqual(0, done.returncode)
            self.assertIn("log root already exists", done.stderr)
...
class AttemptAuthority(unittest.TestCase):
    """V12: `authoritative` names a package, not a run.

    The reviewed lane's `logs/attempts.json` marked THREE attempts
    authoritative -- the head battery, the parent battery, and a package
    attempt already superseded -- while the package that shipped described
    itself as unresolved. The state machine is written out in `assemble.sh`'s
    header; `checks.py --audit-authority` is its executable copy and these are
    its refusals.
    """

    HEAD = "e" * 40

    def test_a_battery_terminal_row_written_authoritative_is_rejected(self):
        rows = [_terminal("head-a", 1, "head", "authoritative")]
        found = checks.audit_authority(rows)
        self.assertTrue(any("never 'authoritative'" in one for one in found),
                        found)

    def test_two_authoritative_package_attempts_are_rejected(self):
        rows = [_terminal("pkg-a", 7, "package", "authoritative"),
                _terminal("pkg-b", 8, "package", "authoritative")]
        found = checks.audit_authority(rows)
    ...
    def test_an_attempt_both_authoritative_and_superseded_is_rejected(self):
        """Two dispositions for one attempt, which is the pair the ledger
        exists to prevent. The legal shape is the test above: one terminal
        row, and a later record=state row that supersedes it."""
        rows = [_terminal("pkg-a", 7, "package", "authoritative"),
        ...
        # And the mirror: two real dispositions on one attempt are the same
        # refusal said the other way round.
        both = checks.audit_authority(
            [_terminal("pkg-a", 7, "package", "authoritative"),
             _terminal("pkg-a", 7, "package", "discarded")])
        self.assertTrue(any("2 terminal rows" in one for one in both), both)

    def test_discarded_then_authoritative_is_an_illegal_transition(self):
        rows = [_terminal("pkg-a", 7, "package", "discarded"),
    ...
    def test_the_shipped_ledger_names_this_package_and_this_head(self):
        """The row is terminal, it is in the member, and it says which
        package and which head it is about -- which is what the reviewed
        package's own ledger could not say about itself."""
        rows = [
            _state("head-a", 1, "head", "started"),
            _terminal("head-a", 1, "head", "complete", head=self.HEAD),
            _state("pkg-a", 7, "package", "started"),
            _state("pkg-a", 7, "package", "sealing"),
            _terminal("pkg-a", 7, "package", "authoritative",
                      package="20260101T000000Z-pkg", head=self.HEAD),
        ]
        self.assertEqual([], checks.audit_authority(rows))
        with tempfile.TemporaryDirectory() as scratch:
            ...
            with contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
                code = checks.main(
                    ["--seal-ledger", "--package", str(root),
                     "--attempts", str(attempts), "--attempt", "pkg-a",
                     "--attempt-no", "7",
                     "--package-name", "20260101T000000Z-pkg",
                     "--head", self.HEAD])
            self.assertEqual(0, code, err.getvalue())
            shipped = json.loads(
                (root / "logs" / "attempts.json").read_text(encoding="utf-8"))
            mine = [one for one in shipped["attempts"]
                    if one["attempt"] == "pkg-a"]
            self.assertEqual([{"attempt": "pkg-a", "reason": "",
                               "status": "authoritative"}], mine)
            terminal = [one for one in shipped["rows"]
                        if one["attempt"] == "pkg-a"
                        and one["record"] == "attempt"]
            self.assertEqual(1, len(terminal))
            self.assertEqual("20260101T000000Z-pkg", terminal[0]["package"])
            self.assertEqual(self.HEAD, terminal[0]["head"])

    def test_the_seal_ledger_write_refuses_a_wrong_package_name(self):
        rows = [_terminal("pkg-a", 7, "package", "authoritative",
                          package="20260101T000000Z-other", head=self.HEAD)]
        with tempfile.TemporaryDirectory() as scratch:
            root = Path(scratch) / "package"
            (root / "logs").mkdir(parents=True)
            attempts = Path(scratch) / "attempt-ledger.jsonl"
            ...
            with contextlib.redirect_stdout(out), contextlib.redirect_stderr(err):
                code = checks.main(
                    ["--seal-ledger", "--package", str(root),
                     ...
                     "--head", self.HEAD])
            self.assertEqual(1, code)
            self.assertIn("AUTHORITY AUDIT FAILED", err.getvalue())
            self.assertFalse((root / "logs" / "attempts.json").exists(),
