...
An earlier package of this lane sits on the same evidence branch and is
superseded by this one. It is not deleted. Its figures are identical — the
same battery at the same head — and its head ledger recorded each command's
start, end and exit without recording the command, so its `checks.txt` said
`command : (not recorded)` for the whole head half. `LIMITATIONS.md` §12 sets
out what happened and whose error it was.

Read this package. The other one is kept so that account can be checked.
