...
This package mutates nothing. It names, and does not touch, the V6 package.

It DOES supersede one thing: an earlier package of this same lane, at this
same head, whose head-side ordering ledger recorded each command's start, end
and exit without recording the command itself. Both are on the evidence
branch; the earlier one is kept rather than deleted, so that account can be
checked. `LIMITATIONS.md` §12 says what happened and whose error it was.
